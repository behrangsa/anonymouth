...
public class MenuDriver {
	...
	/**
	 * Constructor, takes care of everything, all you need to do is initialize it
	 * and you're listeners are good to go.
	 *
	 * @param main
	 *            GUIMain instance
	 */
	...
	/**
	 * Initializes all menu bar listeners. Should be called sometime during startup
	 * before main window is displayed. Any additional menu bar items should have
	 * their listeners be placed here.
	 *
	 * @param main
	 *            GUIMain instance
	 */
	protected void initListeners() {
		...
		saveAsTestDocListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				 ...
				 */
				/*
				 * File dir; if (savedPath != "") { dir = new File(savedPath);
				 * FileHelper.save.setCurrentDirectory(dir); } else { try { dir = new File(new
				 * File(main.preProcessWindow.ps.getTestDocs().get(ANONConstants.DUMMY_NAME).get
				 * (0).getFilePath()).getCanonicalPath());
				 * FileHelper.save.setCurrentDirectory(dir); } catch (IOException e1) {
				 * Logger.logln(
				 * NAME+"Something went wrong while trying to set the opening directory for the JFileChooser"
				 * , LogOut.STDERR); } }
				 * 
				 * FileHelper.save.setSelectedFile(new File("anonymizedDoc.txt"));
				 * FileHelper.save.addChoosableFileFilter(new ExtFilter("txt files (*.txt)",
				 * "txt")); int answer = FileHelper.save.showSaveDialog(main);
				 * 
				 * if (answer == JFileChooser.APPROVE_OPTION) { File f =
				 * FileHelper.save.getSelectedFile(); String path = f.getAbsolutePath();
				 */
				...
				if (savedPath != "") {
					...
					try {
						FileHelper.goodSave.setDirectory(new File(main.preProcessWindow.ps.getTestDocs()
								.get(ANONConstants.DUMMY_NAME).get(0).getFilePath()).getCanonicalPath());
		...
		aboutListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				JOptionPane.showMessageDialog(null, About.aboutAnonymouth, "About Anonymouth",
						JOptionPane.INFORMATION_MESSAGE, ThePresident.aboutLogo);
			}
		};
		...
		undoListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// Logger.logln(NAME+"Issue occurred while waiting for version control thread to
				// sleep");
				// }
				// }
				// }
		...
		hideAnonymityBarListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				main.documentPanel
						.setLayout(new MigLayout("fill, wrap, ins 0, gap 0 0", "[grow, fill]", "[grow, fill][]"));
		...
		// Print functionality, not yet implemented or complete, we'll get around to it
		// at some point
		/*
		 * printMenuItemListener = new ActionListener() {
		 * 
		 * @Override public void actionPerformed(ActionEvent e) { PrinterJob job =
		 * PrinterJob.getPrinterJob(); PrintRequestAttributeSet aset = new
		 * HashPrintRequestAttributeSet(); PageFormat pf = job.pageDialog(aset);
		 * job.setPrintable(new PrintDialogExample(), pf); boolean ok =
	...
	private void updateStates() {
		...
		if (main.anonymityBarState == ANONConstants.STATE.VISIBLE
				&& main.suggestionTabsState == ANONConstants.STATE.VISIBLE) {
			main.setLayout(new MigLayout("wrap 3, gap 10 10", // layout constraints
			...
			main.add(main.helpersTabPane, "width :353:353, spany, shrinkprio 1"); // RIGHT (Word Suggestions,
																					// Translations, etc.)
			// If Anonymity Bar is visible but Suggestion Tabs are hidden
		} else if (main.anonymityBarState == ANONConstants.STATE.VISIBLE
				&& main.suggestionTabsState == ANONConstants.STATE.HIDDEN) {
			main.setLayout(new MigLayout("wrap 2, gap 10 10", // layout constraints
					"[][grow, fill]", // column constraints
					"[grow, fill]")); // row constraints
			main.add(main.anonymityPanel, "width 80!, spany, shrinkprio 1"); // LEFT (Anonymity bar, results)
			main.add(main.editorTabPane, "width 100:400:, grow, shrinkprio 3"); // MIDDLE (Editor)
			// If Anonymity Bar is hidden but Suggestion Tabs are visible
		} else if (main.anonymityBarState == ANONConstants.STATE.HIDDEN
				&& main.suggestionTabsState == ANONConstants.STATE.VISIBLE) {
			main.setLayout(new MigLayout("wrap 3, gap 10 10", // layout constraints
					"[grow, fill][shrink]", // column constraints
					"[grow, fill]")); // row constraints
			...
			main.setLayout(new MigLayout("gap 10 10", // layout constraints
					"[grow, fill][shrink]", // column constraints
					"[grow, fill]")); // row constraints
	...
	/**
	 * Saves the document to anonymize. Treats this as a regular "Save" if the
	 * document has already been "Save As..."ed once or as a "Save As..." if the
	 * user hasn't yet (so we don't just write over their original, and we don't
	 * have another file to do a regular save to yet).
	 *
	 * <p>
	 * This is in a separate method to allow the save operation to be called outside
	 * for such features as auto-save.
	 *
	 * @param main
	 *            GUIMain instance
	 */
