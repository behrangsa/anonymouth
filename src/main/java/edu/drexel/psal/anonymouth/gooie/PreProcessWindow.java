...
/**
 * The new home for the preProces panel. It is now acting as a set-up wizard as
 * opposed to a main component in GUIMain based on user feedback.
 * 
 * This is the "Window" class for preProcessing, which just handles creating and
 * adding swing components and "isSomething"/"hasSomething" methods pertaining
 * to the problem set. As a guideline, any sort of "update" methods should be
 * handled in the "Driver" class though as a rule all listeners/events should be
 * handled in the "Driver" class.
 * 
...
public class PreProcessWindow extends JDialog {
	...
	/**
	 * Initializes the Test doc panel, which allows the user to add the document
	 * they want to anonymize
	 */
	...
	/**
	 * Initializes the Sample Docs panel, which allows the user to add other
	 * documents they have written to a list
	 */
	...
	/**
	 * Initializes the Train doc panel, which allows the user to add other user's
	 * documents in a tree for Anonymouth to test against
	 */
	...
	/**
	 * Initializes the "done" panel, which tells the user that their document set is
	 * complete and whether or not they'd like to save it
	 */
	private void initDonePanel() {
		...
		doneSaveLabel = new JLabel(
				"<html><center>Save your document set for quick<br>loading and starting in the future</center></html>");
	...
	/**
	 * Goes through all titles in the train docs part of the problem set and changes
	 * the Anonymouth-reference titles of those that Have the same title
	 * 
	 * This is needed since, while we do guide the user to rename files of the same
	 * name for a given author, we want to allow them To enter documents of the same
	 * name for two different authors (sort of like in file systems how I can have
	 * two documents of the same name in two different directories). This helps make
	 * things less confusing/irritating for the user
	 */
	...
	/**
	 * Determines whether or not all the various parts of the document set are ready
	 * to begin
	 * 
	 * @return
	 */
	...
	/**
	 * Determines whether or not the user has entered enough sample documents to
	 * proceed
	 * 
	 * @return
	 */
	...
	protected boolean trainDocsReady() {
		try {
			...
			// 4 because we want to adjust for the user as one of the authors (which we
			// don't want to count)
			if (size == 0 || size < 4)
				result = false;
			else {
	...
	/**
	 * Determines whether or not the user has entered at least three different
	 * authors for the train documents
	 * 
	 * @return
	 */
	...
	/**
	 * To be called only in PreProcessWindowDriver when we are switching to the
	 * "test" panel in the set-up wizard
	 */
	...
	/**
	 * To be called only in PreProcessWindowDriver when we are switching to the
	 * "sample" panel in the set-up wizard
	 */
	...
	/**
	 * To be called only in PreProcessWindowDriver when we are switching to the
	 * "train" panel in the set-up wizard
	 */
	...
	/**
	 * To be called only in PreProcessWindowDriver when we are switching to the
	 * "Done" panel in the set-up wizard
	 */
	protected void switchingToDone() {
		...
		if (saved) {
			/**
			 * TODO This doesn't work when the user has a loaded document set, clicks all
			 * the way through modify without changing anything, then clicks modify again
			 * and goes through it to the done panel. It doesn't set the default button that
			 * time
			 * 
			 * This is most likely due to the address/pointer value of doneDoneButton
			 * changing once for some reason:
			 * javax.swing.JButton[,0,0,0x0,invalid,alignmentX=0.0,... to
			 * javax.swing.JButton[,316,0,77x29,invalid,alignmentX=0.0,...
			 * 
			 * This change happens in the trainNextListener at the call
			 * "preProcessWindow.revalidate()". Keep in mind, this only changes the
			 * address/pointer value of the button ONCE for some reason, even though this
	...
	/**
	 * Debugging method to see the contents of "titles" to check and make sure it's
	 * contents are being updated properly
	 */
