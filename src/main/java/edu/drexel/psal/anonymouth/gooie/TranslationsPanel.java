...
public class TranslationsPanel {
	...
	/**
	 * Displays the translations of the given sentence in the translations holder
	 * panel.
	 * 
	 * @param sentence
	...
	public void updateTranslationsPanel(TaggedSentence sentence) {
		...
		// If any of these conditions are met, we just want to clear things that need to
		...
		if (sentence.hasTranslations()) {
			...
			// for each translation, initialize a title label, and a text area that will
			// hold the translation
			// then add those two to a final panel, which will be added to the translation
			// list panel.
			for (int i = 0; i < numTranslations; i++) {
				...
				MigLayout layout = new MigLayout("wrap, ins 0", "", "");
	...
	/**
	 * Swaps the TranslationsTopPanel to the "Empty" state, meaning it's still in
	 * the translations tab but contains no components and has no size, so it's
	 * essentially invisible. This state should be used for sentences that contain
	 * all translations.<br>
	 * <br>
	 * 
	 * Please note that it's a good idea to wrap this method call in an invokeLater
	 * runnable thread as in some instances if this is not done this call fails to
	 * actually revalidate and repaint the window.
	 */
	...
	/**
	 * Swaps the TranslationsTopPanel to the "Progress" state, meaning that for the
	 * currently selected sentence translations are still being obtained and we want
	 * to show it's current progress and nothing else.<br>
	 * <br>
	 * 
	 * Please note that it's a good idea to wrap this method call in an invokeLater
	 * runnable thread as in some instances if this is not done this call fails to
	...
	public void switchToProgressPanel() {
		main.translationsTopPanel.removeAll();
		main.translationsTopPanel.setLayout(new MigLayout("wrap, ins 0, gap 0", "[grow, fill]", "[]"));
	...
	/**
	 * Swaps the TranslationsTopPanel to the "Button" state, meaning that the
	 * currently selected sentence is not translated and it's not in the process of
	 * translating. As a result, the button state offers users the "Translate
	 * Sentence" button, "Help" button, and "Reset" button (if shown)<br>
	 * <br>
	 * 
	 * Please note that it's a good idea to wrap this method call in an invokeLater
	 * runnable thread as in some instances if this is not done this call fails to
	...
	/**
	 * Resets all variables and clears panel of all translations, to be used for
	 * re-processing
	 */
	public void reset() {
...
/**
 * The Translation Swap "Button" JPanel, doesn't actually contain any JButtons,
 * it's just a JPanel with a paintComponent that paints the arrow (so we can get
 * nice animations and also so we're not relying on external, non-resizable
 * arrow images and instead using 100% Java 2D Graphics).
 * 
 * @author Marc Barrowclift
 *
 */
class SwapButtonPanel extends JPanel {
	...
	/**
	 * Constructor, initializes to default colors and prepares all the animation
	 * threads
	 * 
	 ...
	 * change the colors to the end of the "hover" animation and repaint() the
	 * panel.<br>
	 * <br>
	 * 
	 * This thread is for when translations are being added one at a time to the
	 * scroll pane. If the user has their mouse in a position such that when a new
	 * translation is added and they are now hovering over one of the swap arrows,
	 * we want to have that button's first paint color be the end of the hover
	 * animation. The best way to see what I mean is try it yourself.
	 */
	...
	/**
	 * Prepares the hoverOn SwingWorker thread that, when executed, will begin the
	 * animation loop to slowly change the panel's colors to the "hover" ones.
	 */
		...
		hoverOn = new SwingWorker<Void, Void>() {
			...
			protected Void doInBackground() throws Exception {
				...
				for (int i = 0; i < ANIMATION_FRAMES; i++) {
					...
					int r = (int) (HOVER_ARROW_COLOR.getRed() * curPercent
							+ default_Arrow_Color.getRed() * (1 - curPercent));
					int g = (int) (HOVER_ARROW_COLOR.getGreen() * curPercent
							+ default_Arrow_Color.getGreen() * (1 - curPercent));
					int b = (int) (HOVER_ARROW_COLOR.getBlue() * curPercent
							+ default_Arrow_Color.getBlue() * (1 - curPercent));
					arrowColor = new Color(r, g, b);

					r = (int) (hover_Background_Color.getRed() * curPercent
							+ DEFAULT_BACKGROUND_COLOR.getRed() * (1 - curPercent));
					g = (int) (hover_Background_Color.getGreen() * curPercent
							+ DEFAULT_BACKGROUND_COLOR.getGreen() * (1 - curPercent));
					b = (int) (hover_Background_Color.getBlue() * curPercent
							+ DEFAULT_BACKGROUND_COLOR.getBlue() * (1 - curPercent));
	...
	/**
	 * Prepares the hoverOff SwingWorker thread that, when executed, will begin the
	 * animation loop to slowly change the panel colors back to their defaults from
	 * the hover.
	 */
		...
		hoverOff = new SwingWorker<Void, Void>() {
			...
			protected Void doInBackground() throws Exception {
				...
				for (int i = 0; i < ANIMATION_FRAMES; i++) {
					...
					int r = (int) (default_Arrow_Color.getRed() * curPercent
							+ HOVER_ARROW_COLOR.getRed() * (1 - curPercent));
					int g = (int) (default_Arrow_Color.getGreen() * curPercent
							+ HOVER_ARROW_COLOR.getGreen() * (1 - curPercent));
					int b = (int) (default_Arrow_Color.getBlue() * curPercent
							+ HOVER_ARROW_COLOR.getBlue() * (1 - curPercent));
					arrowColor = new Color(r, g, b);

					r = (int) (DEFAULT_BACKGROUND_COLOR.getRed() * curPercent
							+ hover_Background_Color.getRed() * (1 - curPercent));
					g = (int) (DEFAULT_BACKGROUND_COLOR.getGreen() * curPercent
							+ hover_Background_Color.getGreen() * (1 - curPercent));
					b = (int) (DEFAULT_BACKGROUND_COLOR.getBlue() * curPercent
							+ hover_Background_Color.getBlue() * (1 - curPercent));
					backgroundColor = new Color(r, g, b);
	...
	/**
	 * Back when the translation swap button was still an actual JButton, I used the
	 * setActionCommand() method to determine what language the translation was so
	 * that we could easily get the correct translation taggedSentence from the
	 * translationsMap in TranslationsDriver. I'm basically just replicating it
	 * here.
	 * 
	 * @param command
	 *            The name of the language represented by this SwapButtonPanel
	 *            instance (Japanese, French, etc.)
	 */
	 ...
	 * 
	 * @return The name of the language represented by this SwapButtonPanel instance
	 *         (Japanese, French, etc.)
	 */
	...
	/**
	 * Determines whether or not the panel is ready to begin a new animation thread
	 * (we don't want two going at the same time)
	 * 
	 * @return True or false, depending on whether or not there are any animation
	 *         threads going on.
	 */
	...
	/**
	 * Updates the vertical arrow dimensions with the passed height so that the
	 * arrow can be perfectly in the middle no matter how large or small the
	 * component's height is. Also saved a local copy of the height for future
	 * reference in the paintComponent() method.<br>
	 * <br>
	 * 
	 * MUST only be called in the paintComponent() method, because anywhere else
	 * Swing is dumb as shit and doesn't actually know the component's dimensions
	 * yet. The only place I've found where we can reliably get the final height of
	 * the component is in it's paintComponent method().
	 * 
	 * @param height
	...
	/**
	 * Calculate's the arrow's base color and the background's hover color based on
	 * the translation's ranking with respect to other translations anonymity index
	 * (the higher it is the greener it is, meaning it's the "better choice" than
	 * the lower red ones).
	 * 
	 * @param position
