 ...
 *
 * <ul>
 * <li>Translate sentence button
 * <li>Reset translator button (if shown)
 * </ul>
 *
 * This driver, unlike most others in Anonymouth, also acts as a mouse listener
 * itself (so it can be added to every translation swap button instance easily),
 * so in order to truly initialize all listeners for this driver it must be
 * initilazed and ALSO added to each translation swap button instance.
 *
...
public class TranslationsDriver implements MouseListener {
	...
	/**
	 * Constructor, automatically initializes all listeners associated with
	...
	public void initListeners() {
		translateSentenceListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				/**
				 * We're forcing the translations progress panel here instead of letting the the
				 * TranslationsPanel eventually change it since this sometimes takes a second or
				 * two but we want the progress bar to replace the button near immediately so
				 * the user knows something's happening (and also to prevent them from clicking
				 * on it numerous times because they don't think it did anything)
				 */
		...
		resetTranslatorListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				int answer = JOptionPane.showOptionDialog(null,
						...
						"Reset Translator", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE,
						UIManager.getIcon("OptionPane.warningIcon"), null, null);
				if (answer == JOptionPane.YES_OPTION) {
					main.translationsHolderPanel.removeAll();
					main.notTranslated.setText(
							"Sentence has not been translated yet, please wait or work on already translated sentences.");
		...
		helpTranslationListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				JOptionPane.showMessageDialog(main,
						"<html>" + "Sometimes it's difficult to think of different ways to phrase<br>"
								...
								+ "arrow to swap it in automatically)" + "</html>",
						"Translations Help", JOptionPane.INFORMATION_MESSAGE, ThePresident.dialogIcon);
			}
		};
	...
	/**
	 * Acts as the "ActionListener" to the translation swap arrow by immediately
	 * swapping the sentence in the working document with the relevant translation,
	 * clearing all translations from the translations holder scroll pane (since
	 * they were for the old sentence not the new one), and switching the
	 * translations top panel back to the default button panel with the "Translate
	 * Sentence", "?", and "Reset" button if shown.
	 *
	 * <p>
	 * NOTE: In it's current state this is admittedly a bit of a mess. I'm about to
	 * leave PSAL in a bit so I'm sadly not able to polish this up as much as I'd
	 * like to, so please anywhere you find code find of shit like this please feel
	 * free to organize and rewrite in a way that's a bit more sane. -Marc
	 */
	@Override
	public void mouseReleased(MouseEvent e) {
		...
		// We don't want to display translations for this sentence anymore since the
		// sentence is
		// changing
		SwingUtilities.invokeLater(new Runnable() {
			@Override
		...
		int lengthShift = newSentence.length()
				- main.editorDriver.taggedDoc.getSentenceNumber(main.editorDriver.sentNum).getLength();
		...
		/**
		 * Everything in this chunk here is a near copy and past of the code for
		 * inserting more than two characters of text in EditorDriver's insertion()
		 * method (which is part of the reason this is ugly as sin)
		 *
		 * <p>
		 * This only concerns itself with tracking any special characters in the
		 * sentence text being swapped in
		 */
		main.editorDriver.taggedDoc.specialCharTracker.removeEOSesInRange(main.editorDriver.sentIndices[0],
				main.editorDriver.sentIndices[1]);
		main.editorDriver.taggedDoc.specialCharTracker.removeTextWrappersInRange(main.editorDriver.sentIndices[0],
				main.editorDriver.sentIndices[1]);
		...
		for (int i = 0; i < sentChars.length; i++) {
			if (main.editorDriver.taggedDoc.watchForEOS != -1) {
				if (main.editorDriver.isWhiteSpace(sentChars[i])) {
					main.editorDriver.taggedDoc.specialCharTracker.setIgnoreEOS(main.editorDriver.taggedDoc.watchForEOS,
							false);
				}

				main.editorDriver.taggedDoc.watchForEOS = -1;
			} else if (main.editorDriver.taggedDoc.watchForLastSentenceEOS != -1
					&& main.editorDriver.newCaretPosition[0] == main.editorDriver.taggedDoc.length) {
				if (main.editorDriver.isWhiteSpace(sentChars[i])) {
					main.editorDriver.taggedDoc.endSentenceExists = false;
					main.editorDriver.taggedDoc.specialCharTracker
							.setIgnoreEOS(main.editorDriver.taggedDoc.watchForLastSentenceEOS, false);
				}

				main.editorDriver.taggedDoc.watchForLastSentenceEOS = -1;
			} else {
				if (main.editorDriver.taggedDoc.specialCharTracker.isEOS(sentChars[i])) {
					if (main.editorDriver.taggedDoc.length == main.editorDriver.priorCaretPosition
							+ main.editorDriver.charsInserted) {
						...
						main.editorDriver.taggedDoc.specialCharTracker.addEOS(sentChars[i],
								main.editorDriver.taggedDoc.watchForLastSentenceEOS, false);
					} else {
						main.editorDriver.taggedDoc.watchForEOS = i + buffer;
						main.editorDriver.taggedDoc.specialCharTracker.addEOS(sentChars[i],
								main.editorDriver.taggedDoc.watchForEOS, false);
					}
				} else {
		...
		if (lastSentence != -1) {
			main.editorDriver.taggedDoc.makeNewEndSentence(
					main.documentPane.getText().substring(lastSentence, main.editorDriver.taggedDoc.length - 1));
		}

		main.editorDriver.taggedDoc.specialCharTracker.shiftAll(sentChars.length + buffer, lengthShift);

		/**
		 * At last, we will sync all changes we have made to the tagged document to the
		 * main JTextPane so the user sees the changes made. There's absolutely more
		 * effecient ways to do this.
		 */
	...
	/**
	 * For when the mouse hovers over a particular translation's swap arrow, we want
	 * to immediately being the hoverOn Swing Worker thread to animate a nice
	 * "hover" animation
	 */
	@Override
	...
	/**
	 * For when the mouse exits hovering over a particular translation's swap arrow,
	 * we want to immediately begin the hoverOff Swing Worker thread to animate a
	 * nice "hover" turn off animation
	 */
	@Override
