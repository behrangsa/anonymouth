...
public class PreProcessAdvancedDriver {
	 ...
	 *            - PreProcessAdvancedWindow instance, so we can access the swing
	 *            components
	 * @param main
	 *            - GUIMain instance
	 */
	...
	private void initFeatureListeners() {
		...
		featureViewInfoListener = new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				Logger.logln(NAME + "View Info button clicked for feature "
						+ (String) advancedWindow.featureChoice.getSelectedItem());
				advancedWindow.viewInfoFrame.setTitle(
						"Viewing " + (String) advancedWindow.featureChoice.getSelectedItem() + " Information");
		...
		viewInfoListListener = new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent arg0) {
				...
				if (selected == lastSelected)
					return;
				Logger.logln(NAME + "Feature selected in the view info window: "
						+ advancedWindow.viewInfoList.getSelectedValue());
		...
		viewInfoCanonTableListener = new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (selected == -1) {
					...
					Canonicizer c = cfd.featureDriverAt(advancedWindow.viewInfoList.getSelectedIndex())
		...
		viewInfoCullTableListener = new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				if (selected == -1) {
					...
					EventCuller ec = cfd.featureDriverAt(advancedWindow.viewInfoList.getSelectedIndex())
							.cullerAt(selected);
	...
	private void initClassifierListeners() {
		...
		classEditListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				advancedWindow.argsFrame
						.setTitle("Edit " + (String) advancedWindow.classChoice.getSelectedItem() + "'s Arguments");
	...
	private void initArgListeners() {
		...
		argsOKListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					Logger.logln(NAME
							+ "Problem occured while trying to set user defined arguments, shouldn't have got here.",
							LogOut.STDERR);
				}
		...
		argsCancelListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				advancedWindow.argsField.setText(
						advancedWindow.getOptionsStr(((OptionHandler) advancedWindow.classifiers.get(0)).getOptions()));
				advancedWindow.argsFrame.setVisible(false);
			}
		};
	...
	/**
	 * Either enables or disables the ArgsOKButton based on whether or not the
	 * arguments from the argument text field are acceptable in Weka or not.
	 *
	 * <p>
	 * NOTE: This does not catch arguments that don't make sense or may break Weka,
	 * we are simply filtering out arguments that Weka won't even let us set. If
	 * there is a "deeper" way to ensure that the arguments are valid while still
	 * retaining good performance feel free to do so.
	 */
	...
	/**
	 * Updates the "View ${FEATURE_SET} Info" window's text panes and tables with
	 * data corresponding to the currently selected feature set
	 */
	...
	/**
	 * Populates the given tableModel with parameters and their values for the given
	 * event driver / canonicizer / culler. Assumes the table is set to have three
	 * columns.
	 */
