...
/**
 * The starting window of Anonymouth that allows the user to load, modify, or
 * start a new document set, Make changes in advanced configuration, or start
 * Anonymouth.<br>
 * <br>
 * This is created in an effort to help guide the user along and not throw too
 * much at them at the beginning.
 *
...
public class StartWindow extends JFrame {
	...
	private void initGUI() {
		 ...
		 * bottomPanel.add(quitButton); bottomPanel.add(loadDocSetButton);
	...
	private void initLookAndFeel() {
		...
		final String[] themeClassesToTry = {"com.sun.java.swing.plaf.windows.WindowsLookAndFeel",
		...
		for (String themeClassName : themeClassesToTry) {
			try {
				...
				// If setting the GTK look and feel fails, then default to the system look and
				// feel
				try {
	...
	private void initListeners() {
		startListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				/**
				 * makes sure to internally rename files by different authors that have the same
				 * name. It Used to be handed upon adding, but better for the user if they never
		...
		loadDocSetListener = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				try {
					 ...
					 * FileHelper.load.setCurrentDirectory(new
					 * File(JSANConstants.JSAN_PROBLEMSETS_PREFIX));
					 ...
					 * FileHelper.load.showOpenDialog(startingWindows);
					 * 
	...
	/**
	 * If there was a recently used/saved document set in the properties file,
	 * Anonymouth will load up and initialize itself around that problem set so
	 * Anonymouth's ready to immediately start if the user wants.
	 */
	protected void setLastDocumentSet() {
		try {
			...
			Logger.logln(NAME + "Problem loading last used document set: " + PropertiesUtil.getProbSet(),
					LogOut.STDERR);
	...
	/**
	 * Determines whether or not the user has an acceptable document set built and
	...
	/**
	 * Loads the problem set from the given path, attempts to load everything, then
	 * checks to make sure everything's ready. Updates Components accordingly
	 *
	 * @param path
	 *            - The absolute path to the problem set we want to load
	 */
	public void loadProblemSet(String path) {
		...
		try {
			...
			if (main.preProcessDriver.updateAllComponents() && probSetReady) {
				...
				main.preProcessDriver.updateOpeningDir(main.preProcessWindow.ps.getAllTrainDocs().get(0).getFilePath(),
						true);
			} else {
				...
				if (probSetReady) {
					JOptionPane.showMessageDialog(startingWindows,
							...
							"Problems with Loading Document Set", JOptionPane.WARNING_MESSAGE, ThePresident.dialogLogo);
					setReadyToStart(true, true);
					ThePresident.canDoQuickStart = true;
				} else {
					JOptionPane.showMessageDialog(startingWindows,
...
/**
 * Simple Frame to ask the user for their name so we can name the log files.
 * This is intended for use in user case studies, and (at least from last
 * discussion) should not be shown or used in a release version of Anonymouth.
 * Whether or not to display this can be easily flipped via the constant boolean
 * "IS_USER_STUDY" in ANONConstants.
 *
 * @author Marc Barrowclift
 */
class UserStudySessionName extends JFrame {
	 ...
	 * Constructor
	 *
	 * @param startingWindows
	 *            - StartingWindows instance
	 */
	...
	/**
	 * Initialzes all listeners used by the frame and adds them to their respective
	 * components
	 */
	private void initListeners() {
