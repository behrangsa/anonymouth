 ...
 * 
 * 1. Since Anonymouth on first load takes a while to load, we want to show some
 * indication that it is doing so (like Photoshop) 2. Help give Anonymouth a
 * professional sheen.
 * 
...
public class SplashScreen extends JFrame {
	...
	public SplashScreen() {
		...
		/**
		 * Undecorated and setLocationRelativeTo(null) don't actual set it to the middle
		 * since setLocationRelativeTo(null) still counts the menu bar of the
		 * undecorated from when making it's calculation even if it's not shown since
		 * it's undecorated. Thus, we calculate it ourselves to get a true center of the
		 * screen.
		 */
	...
	/**
	 * Initializes the fade in Swing Worker for the splash screen. Should only be
	 * called once by the constructor since the splash screen need only be shown
	 * once (meaning only one fade in).
	 */
	 ...
	 * 
	 * @param newText
	 *            The new message you want to display. A "..." will be appended to
	 *            it before updating.
	 */
	 ...
	 * application from continuing to load.<br>
	 * <br>
	 * 
	 * But here, we don't want the start window to appear right over the splash
	 * screen before it can have a chance to fade out nicely, so that's why we
	 * quickly do so in the EDT before it displays the start window.
	 */
