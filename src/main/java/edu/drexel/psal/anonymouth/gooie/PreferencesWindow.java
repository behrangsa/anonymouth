...
/**
 * The main preferences window for Anonymouth. Stores under a different pull
 * down menu depending on the system (if Mac, it's put under Anonymouth >
 * Preferences to keep with the Mac L&F, and if Windows/Linux it's put under
...
public class PreferencesWindow extends JFrame implements WindowListener {
	...
	/**
	 * Initializes all of the panels on the tree. Must be called before the tree is
	 * created, because when the tree is initialized it selects the first leaf in
	 * the tree which causes the panel to be shown.
	 */
	private void initTabs() {
		...
		general.setLayout(generalLayout);
		{
			...
			autoSaveNote = new JLabel(
					"<html><center>" + "Note: Will only save over previously saved Anonymized document,<br>"
							+ "if one exists. Will not overwrite original document." + "</center></html>");
			...
			filterAddWords.setToolTipText("<html><center>" + "Filtered out words include but are not limited to:<br>"
					+ "	-Email addresses<br>" + "	-Dates and other numbers<br>" + "	-Web addresses"
					+ "</center></html>");
			...
			translationsNote = new JLabel(
					"<html><link rel=\"stylesheet\" type=\"text/css\" href=\"mystyles.css\" media=\"screen\" /><center>"
		...
		advanced.setLayout(advancedLayout);
		{
			...
			versionAutoSaveNote = new JLabel(
					"<html>Note: Backups saved to hidden directory \"./edited_documents\" in the<br>Anonymouth root directory</html>");
	...
	/**
	 * We want to be absolutely sure that when Preferences is closed that the set
	 * values are within acceptable boundaries, and if they aren't change them back
	 * to their editor.
	 */
	@Override
