...
/**
 * Used to display the state of the application when the "Process" button is
 * pressed. NOTE: Currently a structure to accurately track the progress of the
 * process is NOT in place. Initially, we solved this by having the bar be
 * indeterminable, but due to a bug in the JRE 7, the indeterminable progress
 * bar does not update on OS X. As such, Andrew created a quick hack to have the
 * bar cycle to being full then repeat until the process is done.
 * 
				...
				SwingUtilities.invokeLater(new Runnable() {
					...
					public void run() {
						int confirm = JOptionPane.showOptionDialog(GUIMain.inst,
								...
								"Document Still Processing!", JOptionPane.YES_NO_OPTION, JOptionPane.WARNING_MESSAGE,
								UIManager.getIcon("OptionPane.warningIcon"), ACTIONS, ACTIONS[1]);
