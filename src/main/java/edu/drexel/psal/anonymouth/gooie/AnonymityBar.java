...
/**
 * The template for the new and improved Anonymity bar!<br>
 * <br>
 ...
 * If you wish to make any changes to the Anonymouth Anonymity Bar, it would be
 * easier and quicker to test them out in the standalone AnonymityBar.java file
 * located in:<br>
...
public class AnonymityBar extends JPanel {
	...
	/**
	 * The gap between the bottom of the drawing panel and the bottom of the bulb.
	 * Must be at least 20, otherwise, the bulb starts getting drawn off the bottom
	...
	private final int TUBE_VERTICAL_GAP = 13; // the gap you want the tub to have from the top and bottom of the drawing
												// panel
	...
	private int bulbBarGap; // The decrease in width to be done on the tub with respect to the bar bulb
							// width
	 ...
	 * height and width. In Anonymouth, the change we will most likely see is a
	 * change in height, but we want to update the width to make sure.<br>
	 * <br>
	 * Also automatically calls updateDrawingVariables() for you, because as the
	 * name suggests it updates all variables.
	 */
	public void updateForNewSize() {
		/**
		 * Only values we actually have to manually change, the rest use these updated
		 * values to update their respective values
		 */
	...
	/**
	 * Updates all the drawing variables dynamically based on what the width and
	 * height of the frame (and thus the panel) currently are. For Anonymouth, we
	 * will not use the frame width and heights as the reference but rather the
	 * drawing panel width and height.
	 */
	private void updateDrawingVariables() {
		...
		/**
		 * We want to make sure that the bar tube width is always a nice ratio smaller
		 * than the bar bulb width. This is so we actually have it look a bit more like
		 * a thermometer. We want this to dynamically change based on what the new
		 * barBulbWidth value is (so that way the tube width is guaranteed to be the
		 * same nice ratio smaller than the bar bulb width).
		 */
		...
		// =========================================================
		/**
		 * These do not need to dynamically change using the panel width and height
		 * since it's already taken care of above and these variables are updated with
		 * respect to them (with the exception of barTubeHeight).
		 */
		...
		/**
		 * Times 2 to the TUBE_VERTICAL_GAP because of top AND bottom for the gap and
		 * Times 2 to the BAR_STROKE To accommodate for the additional stroke size on
		 * the top AND bottom (so it's not drawn off the panel)<br>
		 * <br>
	...
	/**
	 * Updates the fill gauge of the tube. This includes updating all variables
	 * relating to the fill, updating the color automatically for you depending on
	 * the fill amount, and also painting the changes.
	 */
	private void updateTubeFill() {
		bottomY = (int) (((barBulbHeight / 5) + 0.5) + ((barBulbWidth / barBulbHeight)) + 0.5) + barBulbY
				- ((int) (BAR_STROKE + 0.5));
		topY = barTubeY;
		curPercent = newValue / maxFill; // In Anonymouth, this is where you get an calculate the percent (range
		// 0-100%, you have to adjust it)
		...
		// Just in case the Anonymity bar is hidden, we want to also update the JLabel
		// with the percent
		// string
	...
	/**
	 * Just updates the color, shouldn't be called by the programmer any additional
	 * places.
	 */
	...
	/**
	 * This is the method that should be called by any outside source when they want
	 * the bar to be updated. Anything else (not including window resizing and it's
	 * call to updateForNewSize()) is strictly for internal purposes.
	 */
	...
	public void paintComponent(Graphics g) {
		...
		// ========================================================================
		/**
		 * Draws the background of the bar. The reason we're doing it this way instead
		 * of just painting the whole bloody thing is this way we can have the
		 * surrounding color of the bar be transparent while only the background color
		 * of the bar itself is painted.
		 *
		 * <p>
		 * Also must be done to g2d and NOT gbi, because otherwise it would get
		 * overridden.
		 */
		g2d.setColor(BACKGROUND_COLOR);
		g2d.fillRoundRect(barTubeX, barTubeY, barTubeWidth, barTubeHeight - BULB_BOTTOM_GAP, barTubeWidth,
				barTubeWidth);
		...
		// ========================================================================
		/*
		 * Creates the buffered image. We need to do this in order to clean up parts of
		 * the bar (to make it seem like one solid shape). You must draw to gbi.
		 */
		BufferedImage buffImg = new BufferedImage(main.anonymityWidth, main.anonymityHeight,
		...
		// Drawing the tube
		gbi.setColor(BORDER_COLOR);
		gbi.drawRoundRect(barTubeX, barTubeY, barTubeWidth, barTubeHeight - BULB_BOTTOM_GAP, barTubeWidth,
				barTubeWidth);
		...
		// Clearing the top arc part of the bulb that's inside the tube (to get a solid
		// outline shape
		// with nothing inside)
		gbi.setComposite(AlphaComposite.Clear);
		gbi.fillRect(barTubeX + ((int) ((BAR_STROKE / 2) + 0.5)), barBulbY - ((int) (BAR_STROKE + 0.5)),
				barTubeWidth - ((int) (BAR_STROKE + 0.5)), (int) (((barBulbHeight / 5) + 0.5) + 3));

		// ========================================================================
		// ****************** BAR FILL (IF ANY) **********************
		// ========================================================================
		if (showFill) {
			gbi.setComposite(AlphaComposite.getInstance(AlphaComposite.DST_OVER, 1.0f)); // Paint just inside existing
																							// border
			gbi.setColor(color);
			/**
			 * The division for the last two lines allow the fill top to slowly round to fit
			 * the top of the rounded tube as it ascends
			 */
			gbi.fillRoundRect(barTubeX, bottomY - curHeight, barTubeWidth, curHeight + (barTubeWidth / 2),
	...
	/**
	 * Sets the maximum value the bar's fill can achieve. NOTE: THIS MUST ONLY BE
	 * CALLED ONCE IN BACKEND INTERFACE.
	 *
	 * @param maxPercent
	...
	/**
	 * Resets the bar to it's default fill value and color, should be called for
	 * reprocessing
	 */
	protected void reset() {
		...
		updateTubeFill(); // Since the fill changed, we want to unsure all variables are updated before
		// painting
	}
}
