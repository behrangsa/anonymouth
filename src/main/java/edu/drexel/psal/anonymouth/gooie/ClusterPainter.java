...
public class ClusterPainter extends JPanel {
	...
	public ClusterPainter(Cluster[] clusters, int featureNumber, double minimum, double maximum, double authorMin,
			double authorMax, double presentValue) {
	...
	protected void paintComponent(Graphics g) {
		...
		if (maximum < 1)
			maximum = 1.0; // prevent divide by zero, or creating too large a scale by dividing by a
							// decimal....
		...
		double[][] thisClustersMinsAndMaxes = new double[numClusters][2]; // [min,max] (index == cluster number) => used
																			// to update cluster colors
		...
		for (i = 0; i < numClusters; i++) {
			...
			/*
			 * g2.setPaint(transBlue); int j=0; Pair[] pRay = current.getElements(); int
			 * lenPRay = pRay.length; for(j=0; j < lenPRay; j++){ double position =
			 * transform(pRay[j].value,false); g2.fill(new
			 * Ellipse2D.Double(position-Math.sqrt(dim), yoffset-Math.sqrt(dim),dim,dim)); }
			 * 
			 * g2.setColor(Color.black); dim = 4; g2.fill(new
			 * Ellipse2D.Double(centroid-Math.sqrt(dim), yoffset-Math.sqrt(dim),dim,dim));
			 */

			int selectedCluster = ClustersDriver.selectedClustersByFeature[featureNumber];

			/**
			 * this needs to be offset by 1, because a '1' was added to the cluster numbers
			 * to avoid a cluster being number '0'.
			 */
			...
			// g2.draw(new Ellipse2D.Double(minValue,yoffset*.75,maxValue -
			// minValue,yoffset*.5));
			// }
