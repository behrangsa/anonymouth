 ...
 * isn't actually editing text but rather editing the TaggedDocument, which has
 * TaggedSentences and their TaggedWords)
 *
...
public class EditorDriver {
	...
	/**
	 * Whether or not to automatically highlight in red all words to remove in the
	 * current sentence
	 */
	 ...
	 * updating the suggestions that auto highlights uses) when the call is made to
	 * calculate the indices for the automatic highlights. When this is the case, we
	 * switch this to true and the updateSuggestionsThread will then do auto
	 * highlights upon completion.
	 */
	private boolean placeAutoHighlightsWhenDone;
	/**
	 * Whether or not any text within the editor has changed. This is used because
	 * we don't want to call updateSentence and go through all that hullabaloo in
	 * the backend if nothing actually changed. Thus, we have this boolean to make
	 * sure we only do that when necessary.
	 */
	...
	// ----------- Caret Variables -------------------------------------------
	/**
	 * An array of 2 values that holds what will be the new position of the caret
	 * AND the end position of the selection (if there is one), and another array
	 * that holds the what the previous "newCaretPosition"s were.<br>
	 * <br>
	 * This is NOT the sentence indices, which is the start and end of a given
	 * sentence. Most of the time, since the user is just typing, the first and
	 * second indices of these arrays will be equal.
	 */
	 ...
	 * position PRIOR to having any characters inserted or removed. This means that
	 * if the user's just moving the cursor around and not typing anything it will
	 * be equal to newCaretPosition[0] since charsRemoved and charsInserted are 0.
	 * However, if the user has typed or pasted text this will reflect the length of
	 * those changes with respect to newCaretPosition[0]
	 */
	 ...
	 * [2] = The number of extra sentences included in the extended highlight to the
	 * LEFT<br>
	 * [3] = The number of extra sentences included in the extended highlight to the
	 * RIGHT<br>
	 * <br>
	 * It's sole purpose is to let us extend the sentence highlights beyond what
	 * sentIndices so that sentences that should be considered one whole sentence
	 * are combined to one full highlight.
	 */
	...
	// --------------- Editing variables -------------------------------------
	/**
	 * The number of characters inserted into the document this caret event (default
	 * is 0 is none were added)
	 */
	protected int charsInserted;
	/**
	 * The number of characters removed from the document this caret event (default
	 * is 0 if none were removed)
	 */
	protected int charsRemoved;

	/**
	 * Whether or not to process changes made to the JTextPane. Exists because
	 * sometimes we remove all text from the pane and replace it with new text (like
	 * in syncTextPaneWithTaggedDoc() and VersionControl's undo and redo) but we
	 * don't want the changes to actually be processed since it would break things.
	 */
	...
	private void initListeners() {
		/**
		 * The main document event listener, this fires whenever the user is typing or
		 * moving around in the document.
		 */
		caretListener = new CaretListener() {
			@Override
			public void caretUpdate(CaretEvent e) {
				// Don't go any further if we're supposed to ignore the event, it would break
				// things
				if (ignoreChanges) {
					return;
				}
				...
				// ------------------------- Undo/Redo updates -------------------------
				// If the user has pasted or deleted/cut a chunk of text, always immediately
				// backup past
				// version
				...
				/**
				 * If the user had previous typed an EOS character and we were unsure whether or
				 * not it was an actual EOS and not an abbreviation, ellipses, etc. AND they
				 * have navigated away from said EOS character (clicking somewhere else in the
				 * document to edit another sentence), we will assume that what was the end of
				 * the sentence and update it as such.
				 */
				if (taggedDoc.watchForEOS != -1 && charsInserted == 0 && charsRemoved == 0) {
					if (taggedDoc.specialCharTracker.isSentenceEndAtIndex(taggedDoc.watchForEOS + 1)) {
						...
						updateSentence(pastSentNum,
								main.documentPane.getText().substring(sentIndices[0], sentIndices[1]));
						taggedDoc.watchForEOS = -1;
					}
				}
				...
				if (charsRemoved > 0) {
					...
					// If we are unsure about an EOS character at the end of the doc, keep track of
					// it.
					...
					/**
					 * Must be prior to insertion() call (unlike deletion above) since, if the user
					 * is adding an eos character right before an existing one, it would then add
					 * them both to the same index, not good. This way, they get added the correct
					 * way and nothing get's screwed up.
					 */
				...
				/** Only update the sentence in the backend if it actually changed (duh) */
				if (textChanged) {
					/**
					 * We need to do a little cleaning up with the end sentence index sometimes with
					 * respect to the last sentence since we don't want a string index out of bounds
					 * exception for highlighting and other things going wrong.
					 */
				...
				/**
				 * If we're selecting a different sentence now, we must update the translations
				 * panel to display its translations, a blank window with the translate button,
				 * or the progress bar with the currently number of translations (all figured
				 * out within updateTranslationsPanel())
				 */
		...
		/**
		 * Needed so we can catch and updated the insert or remove variables for the
		 * caret event updates. This gets fired BEFORE the caret listener.
		 */
	...
	/**
	 * Prepares all threads used by the editor or related to it with their
	 * respective tasks
	 */
	...
	/**
	 * Prepares the updateSuggestionsThread SwingWorker for running. This MUST be
	 * called before every new execution since by design a single SwingWorker
	 * instance cannot be executed more than once.
	 */
			...
			public void done() {
				if (placeAutoHighlightsWhenDone) {
					...
					if (highlightIndices[0] + whiteSpace <= newCaretPosition[0]) {
						highlighterEngine.addAutoRemoveHighlights(highlightIndices[0] + whiteSpace, highlightIndices[1],
								highlightIndices[2], highlightIndices[3]);
					}
				}
			}
		};
	...
	/**
	 * Prepares the updateBarThread SwingWorker thread for running. This MUST be
	 * called before every new execution since by design a single SwingWorker
	 * instance cannot be executed more than once
	 */
	...
	/**
	 * Handles all the Deletion bookkeeping by managing whether or not it was an EOS
	 * character being deleted. If it was, it takes care of combining the sentences
	 * into a new one for you and deleting any sentences in between.
	 *
	 * <p>
	 * This was basically all Andrew McDonald's work, this remained virtually
	 * unchanged during the rewrite because it works so damn well (the only times it
	 * breaks is when something ELSE has). So big shoutout to him for making this a
	 * whole lot easier.
	 */
	private void deletion() {
		...
		if (taggedDoc.specialCharTracker.removeEOSesInRange(newCaretPosition[0], priorCaretPosition)) {
			/**
			 * We're going to be handling all the TaggedDocument manipulation HERE instead
			 * of in the standard updateSentence() method call at the end of the listener
			 * since we have a lot more work to do than a standard change. Therefore, since
			 * we are already updating the backend here, we should NOT update it an
			 * additional time, otherwise it screws everything up.
			 */
			textChanged = false;

			/**
			 * If the user is editing after the last tagged sentence and decides to delete
			 * their EOS character before it was set as a sentence end we want to reset the
			 * watch variable (otherwise we'd keep watching an EOS character that no longer
			 * exists)
			 */
			...
			try {
				...
				if (rightSentInfo[0] != leftSentInfo[0]) {
					/**
					 * Add '1' because we don't want to count the lower bound (e.g. if midway
					 * through sentence '6' down to midway through sentence '3' was deleted, we want
					 * to delete "6 - (3+1) = 2" TaggedSentences.
					 */
					int sentsBetweenToDelete = rightSentInfo[0] - (leftSentInfo[0] + 1);
					int[] taggedSentsToDelete;

					/**
					 * Now we obtain the indices of sentences that need to be removed, which are the
					 * ones between the left and right sentence (though not including either the
					 * left or the right sentence).
					 */
					int j = 0;
					if (wholeLastSentDeleted) {
						/**
						 * We want to ignore the rightmost sentence from our deletion process since we
						 * didn't actually delete anything from it.
						 */
						...
						/**
						 * We want to include the rightmost sentence in our deletion process since we
						 * are partially deleting some of it.
						 */
					...
					/**
					 * First delete every sentence the user has deleted wholly (there's no extra
					 * concatenation or tricks, just delete).
					 */
					taggedDoc.removeTaggedSentences(taggedSentsToDelete);
					Logger.logln(
							NAME + "Ending whole sentence deletion, now handling left and right deletion (if needed)");

					/**
					 * Then read the remaining strings from "left" and "right" sentence:
					 *
					 * <p>
					 * for left: read from 'leftSentInfo[1]' (the beginning of the sentence) to
					 * 'newCaretPosition' (where the "sentence" now ends)
					 *
					 * <p>
					 * for right: read from 'pastCaretPotion' (where the "sentence" now begins) to
					 * 'rightSentInfo[2]' (the end of the sentence)
					 *
					 * <p>
					 * Once we have the string, we call removeAndReplace, once for each sentence
					 * (String)
					 */
					 ...
					 *
					 * <p>
					 * TODO integrate this better within EditorDriver so it's cleaner, right now
					 * this is pretty sloppy and confusing
					 */
					...
					String rightSentCurrent = docText.substring((priorCaretPosition - charsRemoved),
							(rightSentInfo[2] - charsRemoved));
					...
					if (wholeLastSentDeleted && wholeBeginningSentDeleted) {
						...
						taggedDoc.concatRemoveAndReplace(taggedDoc.getTaggedDocument().get(leftSentInfo[0]),
								leftSentInfo[0], taggedDoc.getTaggedDocument().get(leftSentInfo[0] + 1),
								leftSentInfo[0] + 1);
					} else {
						try {
					...
					/**
					 * If the user is deleting a chunk of text at the end of a document and stops
					 * just at an existing EOS character, editing at the end of a document will no
					 * longer split up sentences properly. That's what this check is for, if they
					 * delete and stop before an EOS character at the end of a sentence then we will
					 * make THAT the new EOS character we are watching for with
					 * watchForLastSentenceEOS
					 */
					if (sentNum == taggedDoc.numOfSentences - 1) {
						if (taggedDoc.specialCharTracker
								.isEOS(main.documentPane.getText().charAt(taggedDoc.length - 1))) {
				...
				if (rightSentInfo != null) {
					...
					try {
						Logger.logln(NAME + "\""
								+ main.documentPane.getText().substring(rightSentInfo[0], rightSentInfo[1]) + "\"");
					} catch (Exception e1) {
					}
				} else {
	 ...
	 * adding an EOS character. If they are, it makes the appropriate changes such
	 * that the single sentence is then split in two in the rest of the main
	 * document listener.
	 */
	private void insertion() {
		...
		/**
		 * If the user is pasting text in (meaning the charsInserted is greater than a
		 * single character) we will want to parse through the entire text and handle
		 * each EOS character we find like we would had the user typed it all in
		...
		if (charsInserted > 1) {
			if (taggedDoc.watchForEOS != -1) {
				...
				if (lastSentence != -1) {
					taggedDoc.makeNewEndSentence(
							main.documentPane.getText().substring(lastSentence, taggedDoc.length - 1));
				}

				updateSuggestions();
			}
			/**
			 * Otherwise the user's just entering a single character, in which case we just
			 * analyze the single character.
			 */
		} else {
			...
			if (taggedDoc.watchForEOS != -1) {
				...
				taggedDoc.watchForEOS = -1;
				// If we're keeping an eye out for a possible end of sentence at the VERY END of
				// the
				// document
			} else if (taggedDoc.watchForLastSentenceEOS != -1 && newCaretPosition[0] == taggedDoc.length) {
				if (isWhiteSpace(newChar)) {
				...
				taggedDoc.watchForLastSentenceEOS = -1;
				// If we're not currently watching for any end of sentence, then check the new
				// character
			} else {
				if (taggedDoc.specialCharTracker.isEOS(newChar)) {
					if (sentNum == taggedDoc.numOfSentences - 1) {
	...
	/**
	 * Checks to see if the passed character is considered "special" and tracks it
	 * with the appropriate object if it is.<br>
	 * <br>
	 * This does NOT check EOS characters since in both instances where we check for
	 * it above we run different code on the result, so I can't have the general
	 * case here. I consider this to be pretty poor oragnization as this should
	 * probably be in SpecialCharacterTracker instead
	 *
	 * @param newChar
	...
	/**
	 * Determines whether or not the character is considered whitespace (meaning it
	 * checks if it's a space, newline, or tab)
	 *
	 * @param unknownChar
	 *            The character you want to test
	 * @return Whether or not the character is white space
	 */
	...
	/**
	 * Calculates the indices of all passed "positions". This can ether be a since
	 * int value (meaning it's just a single caret position in the document and
	 * we're finding out it's indices), or it can be multiple int values (meaning
	 * there's a selection and we're finding out it's indices)
	 */
	protected int[][] getSentencesIndices(int... positions) {
		...
		for (int positionNumber = 0; positionNumber < numPositions; positionNumber++) {
			...
			/**
			 * allSentIndices[0]: Length of sentence '0' allSentIndices[1]: Length of
			 * sentence '0' plus '1' etc...
			 */
			...
			/**
			 * The start and end indices of the new sentence. This could be as simple as the
			 * previous sentence minus a character or as drastic as two or more previous
			 * sentences deleted between two new combined sentences
			 */
			int startIndex = 0;
			int endIndex = 0;
			...
			try {
				if (selectedSentence >= numSents)
					...
					if (!taggedDoc.endSentenceExists && taggedDoc.watchForLastSentenceEOS == -1) {
						if (charsInserted > 0) {
							taggedDoc.makeNewEndSentence(
									main.documentPane.getText().substring(priorCaretPosition, newCaretPosition[0]));
						} else {
							taggedDoc.makeNewEndSentence("");
						}
				...
				highlightIndices = taggedDoc.specialCharTracker.extendHighlights(startIndex, endIndex, allSentIndices,
						sentenceLengths, selectedSentence);
		...
		/**
		 * We need to check if the user deleted a whole sentence at BOTH the end and
		 * beginning of their selection. This changes how we handle deleting the
		 * TaggedSentences.
		 */
		if (results.length > 1) {
	...
	/**
	 * Calculates how much extra space should be added to the given index before it
	 * hits the first actual character of the text (meaning we don't want to "trim"
	 * the whitespace out of the highlight)
	 *
	 * @param index
	 *            The index you want to "pad"
	 * @return The number of spaces you should add to the index to get the proper
	 *         start of the sentence
	 */
	public int getWhiteSpaceBuffer(int index) {
		/**
		 * In try catch to prevent it breaking when the user it typing after the last
		 * sentence
		 */
		int whiteSpace = 0;
		try {
	...
	/**
	 * Updates the sentence highlight indices with the new indices in sentIndices.
	 */
	public void moveHighlights() {
		try {
			...
			if (newCaretPosition[0] != newCaretPosition[1] || sentIndices[0] == sentIndices[1]
					|| taggedDoc.getSentenceNumber(sentNum).getUntagged().matches("\\s\\s*")) {
	...
	/**
	 * Doesn't change ANYTHING in the backend (meaning the TaggedDocument), all it
	 * does is completely replace the current text in the JTextPane with the backend
	 * contents. This means we're just scanning through all the TaggedSentences and
	 * pasting all their untagged strings into the main JTextPane.<br>
	 * <br>
	...
	public void syncTextPaneWithTaggedDoc() {
		...
		/**
		 * A small catch needed in case something goes wrong with the highlighting and
		 * sentence indices (which, while getting more and more unlikely, can still
		 * always happen). In these rare instances even reseting the editor won't work
		 * because newCaretPosition always ends up being -1, so here we prepare for that
		 * and so we always allow the editor to be reset so we can start fresh again
		 */
	...
	/**
	 * Updates a given sentence number with a new text. This DOES update the backend
	 * (meaning the TaggedDocument) by updating the given taggedSentence with the
	 * new string.
	 *
	...
	/**
	 * Updates all editor variables that may need changing at end of caret event
	 * (say, if the user started a new sentence, split one into two, deleted one,
	 * etc.). Should be called every caret event.
	 */
	...
	/**
	 * Stops any translation threads going on (if any) and also resets the
	 * EditorDriver instance to it's default values
	 */
	...
	/**
	 * Resets all relevant variables in EditorDriver to their defaults. Used for
	 * initialization and reProcessing.
	 */
