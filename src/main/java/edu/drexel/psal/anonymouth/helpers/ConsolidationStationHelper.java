...
public class ConsolidationStationHelper implements Runnable {
	...
	public ConsolidationStationHelper(HashMap<String, Word> finalHashMap, HashMap<String, Word> substrHashMap) { // ,
																													// boolean
																													// tags){
		// this.tags=tags;
	...
	public ConsolidationStationHelper(HashMap<String, Word> finalHashMap, HashMap<Integer, Word> intHashMap,
			boolean b) {
		intHash = true;
		this.finalHashMap = finalHashMap;
				...
				synchronized (this) {
					...
					for (int i = 0; i < fullstring.length(); i++) {
						if (fullstring.subSequence(i, i + subStr.length()).equals(subStr)) {
							...
							// updatedWord.adjustVals(substrHashMap.get(subStr).rank,
							// substrHashMap.get(subStr).infoGainSum);
			...
			while (iter2.hasNext()) {
				...
				synchronized (this) {
					if (fullstring.length() == integer.intValue()) {
						Word updatedWord = finalHashMap.get(fullstring);
						// updatedWord.adjustVals(intHashMap.get(integer).rank,
						// intHashMap.get(integer).infoGainSum);
						finalHashMap.put(fullstring, updatedWord);
					}
				}
			}
