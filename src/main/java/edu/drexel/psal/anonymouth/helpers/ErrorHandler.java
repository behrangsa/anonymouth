...
/**
 * Handles all fatal error pop-ups, stack trace logging, and quitting (if chosen
 * by the user or necessary).<br>
 * <br>
 * USAGE:<br>
...
public class ErrorHandler {
	...
	// Constants
	private static final String NAME = "( ErrorHandler ) - ";
	...
	/**
	 * Call this method when you have a fatal error anywhere after processing
	 * (meaning the user may have made changes to their document to Anonymize they
	 * want to save first). It will tell them there was a fatal error, and give them
	 * the option to wait so they can save their document or to immediately quit.
	 *
	 * @param parent
	 *            The parent of the error. null is an acceptable value if there is
	 *            no parent
	 * @param e
	 *            An exception (if there was one) for Logging the stack trace. null
	 *            is an acceptable value if there is no exception
	 */
	public static void fatalError(Component parent, Exception e) {
		...
		int ans = JOptionPane.showOptionDialog(parent,
				...
				"Fatal Error", JOptionPane.OK_CANCEL_OPTION, JOptionPane.ERROR_MESSAGE,
				UIManager.getIcon("OptionPane.errorIcon"), RESTART_OPTIONS, RESTART_OPTIONS[0]);
		if (ans == 1) {
			Logger.logln(NAME + "Fatal error encountered and the user chose to immediately quit, will exit now...",
					LogOut.STDERR);
	...
	/**
	 * Call this method when you have a fatal error anywhere during processing
	 * (meaning the user couldn't have made any changes to their document to
	 * Anonymize, so they won't have anything to save). Therefore, it will only tell
	 * them there was a fatal error and allow the user to click "Ok" to quit.
	 *
	 * @param e
	 *            An exception (if there was one) for Logging the stack trace. null
	 *            is an acceptable value if there is no exception.
	...
	/**
	 * Call this method when the Stanford POS error occurs and only for that. A
	 * popup will show telling the user what happened and quit when the user clicks
	 * "Ok"
	 */
		...
		SwingUtilities.invokeLater(new Runnable() {
			...
			public void run() {
				...
				JOptionPane.showMessageDialog(null,
						...
						"Stanford Part-Of-Speech Tagger Fatal Error", JOptionPane.ERROR_MESSAGE,
