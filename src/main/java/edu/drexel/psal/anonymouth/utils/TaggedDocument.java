 ...
 * <br>
 *
 * Since the SpecialCharTracker is specific to each individual TaggedDocument
 * instance (for example, a version on the undo stack may have EOS characters in
 * a different location than the one in the current TaggedDocument instance
...
public class TaggedDocument implements Serializable {
	...
	/**
	 * The way we have taggedSentences structured, while easy in most cases, breaks
	 * whenever the user tries to type after the last sentence. That's why we need
	 * to actively detect when they are doing so and create an empty tagged sentence
	 * for their new writing to go into every caret event that calls for it.<br>
	 * <br>
	 * 
	 * We create this blank tagged sentence whenever they are typing at the end of a
	 * document OR when they click to the end of document from somewhere else and
	 * there isn't already a blank sentence in place ready to accept the new text.
	 * That's where this boolean comes in, it keeps track for us whether or not it's
	 * necessary to create a new extra tagged sentence.
	 */
	public boolean endSentenceExists;

	/**
	 * Instead of having to call size() on taggedSentences every time we want to
	 * know the size, we can keep track of it ourselves and keep things constant
	 * time.
	 */
	public int numOfSentences = 0;

	/**
	 * Instead of having to call length() on all taggedSentences every time we want
	 * to know the length of the document, we can keep track of it ourselves and
	 * keep things constant time.
	 */
	public int length = 0;

	/**
	 * Greater than -1 when we are supposed to be keeping an eye out for a specific
	 * EOS character that we are as of yet unsure whether or not it's an EOS or just
	 * an abbreviation, ellipses, etc. Should be equal to the index of the EOS
	 * character we're watching so we can use it to set ignore to false if
	 * necessary.
	 */
	...
	/**
	 * Greater than -1 when we are supposed to be keeping an eye out for a specific
	 * EOS character at the very end of the document that we are as of yet unsure
	 * whether or not it's an EOS or just an abbreviation, ellispes, etc. Should be
	 * equal to the index of the EOS character (length of the document) we're
	 * watching so we can use it to set ignore to false if necessary.
	 */
	 ...
	 * @param usersDocument
	 *            Whether or not to just init the tracker and that's it, or to
	 *            instead makeAndTagSentences and that's it.
	 */
	...
	/**
	 * Constructor, accepts another TaggedDocument instance and initiates a deep
	 * copy of it (so we're not just creating another pointer like with == but
	...
	/**
	 * Replace Unicode format characters that will ruin the regular expressions
	 * (because non-printable characters in the document still take up indices, but
	 * you won't know they're there until you "arrow" though the document and have
	 * to hit the same arrow twice to move past a certain point. Note that we must
	 * use "Cf" rather than "C". If we use "C" or "Cc" (which includes control
	 * characters), we remove our newline characters and this screws up the
	 * document. "Cf" is "other, format". "Cc" is "other, control". Using "C" will
	 * match both of them.
	 * 
	 * @param text
	...
	/**
	 * Increases or decreases the length of this document based on the given amount.
	 * 
	 * @param amount
	 *            The amount to change the length by (negative for decrease,
	...
	/**
	 * Sets the initial length for this TaggedDocument, should be called only during
	 * initialization.
	 * 
	 * @param document
	...
	/**
	 * Clears all saved translations attributes to every TaggedSentence in this
	 * instance. This is to be used for undo/redo, as it's been pretty much proven
	 * we can't back that shit up without it grinding the editor to a halt.<br>
	 * <br>
	 *
	...
	/**
	 * Consolidates features for an ArrayList of TaggedSentences (does both word
	 * level and sentence level features)
	 * 
	 * @param alts
	 */
	...
	/**
	 * Takes a String of sentences (can be an entire document), breaks it up into
	 * individual sentences (sentence tokens), breaks those up into tokens, and then
	 * tags them (via MaxentTagger). Each tagged sentence is saved into a
	 * TaggedSentence object, along with its untagged counterpart.
	 * 
	 * @param untagged
	 *            String containing sentences to tag
	 * @param appendTaggedSentencesToGlobalArrayList
	 *            if true, appends the TaggedSentence objects to the
	 *            TaggedDocument's arraylist of TaggedSentences
	 * 
	 * @return An ArrayList of the completed TaggedSentences
	 */
	public ArrayList<TaggedSentence> makeAndTagSentences(String untagged,
			boolean appendTaggedSentencesToGlobalArrayList) {
		boolean fullDocument = false;

		/**
		 * If our length variable is 0, that means that the constructor with NO INITIAL
		 * DOCUMENT TEXT was run, therefore we set the length of the string as the
		 * TaggedDocument length and format the String since it's our main document
		 */
		if (length == 0) {
	...
	/**
	 * Creates a new sentence at the very end of the document with the given text
	 * (usually whitespace or most likely "")
	 * 
	 * @param text
	 *            The text you want the new TaggedSentence to be made with (most
	 *            likey "" or whitespace)
	 */
	 ...
	 * TaggedSentence, preserving all original Word objects.<br>
	 * <br>
	 * 
	 * Note that the sentences will be concatenated together in the order that they
	 * are passed into the method.
	 * 
	 * @param taggedSentences
	 *            A variable number of TaggedSentences
	 * 
	 * @return A single tagged sentences with the properties of all the sentences in
	 *         the list.
	 */
	...
	/**
	 * Accepts an ArrayList of TaggedSentences and returns a single TaggedSentence,
	 * preserving all original Word objects.<br>
	 * <br>
	 * 
	 * Note that the sentences will be concatenated together in the order that they
	 * are passed into the method.
	 ...
	 * 'taggedSentenceIndicesToConcat' into one TaggedSentence.<br>
	 * <br>
	 * 
	 * Note that the sentences will be concatenated together in the order that they
	 * are passed into the method.
	 ...
	 * TaggedSentence, and replaces the first with the concatenated TaggedSentence
	 ...
	 * 
	 * @return The completed and combined TaggedSentence
	 */
	public TaggedSentence concatRemoveAndReplace(TaggedSentence taggedSentenceOne, int tsOneIndex,
			TaggedSentence taggedSentenceTwo, int tsTwoIndex) {
	...
	/**
	 * removes TaggedSentence at indexToRemove from this TaggedDocument. Does NOT
	 * take care of any bookkeeping issues -- should only be called by methods that
	 * do (removeAndReplace)
	 * 
	 * @param indexToRemove
	 *            The index where you want to remove a taggedSentence from
	 * 
	 * @return The removed TaggedSentence. Returns a null TaggedSentence if no
	 *         TaggedSentence existed at the given index or if the index was not
	 *         within acceptable bounds (< 0 or >= numOfSentences)
	 */
	private TaggedSentence removeTaggedSentence(int indexToRemove) {
		...
		try {
			...
			Logger.logln(NAME + "Attemp to access TaggedSentence in an unacceptable index = " + indexToRemove,
					LogOut.STDERR);
		}
	 ...
	 *            An integer array of the indices you wish to remove (0, 1, 2, etc.)
	 */
	...
	/**
	 * Removes the existing TaggedSentence at the given index and replaces it with a
	 * new TaggedSentence made from the given text.
	 * 
	 * @param sentsToAdd
	...
	/**
	 * Removes multiple sentences and replaces them with a single TaggedSentence. To
	 * be used with the right-click menu item "combine sentences".
	 * 
	 * @param sentsToRemove
	 ...
	 * 
	 * @return The index (0-based) of the first sentence removed.
	 */
	...
	/**
	 * Removes the TaggedSentence at 'sentNumber', and switches in 'toAdd' in its
	 * place. Takes care of all bookkeeping issues.
	 * 
	 * @param sentNumber
	 *            The number of the TaggedSentence to remove and replace
	 * @param toAdd
	 *            The TaggedSentence to want to replace the removed TaggedSentence
	 *            with
	 * 
	 * @return
	 */
	public TaggedSentence removeAndReplace(int sentNumber, TaggedSentence toAdd) {
		...
		// no need to subtract one from numOfSentences when removing a sentence, because
		// we are putting a new sentence in its place immediatly
		TaggedSentence wasReplaced = removeTaggedSentence(sentNumber);
	...
	/**
	 * Checks all sentences in the tagged document and returns whether or not they
	 * are all translated.
	 */
	 ...
	 * 
	 * @return The formated string to print
	 */
	@Override
	...
	/**
	 * Updates the referenced Attributes 'toModifyValue's (present value) with the
	 * amount that must be added/subtracted from each respective value
	 * 
	 * @param oldSentence
	...
	private void updateReferences(TaggedSentence oldSentence, TaggedSentence newSentence) {
		...
		for (Reference ref : updatedValues.references) {
			// Logger.logln(NAME+"Attribute:
			// "+DataAnalyzer.topAttributes[ref.index].getFullName()+" pre-update value:
			// "+DataAnalyzer.topAttributes[ref.index].getToModifyValue());
			if (DataAnalyzer.topAttributes[ref.index].getFullName().contains("Percentage")) {
				// then it is a percentage.
				Logger.logln(NAME + "Attribute: " + DataAnalyzer.topAttributes[ref.index].getFullName()
						+ "Is a percentage! ERROR!", Logger.LogOut.STDERR);
			} else if (DataAnalyzer.topAttributes[ref.index].getFullName().contains("Average")) {
				// then it is an average
				Logger.logln(NAME + "Attribute: " + DataAnalyzer.topAttributes[ref.index].getFullName()
						+ "Is an average! ERROR!", Logger.LogOut.STDERR);
			} else {
				DataAnalyzer.topAttributes[ref.index]
						.setToModifyValue((DataAnalyzer.topAttributes[ref.index].getToModifyValue() + ref.value));
				// Logger.logln(NAME+"Updated attribute:
				// "+DataAnalyzer.topAttributes[ref.index].getFullName());
			}

			// Logger.logln(NAME+"Attribute:
			// "+DataAnalyzer.topAttributes[ref.index].getFullName()+" post-update value:
			// "+DataAnalyzer.topAttributes[ref.index].getToModifyValue());
		}
	}
	 ...
	 * 
	 * @param sentence
	 *            The TaggedSentence instance you want to obtain words from
	 * 
	 * @return An array of strings representing all the words in the given sentence.
	 */
	 ...
	 *            TaggedSentence
	 * 
	 * @return An array of strings representing all unique words from the
	 *         TaggedSentence
	 */
	 ...
	 * 
	 * @return An ArrayList of TaggedSentences
	 */
	 ...
	 * 
	 * @return The complete String of the document (untagged)
	 */
	 ...
	 *
	 * @return An integer array representing the lengths for each sentence
	 */
	 ...
	 * 
	 * @return The TaggedSentence at that index. Returns a null TaggedSentence if
	 *         none exist at the given index (or if it was an unacceptable position
	 *         < 0 or >= size)
	 */
	...
	/**
	 * Essentially the same thing as getSentenceNumAt(), except instead of accepting
	 * a sentence number and finding the taggedSentence that corresponds to that
	 * number it accepts an index (caret position), and finds the taggedSentence
	 * that corresponds to that index.
	 * 
	 * @param index
	 *            The position in the document text.
	 * 
	 * @return The TaggedSentence found at the index. If none exists, null is
	 *         returned.
	 */
	...
	/**
	 * Calculates and returns the current change needed for the document to reach
	 * it's optimal feature values.
	 * 
	 * @return The double value for the current change needed, to get the actual
	 *         "percent to the goal", simply take this return value, subtract it
	 *         from the saved value from getMaxChangeNeeded(), then divide by the
	 *         saved value from getMaxChangeNeeded(). This should all be handled
	 ...
	 * scenario (though it never happens in the real world since features rely so
	 * much on one another and can easily change each other).<br>
	 * <br>
	 *
	 * This should only need to be called ONCE, the return value should be saved and
	 * references for the future percent change calculations.
	 * 
	 * @return The beginning where getCurrentChangeNeeded() started from, acts as
	 *         the denominator for the percent change needed calculations in
	 *         AnonymityBar
	 */
