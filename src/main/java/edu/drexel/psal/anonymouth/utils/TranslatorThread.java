...
public class TranslatorThread implements Runnable {
	...
	/**
	 * Class that handles the 2-way translation of a given sentence. It starts a new
	 * thread so the main thread doesn't freeze up. This allows it to update the
	 * user about the progress of the translations.
	 * 
	 * @param main
	 *            GUIMain instance
	 */
	...
	/**
	 * Replaces the given "old" sentence in the translation sentence queue with the
	 * new sentence and puts the newsentence first in the translations queue.
	 * 
	 * @param newSentence
	 ...
	 * priority. If translations arent running, it starts running. If translations
	 * are already running, adds new sentences into the front of the queue.
	 */
	...
	/**
	 * To be called for re-processing and when the user turns off the translations.
	 * This essentially "freezes" the translations so they can be picked up right
	 * where they left off.
	 */
	// TODO Boolean value to completely reset translations for re-processing? Talk
	// to Andrew.
	public void reset() {
		stop = true;
		...
		/**
		 * Not sure if not resetting these will break something down the line, it seems
		 * to work fine without it. Commented them out so that if the user turns off
		 * translations but changes their mind the translations will pick off where they
		 * left off AND Upon re-processing the document translations will not
		 * re-translate sentences that didn't change.
		 */
		currentSentNum = 1;
		stoppedLangNum = 0;
		currentLangNum = 1;
	...
	@Override
	public void run() {
		stop = false; // Just making sure stop is now false since this is a new document translation
						// (if it the code below that turns it off wasn't run).
		...
		// translate all languages for each sentence, sorting the list based on anon
		// index after each translation
			...
			if (!sentences.get(currentSentNum - 1).isTranslated()) {
				...
				for (Language lang : translationFetcher.getUsedLangs()) {
					if (currentLangNum >= stoppedLangNum) {
						...
						String translation = translationFetcher
								.getTranslation(sentences.get(currentSentNum - 1).getUntagged(), lang);
						...
						String one = main.editorDriver.taggedDoc.getUntaggedSentences().get(main.editorDriver.sentNum)
								.trim();
						...
						if (addSent) {
							...
							if (sentences.contains(oldSentence))
								...
								main.translationsProgressBar
										.setMaximum(sentences.size() * translationFetcher.getUsedLangs().length);
	...
	/**
	 * Cleans up resources used by the translator at the end of translating all
	 * sentences.
	 */
	private void translationsEnded() {
		/**
		 * We're making another outside call to a "switchTo" method since we want the
		 * progress bar to immediately disappear when all translations are obtained
		 * (instead of having to wait a second or two for the TranslationsPanel to get
		 * to it).
		 */
