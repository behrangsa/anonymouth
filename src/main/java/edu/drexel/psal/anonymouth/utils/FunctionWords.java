...
/**
 * "The WINFY PRUNKILMONGER from the GLIDGEMENT MOMINKLED and BRANGIFIED all his
 * LEVENSERS VEDEROUSLY"<br>
 * <br>
 * 
 * In the above sentence, even though the capitalized words make no sense, we
 * can still derive grammatical meaning from them (WINFY is most likely an
 * adjective, PRUNKILMONGER, GLIDGEMENT, and LEVENSERS most likely nouns,
 * etc.).<br>
 * <br>
 * 
 * In short, Function words are words that have more ambiguous meaning (like
 * another, below, there, this, etc.), but do express some grammatical
 * relationship. For more examples of FunctionWords, see
 * "koppel_function_words.txt" in the jsan_resources directory.<br>
 * <br>
 * 
 * This class keeps track of the function words from that file and initializes a
 * Trie of the function words for searching. At this time this doesn't seem to
 * be implemented anywhere.<br>
 * <br>
 * 
 * TO USE:<br>
 * 1) Initialize<br>
...
public class FunctionWords implements Runnable {
	...
	/**
	 * Main run method for the class. Should be called when you want to initialize
	 * and fill the functionWordList, functionWordArray, and trie objects. Should,
	 * to my knowledge anyway, be only called once.
	 */
	@Override
	public void run() {
		...
		// We only want to use a predetermined number of words from the ArrayList we
		// obtained
	 ...
	 * @return The function word at the given index. Returns null if not found or if
	...
	protected String getWordAt(int index) {
		...
		try {
			...
			Logger.logln(NAME + "Tried to get function word at an unacceptable index: " + index
					+ " (functionWordArray.length = " + functionWordArray.length + ")");
		}
	...
	/**
	 * ====================MAIN METHOD TEST==================== To be used only for
	 * testing purposes. Tests all current methods in FunctionWords.java and
	 * displays results.
	 */
