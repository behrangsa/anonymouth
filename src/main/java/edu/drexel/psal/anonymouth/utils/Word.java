...
/**
 * Holds a 'word' as a String, and retains its rank and the collective
 * information gain of all features that have been found within the 'word' Can
 * also hold two or three 'words' in 'word' field.
 * 
...
public class Word implements Comparable<Word>, Serializable {
	 ...
	 * 
	 * @param word
	 *            the word to construct a Word for
	 */
	public Word(String word) {
		wordLevelFeaturesFound = new SparseReferences(10);// probably won't find > 10 features in a word (wild guess)
		partOfSpeech = new ArrayList<String>(); // is an array list because it is possible to have one word as more than
												// one part of speech. It doesn't seem to make sense at this point to
												// count them as different words.
		this.word = word;
	}
	...
	/**
	 * Computes the AnonymityIndex of this Word: SUM
	 * ((#appearancesOfFeature[i]/numFeaturesFoundInWord)*(infoGainOfFeature[i])*(%changeNeededOfFeature[i])).
	 * These numbers are determined by the percent change needed and information
	 * gain for each feature found in the "Word" from each feature's respective
	 * Attribute object in the static Attribute array 'topAttributes'
	 * 
	...
	/**
	 * Slightly modified version of getAnonymityIndex() such that the call to
	 * getPercentChangeNeeded() inside will return negative values as well. This is
	 * used for us to better gauge what words should be removed.
	 * 
	 * @return
	 */
	public double getWordAnonymity() {
		...
		for (i = 0; i < numFeatures; i++) {
			...
			double value = tempFeature.value;
			anonymityIndex += (value) * (currentAttrib.getInfoGain())
					* (currentAttrib.getPercentChangeNeeded(false, false, false));// for 'getPercentChangeNeeded', the
																					// first boolean says not to
																					// normalize the result to the
																					// baslinePercentChangeNeeded, the
																					// second says to invert the
																					// percentage, and the third says to
																					// take the absolute value (ignore
																					// direction of change)
		}
		return anonymityIndex;
	}
	...
	/**
	 * Merges two words, provided that the 'word' (string) inside are equivalent
	 * (case sensitive), and that both 'word' strings have been determined to be of
	...
	/**
	 * defines two Word objects to be equal if they contain the same 'word' String
	 * object.
	 * 
	...
	public String toString() {
		return "[ WORD: " + word + " ||| Anonymity Index: " + getAnonymityIndex() + " ||| wordLevelFeaturesFound: "
				+ wordLevelFeaturesFound + "]";
	}
