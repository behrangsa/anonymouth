 ...
 * instances of '.', '!', and '?' in a document, so it's not restricted to just
 * sentence ends but rather any time those appear (so abbreviations, ellipses,
 * etc. all are kept here as well). We use this to determine which EOS
 * characters are "real". Instead of using swap characters we are instead opting
 * to preserve the original text for the tagged sentences and backend and
 * instead keep an "ignore" boolean tied to each index of an EOS character, and
 * SentenceMaker will then skip over these since they aren't actually and
 * EOS.<br>
 * <br>
 *
...
public class SpecialCharTracker implements Serializable {
	...
	/**
	 * Constructor, takes another instance of SpecialCharTracker and does a deep
	 * copy of it's contents.
	 * 
	 * @param eosCT
	...
	/**
	 * Utilities method for the SpecialCharTracker constructor above and performs a
	 * deep copy for the current TextWrapper object ArrayList.
	 * 
	 * @param specialCharTracker
	 *            The instance of specialCharTracker we're deep copying
	 * @param curIndex
	 *            The current index we're on, representing the current TextWrapper
	 *            array and size we're working with (Quotes, Parenthesis, Brackets,
	 *            etc.)
	 */
	...
	/**
	 * Initialies the textWrappers array with empty TextWrapper Arrays
	 */
	...
	/**
	 * Initializes the textWrapperSizes array with 0's for each one.
	 */
	 ...
	 * 
	 * @param index
	 *            The index past which you want all EOS objects tracked to be
	 *            shifted
	 * @param shiftAmount
	 *            The amount of spaces you want to shift them by (negative if you
	 *            want left)
	 */
	...
	/**
	 * To be called by EditorDriver's getSentenceIndices() every single time the
	 * sentence highlight is being changed (new sentence, sentence changes, etc).
	 * This is to ensure that the highlight extends to what the user precieves as a
	 * full sentence to give the illusion of a complete one (while in actuality they
	 * are separate tagged sentence objects). We do it this way since, in the past,
	 * we were actually combining and splitting this sentences based on text wrapper
	 * positions but to be quite honest it sucked. This way, it's faster, smarter,
	 * and all around easier to understand.
	 *
	 * Say a sentence like "Hi."'s indices is passed. This will determine if it's in
	 * between text wrappers, like "She said "Nice to meet you. Hi. My name's Lucy."
	 * kind of loud." If this is the case, instead of just highlighting the sentence
	 * "Hi.", we extend the highlight to all the tagged sentences we deem to be part
	 * of a single one. The best way to understand how this is done is simply to
	 * read the code.
	 *
	 * @param start
	 ...
	 *
	 * @return An integer array representing the new highlights. The indices
	 *         represent:<br>
	 *         [0] = Where the start of the highlight should be<br>
	 *         [1] = Where the end of the highlight should be<br>
	 *         [2] = The number of extra sentences included in the extended
	 *         highlight to the LEFT<br>
	 *         [3] = The number of extra sentences included in the extended
	 *         highlight to the RIGHT<br>
	 *         <br>
	 *
	 *         The last two indices of the array are specifically for the automatic
	 *         word to remove highlights, as they require the sentence numbers to
	 *         obtain the words to remove. If that could be updated with no loss in
	 *         performance to instead use just indices then we could get ride of
	 *         these "extra sentences" portion of this return array.
	 */
	public int[] extendHighlights(int start, int end, int[] allSentIndices, int[] sentenceLengths,
			int selectedSentence) {
		int[] highlightIndices = {start, end, 0, 0};
		for (int i = 0; i < NUM_OF_TEXT_WRAPPERS; i++) {
			highlightIndices = extendHighlights(highlightIndices[0], highlightIndices[1], allSentIndices,
					sentenceLengths, selectedSentence, allTextWrappers.get(i), allTextWrapperSizes[i],
					highlightIndices);
		}
	...
	private int[] extendHighlights(int asdf, int endgf, int[] allSentIndices, int[] sentenceLengths,
		...
		for (int t = 0; t < curSize; t++) {
			if (curTextWrapper.get(t).closed) {
				...
				if (curTextWrapper.get(t).startIndex > highlightIndices[0]
						&& curTextWrapper.get(t).endIndex < highlightIndices[1]) {
					continue;
					// Whole sentence between text wrapper
				} else if (highlightIndices[0] >= curTextWrapper.get(t).startIndex
					...
					for (int sent = selectedSentence; sent >= 0; sent--) {
						if (highlightIndices[0] > (allSentIndices[sent] - sentenceLengths[sent])
								&& highlightIndices[0] < allSentIndices[sent]) {
					...
					for (int sent = selectedSentence; sent < main.editorDriver.taggedDoc.numOfSentences; sent++) {
						if (highlightIndices[1] > (allSentIndices[sent] - sentenceLengths[sent])
					...
					// Start of text wrapper in sentence
						...
						&& highlightIndices[1] > curTextWrapper.get(t).startIndex) {
					highlightIndices[1] = curTextWrapper.get(t).endIndex;
					for (int sent = selectedSentence; sent < main.editorDriver.taggedDoc.numOfSentences; sent++) {
						if (highlightIndices[1] > (allSentIndices[sent] - sentenceLengths[sent])
								&& highlightIndices[1] < allSentIndices[sent]) {
				...
				} else if (highlightIndices[0] <= curTextWrapper.get(t).endIndex
						&& highlightIndices[1] > curTextWrapper.get(t).endIndex) {
					highlightIndices[0] = curTextWrapper.get(t).startIndex;
					for (int sent = selectedSentence; sent >= 0; sent--) {
						if (highlightIndices[0] > (allSentIndices[sent] - sentenceLengths[sent])
	 ...
	 * 
	 * @param lowerBound
	 *            The beginning of hte range you want to remove from (includes this
	 *            position)
	 * @param upperBound
	 *            The end fo the range you want to remove from (does not include
	 *            this position)
	 * 
	 * @return Whether or not any Text Wrapper Objects were removed from the given
	 *         range
	 */
	...
	public void removeTextWrappersInRange(int lowerBound, int upperBound, ArrayList<TextWrapper> curTextWrapper,
			int curSize) {
			...
			if (curTextWrapper.get(i).startIndex >= lowerBound && curTextWrapper.get(i).startIndex < upperBound) {
				if (curTextWrapper.get(i).closed) {
					if (curTextWrapper.get(i).endIndex >= lowerBound && curTextWrapper.get(i).endIndex < upperBound) {
						Logger.logln(NAME + "Removed TextWrapper Object at " + curTextWrapper.get(i).startIndex + " - "
								+ curTextWrapper.get(i).endIndex);
						curTextWrapper.remove(i);
						i--; // decrement 'i' so that we don't miss the object that shifts down into the spot
								// just freed.
	 ...
	 * typed, regardless of whether or not it's the starting quote or a closing one.
	 *
	 * addQuote() checks to see if there there is a non-closed quote prior to this
	 * one's index, and if there is it will treat this ass the closing index and add
	 * that index as endIndex to that TextWrapper object, otherwise it will create a
	 * new Quote object with this passed index as the startIndex, endIndex = -1, and
	 * closed = false
	 * 
	 * @param index
		...
		for (int i = 0; i < allTextWrapperSizes[0]; i++) {
			if (!quotes.get(i).closed) {
				...
				if (quotes.get(i).endIndex == -1) {
					// We need to check to make sure we are closing the closest available unclosed
					// text wrapper
					if (closingQuote == -1) {
						closingQuote = i;
					} else {
						// If it's closer, then we're closing this text wrapper and not the one farther
						// away
	 ...
	 * 
	 * @return True or false, depending on whether or not it's a quote
	 */
	 ...
	 *            The character you want to test
	 * 
	 * @return True or false, depending on whether or not it's a parenthesis
	 */
	...
	/**
	 * Determines whether or not the passed paren is a closing one or not
	 * 
	 * @param paren
	 *            The paren char you want to check
	 * 
	 * @return True or false, depending on whether or not it's a closing paren
	 */
	...
	public void addParenthesis(int index, char paren) {
		...
		for (int i = 0; i < allTextWrapperSizes[1]; i++) {
			if (!parenthesis.get(i).closed) {
				// If the text wrapper doesn't have an end index and the new character is a
				// closing one
				if (parenthesis.get(i).endIndex == -1 && isClosingParenthesis(paren)) {
					...
					// If the text wrapper doesn't have a start index and the new character is a
					// starting one
	 ...
	 *            The character you want to test
	 * 
	 * @return True or false, depending on whether or not it's a bracket
	 */
	...
	/**
	 * Determines whether or not the passed bracket is a closing one or not
	 * 
	 * @param bracket
	 *            The bracket char you want to check
	 * 
	 * @return True or false, depending on whether or not it's a closing bracket
	 */
	 ...
	 * typed, regardless of whether or not it's the opening bracket or a closing
	 * one.
	 *
	 * addBracket() checks to see if there there is a non-closed bracket prior to
	 * this one's index, and if there is it will treat this ass the closing index
	 * and add that index as endIndex to that TextWrapper object, otherwise it will
	 * create a new Bracket object with this passed index as the startIndex,
	 * endIndex = -1, and closed = false
	 * 
	 * @param index
	 ...
	 *            The character you want to test
	 * 
	 * @return True or false, depending on whether or not it's a squiggly
	 */
	...
	/**
	 * Determines whether or not the passed squiggly is a closing one or not.
	 * 
	 * @param squiggly
	 *            The squiggly char you want to check
	 * 
	 * @return True or false, depending on whether or not it's a closing squiggly
	 */
	 ...
	 * typed, regardless of whether or not it's the opening squiggly or a closing
	 * one.
	 *
	 * addsquiggly() checks to see if there there is a non-closed squiggly prior to
	 * this one's index, and if there is it will treat this ass the closing index
	 * and add that index as endIndex to that TextWrapper object, otherwise it will
	 * create a new squiggly object with this passed index as the startIndex,
	 * endIndex = -1, and closed = false
	 * 
	 * @param index
	 *            The index of the newly added Bracket
	...
	/**
	 * Adds the EOS character to the tracker, this should be called EVERY TIME a new
	 * EOS character is typed, regardless of whether or not it's part of an
	 * abbreviation, an end of a sentence, etc.<br>
	 * <br>
	 *
	 ...
	 * @param ignore
	 *            Whether or not to skip the EOS character as an actual sentence
	 *            end.
	 */
	...
	/**
	 * Obtains the EOS character at the given index and sets whether or not to
	 * ignore it.
	 * 
	 * @param index
	 ...
	 *
	 * @return Whether or not an EOS character was found at the given index
	 */
	 ...
	 *            The character you want to test
	 * 
	 * @return True or false, depending on whether or not it's an end of sentence
	 *         character
	 */
	...
	/**
	 * Checks the given index for an EOS character and returns true if it finds one
	 * and it is not set to be ignored.
	 * 
	 * @param index
	 *            - The index where we want to check if there is an EOS character
	 *            present
	 * @return result - whether or not an EOS character that is NOT set to be
	 *         ignored exists in the index given.
	 */
	/**
	 * Checks whether or not a sentence ends at the given index (meaning we are
	 * checking to see if an EOS character exists in our tracker at the given index
	 * AND if it's not being ignored).
	 * 
	 * @param index
	 *            The index you want to check for a sentence end.
	 * 
	 * @return Whether or not a sentence ends at the given index
	 */
	public boolean isSentenceEndAtIndex(int index) {
		boolean result = false;

		/**
		 * For first processing (TODO and possibly reprocessing, if this is the case
		 * then this check will only work for the first process), we don't want the code
		 * below to run and check everything, just return true every time. (Just trust
		 * me, it works).
		 */
		if (eosSize == 0) {
			result = true;
	...
	/**
	 * Removes any EOS objects in between the given indices as [5, 10), meaning
	 * inclusive for the first and exclusive for the last.
	 * 
	 * @param lowerBound
	 *            The beginning of the range you want to remove from (includes this
	 *            position)
	 * @param upperBound
	 *            The end of the range you want to remove from (does not include
	 *            this position)
	 * 
	 * @return Whether or not any EOSes were removed from the given range
	 */
...
/**
 * Holds the EOS characters at a given location within the document with respect
 * to the first index of the document (0).
 *
 * @author Andrew W.E. McDonald
 * @author Marc Barrowclift
 */
class EOS implements Serializable {
	 ...
	 * debugging
	 * 
	 * @return Our modified string representation of the EOS character
	 */
	@Override
	public String toString() {
