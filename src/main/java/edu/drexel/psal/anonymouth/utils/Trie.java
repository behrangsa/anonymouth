 ...
 * word. Functionalty can be extended to all ASCII characters easily by
 * decreasing the offset, and increasing the size of the TrieNode array. Trie
 * doesn't save actual characters/strings, only the characters' positions:
 * 
 * a TrieNode only contains an array of TrieNodes. When adding a word, if the
 * first letter is 'a', the char value of 'a' is 97, and the offset is 96, so 97
 * minus 96 = 1 => create a new TrieNode at index 1 in the root's TrieNode child
 * array, and so on.
 * 
...
public class Trie {
	 ...
	 * @param c
	 *            letter to add
	 * @return The newly created child TrieNode of the input TrieNode in the index
	 *         of the input character's value minus the offset.
	 */
...
/**
 * The TrieNode. Uses an empty constructor, and only contains an array of
 * TrieNodes. No node ever has a 'value'. Each child node (each node in the
 * index), is either equal to 'null', or contains a new TrieNode.
 * 
 * @author Andrew W.E. McDonald
 *
 */
