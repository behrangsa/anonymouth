...
public class ConsolidationStation {
	...
	/**
	 * constructor for ConsolidationStation. Depends on target values, and should
	 * not be called until they have been selected.
	 * 
	 * @param attribs
	 * @param parsed
	 */
	...
	/**
	 * Adds Reference objects to each Word objects' SparseReferences indicating
	 * which features were found in each word, and how many times that feature was
	 * found
	 * 
	 * @param word
	 */
		...
		for (int i = 0; i < attribLen; i++) {
			...
			if (toAddLength == 0) {
				...
				if (tempNumber > 0) {
					...
					// Logger.logln(NAME+"Value i: "+i+" Value indexOf Attrib:
					// "+DataAnalyzer.topAttributes[i].getIndexNumber()+" Attribute:
					// "+DataAnalyzer.topAttributes[i].getFullName()+" the word: "+wordString);
					word.wordLevelFeaturesFound.addNewReference(i, tempNumber);
					// Logger.logln(NAME+"Added a feature:
					// "+word.wordLevelFeaturesFound.toString());
				}
			}
		}
	}
	...
	public static void setSentenceFeatures(TaggedSentence sent) {
		// TODO -- We already found the 'word' level features, and they are stored
		// differently/independently... so, we start with word bigrams, and move up
		// (trigrams, possibly POS bi/trigrams, and punctutation)
		...
		for (int i = 0; i < attribLen; i++) {
			...
			if (!stringInBrace.equals("") && stringInBrace.charAt(0) == '(' && !curAttribute.isPartofSpeech()) {// if
																												// the
																												// string
																												// in
																												// braces
																												...
																												// trigram
																												// feature
				// System.out.printf("Finding feature number '%d': '%s' in sentence,
				// '%s'\n",i,stringInBrace,sent.getUntagged());
				// Check the number of open parentheses -- if (1), continue, if (2), it's a
				// bigram, if (3) it's a trigram
				int numOpenParens = 0;
				...
				if (numFound > 0) {
					// add a reference to the feature and the number of times it appeared to the
					// sentence
					// Logger.logln(NAME+"AddNewReference from ConsolStation.featurePacker");
					// Logger.logln(NAME+"Value i: "+i+" Value indexOf Attrib:
					// "+DataAnalyzer.topAttributes[i].getIndexNumber()+" Attribute:
		...
		// System.out.println("number of sentence level features found:
		// "+sent.sentenceLevelFeaturesFound.length());
	}

	/**
	 * This is the test method to see if a bunch of translated sentences may be able
	 * to be pieced together to produce an anonymous document It replaces the
	 * original sentences with the best replacement out of the replacements given
	 * for each sentence (version 1) It outputs files containing the new document
	 * text comprised of the first, second, third, etc. best replacements. This
	 * exits Anonymouth when finished. (version 2)
	 * 
	 * @param attributable
	 *            The TaggedDocument (which must already be fully tagged, with all
	 *            SparseReferences added [i.e. all features found])
	 * @param newSentences
	 *            Basically a 2D array of sentences. Each inner ArrayList contains
	 *            all potential sentence substitutions for the sentence number that
	 *            corresponds to that ArrayList's index in the outer ArrayList.
	 */
	public static void anonymizeDocument(TaggedDocument attributable, ArrayList<ArrayList<String>> newSentences,
			boolean doVersionTwo) {
		...
		for (sentenceNumber = 0; sentenceNumber < numSents; sentenceNumber++) {
			...
			optionNumber = 0;
			// create a TaggedSentence for each alternative sentence for that sentence,
			// calculate the Anonymity Index, and save both in parallel ArrayLists
			...
			while (sentenceChoices.hasNext()) {
				...
				System.out.printf("option '%d' (with AI '%f'): %s\n", optionNumber, anonymityIndices[optionNumber][0],
						ts.getUntagged());
				optionNumber += 1;
			}
		...
		// Now we know the Anonymity Index of each possible substitute for each
		// sentence, so we find the sentence with the highest Anonymity
		for (sentenceNumber = 0; sentenceNumber < numSents; sentenceNumber++) {
			double[][] thisSentenceOptionsAI = allAnonymityIndices.get(sentenceNumber);
			// sort the 2D array of doubles by the Anonymity Index, which is index '0'.
			// Index '1' contains the original index (which corresponds to the actual
			// sentence's index in its own array (the inner arrays in 'replacements')
			Arrays.sort(thisSentenceOptionsAI, new Comparator<double[]>() {
				public int compare(final double[] first, final double[] second) {
					return ((-1) * ((Double) first[0]).compareTo(((Double) second[0]))); // multiplying by -1 will sort
		...
		// todo for the time being, just to test this out, I'm going to replace all
		// sentences in the TaggedDocument with each sentences highest ranked
		// replacement
		...
		if (!doVersionTwo) {
			...
			for (sentenceNumber = 0; sentenceNumber < numSents; sentenceNumber++) {
				...
				if (numOptions < 1)
					continue;
				ayEye = allAnonymityIndices.get(sentenceNumber)[0][0]; // Since this was sorted in reverse order, index
																		// '0' contains the highest AI, and index '1'
																		// tells us the index of the corresponding
																		// sentence in its array.
				actualSentenceIndex = (int) allAnonymityIndices.get(sentenceNumber)[0][1]; // Since this was sorted in
																							// reverse order, index '0'
																							// contains the highest AI,
																							// and index '1' tells us
																							// the index of the
				...
				attributable.taggedSentences.set(sentenceNumber,
						replacements.get(sentenceNumber).get(actualSentenceIndex));
			}
			...
			try {
				...
				for (qualityRank = 0; qualityRank < numTranslations; qualityRank++) { // because these were sorted in
																						// reverse order, '0' is the
																						// highest quality.
					...
					for (sentenceNumber = 0; sentenceNumber < numSents; sentenceNumber++) {
						...
						if (numOptions < 1) {
							replacementSent = "[" + attributable.taggedSentences.get(sentenceNumber).getUntagged()
									+ "]";
							ayEyeRay[sentenceNumber] = attributable.taggedSentences.get(sentenceNumber)
									.getSentenceAnonymityIndex();
						} else {
							// System.out.println("sentenceNumber: "+sentenceNumber+" numOptions:
							// "+numOptions+"+ qualityRank = "+qualityRank);
							ayEyeRay[sentenceNumber] = allAnonymityIndices.get(sentenceNumber)[qualityRank][0]; // Since
																												// this
																												// was
																												// sorted
																												// in
																												...
																												// its
																												// array.
							actualSentenceIndex = (int) allAnonymityIndices.get(sentenceNumber)[qualityRank][1]; // Since
																													// this
																													// was
																													// sorted
																													// in
					...
					anonIndexWriter.write("For document with rank '" + qualityRank
							+ "', the anoymity indices were (left => sentence '0', right => last sentence):\n[");
					...
					anonIndexWriter.flush();
					File thisReplacementDoc = new File(
							dirName + "/" + ThePresident.sessionName + "_AnonIndex_" + qualityRank + ".txt");
					try {
	...
	/**
	 * Goes through all words in the TaggedDocument instance, sorts them from least
	 * to greatest in terms of Anonymity Index, and returns either the lowest ranked
	 * or highest ranked percent as strings. NOTE: * percentToReturn, the percent of
	 * highest or lowest ranked words (String) to return, should be a number between
	 * 0 and 1. * if findTopToRemove is false, the highest ranked Words will be
	 * returned (as Strings) (these would then be the most important words to ADD to
	 * the documentToAnonymize) * if findTopToRemove is true, the lowest ranked
	 * Words will be returned (as String) (these would then be the most important
	 * words to REMOVE from the documentToAnonymize)
	 * 
	 * @param docsToConsider
	 *            the TaggedDocuments to extract Words from
	 * @param findTopToRemove
	 *            true to find the top words to remove, false to find the top words
	 *            to add
	 * @param percentToReturn
	 *            the percent of words found to return (should probably be MUCH
	 *            smaller if finding top words to add, because this will look at all
	 *            otherSampleDocuments
	 * @return
	 */
	public static ArrayList<String> getPriorityWordsToAdd(ArrayList<TaggedDocument> docsToConsider,
		...
		/**
		 * NOTE: We MUST be using getAnonymity() for both Word's compareTo method and
		 * for retrieving the word's anonymity index. This is because the old method
		 * call to getAnonymityIndex() did not allow the index to be negative. This
		 * wouldn't work here since we use the more negative the index is to rate how
		 * important it is a word be removed. As a result, we are now using a slightly
		 * modified version of getAnonymityIndex() called getAnonymity() so that we get
		 * the negative range back.
		 */
		...
		if (mergedNumWords <= numToReturn) {
			Logger.logln(NAME
					+ "The number of priority words to return is greater than the number of words available. Only returning what is available");
	 ...
	 * Goes through all words in the TaggedDocument instance, sorts them from least
	 * to greatest in terms of Anonymity Index, and returns either the lowest ranked
	 * or highest ranked percent as strings as well as the number of times the given
	 * word occurs. NOTE: * percentToReturn, the percent of highest or lowest ranked
	 * words (String) to return, should be a number between 0 and 1. * if
	 * findTopToRemove is false, the highest ranked Words will be returned (as
	 * Strings) (these would then be the most important words to ADD to the
	 * documentToAnonymize) * if findTopToRemove is true, the lowest ranked Words
	 * will be returned (as String) (these would then be the most important words to
	 * REMOVE from the documentToAnonymize)
	 * 
	 * @param docsToConsider
	 *            the TaggedDocuments to extract Words from
	 * @param findTopToRemove
		...
		try {
			...
			for (int i = 0; i < numToReturn; i++) {
				if ((tempWord = words.get(i)).getAnonymityIndex() <= 0) {
					...
					if (occurances == 0) {
						Logger.logln(
								NAME + "Skipping word to remove, not actually in any documents (Look into this!!!!)"); // TODO
																														// Should
																														// not
																														// be
																														// here,
						...
						toReturn.add(new String[]{tempWord.word, Integer.toString(occurances)});// +"
																								// ("+tempWord.getAnonymity()+")");
					}
				} else {
					break;
				}
	 ...
	 * 
	 * @param text
	 *            - The text you want to find the word in
	 * @param word
	...
	public static ArrayList<Word> removeDuplicateWords(ArrayList<Word> unMerged) {
		HashMap<String, Word> mergingMap = new HashMap<String, Word>((unMerged.size()));// Guessing there will be at
																						// least an average of 3
																						// duplicate words per word ->
																						// 1/3 of the size is needed
		for (Word w : unMerged) {
			if (mergingMap.containsKey(w.word) == true) {
				...
				if (w.equals(mergingMap.get(w.word))) {
					...
					if (!w.wordLevelFeaturesFound.equals(mergingMap.get(w.word).wordLevelFeaturesFound)) {
						Logger.logln("(ConsolidationStation) - The wordLevelFeaturesFound in the words are not equal.",
								Logger.LogOut.STDERR);
					}
				} else {
					Logger.logln("(ConsolidationStation) - Problem in mergeWords--Words objects not equal",
							Logger.LogOut.STDERR);
				}
	...
	// Logger.logln("(ConsolidationStation) - TOADD:
	// "+toAdd.get(i).getStringInBraces());
	...
	// for(int j=0;j<str.length()-toAddLength;j++){//loops through word to check
	// if/howManyTimes the stringInBraces is found in the word.
	// if(str.substring(j,
	// j+toAddLength).equals((String)toAdd.get(i).stringInBraces)){
	// tempNumber++;
	// }
	// }
	...
	// Logger.logln("(ConsolidationStation) - TOREMOVE:
	// "+toRemove.get(i).getStringInBraces());
	...
	// if/howManyTimes the stringInBraces is found in the word.
	// if(str.substring(j,
	// j+toAddLength).equals((String)toRemove.get(i).stringInBraces)){
	// tempNumber++;
	// }
	// }
	// //newWord.adjustVals(tempNumber, featureInfoGain,
	// featurePercentChange);//respresents a word to remove, so it should be
	// negative
	// }
	// }
