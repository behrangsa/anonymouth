...
public class GramMatcher {
	...
	/*
	 * public static void main(String args[]){ GramMatcher gm = new GramMatcher();
	 * String bg1 = "(.)-(said)"; String bg2 = "(then)-(.)"; String tg1 =
	 * "(Oh)-(,)-(well)"; String tg2 = "(good)-(though)-(.)"; String sent1 =
	 * "\"Hello there!\", he said. I was thinking, \"Oh, well, I guess I'll talk to him then.\" This is good though. said"
	 * ; int num_bg1 = gm.getOccurrencesOfBigram(bg1,sent1); int num_bg2 =
	 * gm.getOccurrencesOfBigram(bg2,sent1); int num_tg1 =
	 * gm.getOccurrencesOfTrigram(tg1,sent1); int num_tg2 =
	 * gm.getOccurrencesOfTrigram(tg2,sent1); System.out.
	 * printf("The number of each feature found is:\nbg1 => %d\nbg2 => %d\ntg1 => %d\ntg2 => %d\n"
	...
	public int getOccurrencesOfBigram(String wordBigram, String stringToSearch) {
		...
		int totalBigramLen = 0; // the total length includes one space between the words/characters (because
								// there had to be a space in the document for it to be considered a bigram and
								// not a word)
	...
	public static ArrayList<int[]> findIndices(String stringToTest, String theRegEx) {
		...
		stringToTest = stringToTest.replaceAll("\\p{C}", " ");// replaces all unicode control / invisible characters and
																// unused code points
		...
		while (true) {
			...
			if (found == true) {
				int start = m.start();
				int end = m.end();
				// System.out.println("Found match for: '"+theRegEx+"' =>
				// "+stringToTest.substring(start,end));
	...
	public int getOccurrencesOfTrigram(String wordTrigram, String stringToSearch) {
		...
		int numTokesMinusOne = numTokes - 1;
		String[] tokens = new String[numTokes];
		int totalTrigramLen = 0; // this count includes 1 space between words/characters (otherwise the feature
									// wouldn't be a trigram)
		boolean sent_end = false;
		int i;
		for (i = 0; i < numTokes; i++) {
			tokeLen = split[i].length();
		...
		if (sent_end)
			...
			return findIndices(stringToSearch, "\\b" + tokens[0] + "\\s*" + tokens[1] + "\\s*" + tokens[2] + "\\b{1}+")
					.size();
	}
