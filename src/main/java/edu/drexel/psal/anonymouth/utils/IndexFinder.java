...
/**
 * Finds the indices of sections of the Author's document to be modified that
...
public class IndexFinder {
	 ...
	 * @param theDoc
	 * @param theList
	 * @return ArrayList<int[]> with the starting and ending indices of all words in
	 *         'theList' that were found in 'theDoc'
	 */
	public static ArrayList<int[]> findIndices(String theDoc, String theWord) {
		...
		try {
			...
			Logger.logln("'" + theWord
					+ "' - was not matched. Are there symbols or spaces between the single quotes? If so, that may be why.");
		}
		return theIndices;
	}
	 ...
	 * @return
	 */
	public static ArrayList<int[]> findIndicesInSection(String theDoc, String theWord, int startSection,
			int endSection) {
		ArrayList<int[]> theIndices = new ArrayList<int[]>();
		int j;
		int start;
	...
	/**
	 * Finds indices of the starting and ending locations of each place the given
	 * regular expression is found in the given string
	 * 
	 * @param stringToTest
	 *            the string/document to test the regex on
	 * @param theRegEx
	 *            the expression to find
	 * @return ArrayList containing the starting and ending indices of each match.
	 */
	...
	/**
	 * Finds indices of word bigrams in the input string (document), and returns an
	 * Arraylist of integer arrays of starting and ending indices.
	 * 
	 * @param theDoc
	 * @param theList
	 * @return
	...
	public static ArrayList<int[]> findWordBigramIndices(String theDoc, LinkedList<String> theList) {
		...
		String wordOne;
		String wordTwo;
		int listLengthDivTwo = listLength / 2; // unless something odd happens, this number will always be a multiple of
												// 2
		String spaces = " ";
		theDoc = theDoc.replaceAll("\\p{C}", " ");
		for (i = 0; i < listLengthDivTwo; i++) {
			...
			try {
				...
				Logger.logln("'" + wordOne + " " + wordTwo
						+ "' -was not matched. if there is more than 1 space or symbols exist within the single quotes, that may be why.");
			}
		}
		return theIndices;
	}
	...
	/**
	 * Finds word trigram indices in the input string (document), and returns an
	 * Arraylist of integer arrays of starting and ending indices.
	 * 
	 * @param theDoc
	 * @param theList
	...
	public static ArrayList<int[]> findWordTrigramIndices(String theDoc, LinkedList<String> theList) {
		...
		String wordTwo;
		String wordThree;
		int listLengthDivThree = listLength / 3; // unless something odd happened, this number will always be a multiple
													// of three.
		String spaces = " ";
		theDoc = theDoc.replaceAll("\\p{C}", " ");
		for (i = 0; i < listLengthDivThree; i++) {
			...
			if (wordThree.contains("."))
				wordThree = wordThree.replace("?", "\\?");
			try {
				Pattern wordToFind = Pattern
						.compile("((\\s|\\b)(" + wordOne + "\\s*" + wordTwo + "\\s*" + wordThree + ")(\\s|\\b)){1}+");
				Matcher theMatch = wordToFind.matcher(theDoc);
				theMatch.find();
				if (theMatch.group(2).matches("\\s"))
					start = theMatch.start() + 1;
				...
				Logger.logln("'" + wordOne + " " + wordTwo + "  " + wordThree
						+ "'  - was not matched. if there is more than 1 space or symbols exist within the single quotes, that may be why.");
			}
		}
		return theIndices;
