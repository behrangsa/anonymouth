...
public class TaggedSentence implements Comparable<TaggedSentence>, Serializable {
	...
	/*
	 * private TaggedSentence(int numWords, int numTranslations) { wordsInSentence =
	 * new ArrayList<Word>(numWords); translationNames = new
	 * ArrayList<String>(numTranslations); translations = new
	 * ArrayList<TaggedSentence>(numTranslations); }
	 */
	...
	public TaggedSentence(String untagged) {
		sentenceLevelFeaturesFound = new SparseReferences(10); // probably won't find more than 10 features in the
																// sentence.
	...
	/**
	 * Constructor for TaggedSentence. Essentially does a deep copy of the input
	 * TaggedSentence
	 * 
	 * @param ts
	 */
	public TaggedSentence(TaggedSentence ts) {
		...
		// Then copy the translation information (language names and translations).
		// Since translationNames is an ArrayList of Strings -- and strings are
		// immutable -- we can just do the equal thing
	...
	/**
	 * Gets the translation names for each corresponding translation for this tagged
	 * sentence.
	 * 
	 * @return
	 */
	...
	public void sortTranslations() {
		...
		double[][] toSort = new double[translations.size()][2]; // [Anonymity Index][index of specific translation] =>
																// will sort by col 1 (AI)
		int i;
		for (i = 0; i < numTranslations; i++) {
		...
		translations = sortedTrans; // set translations to be the same list of translated sentences, but now in
									// order of Anonymity Index
		translationNames = sortedTranNames; // set translations to be the same list of translated sentences, but now in
											// order of Anonymity Index
	}

	/**
	 * Returns true if the translations ArrayList for this tagged sentence has a
	 * size > 0, false if size == 0
	 * 
	 * @return
	 */
	...
	/**
	 * Tags the untagged sentence in this TaggedSentence, and finds the features
	 * present in it. For use when <i>not</i> storing TaggedSentence in a
	 * TaggedDocument
	 */
	 ...
	 * 
	 * @param tagged
	 *            the tagged sentence as output by the Standford POS Tagger
	 * @return
	 */
	...
	/**
	 * Retrieves all Reference objects associated with each word in the sentence,
	 * and merges them into a single instance of SparseReferences
	 * 
	 * @return
	 */
	public SparseReferences getReferences() {
		int numWords = this.size();
		SparseReferences allRefs = new SparseReferences((numWords * 5) + sentenceLevelFeaturesFound.length()); // just a
																												// guess
																												// - I
																												// don't
																												// think
	...
	/**
	 * Clears all translations and translations names associated with the
	 * TaggedSentence instance.
	 */
	...
	/**
	 * Clears all variables tied to this specific taggedSentence before finally
	...
	/**
	 * returns a SparseReference object containing the index of each attribute who's
	 * value needs to be updated, along with the amount it must be changed by (if
	 * positive, the present value should increase, if negative, it should decrease.
	 * Therefore, you only need to add the 'value' of each Reference to the
	 * 'index'th Attribute's presentValue in the Attribute[] array.
	 * 
	 * note: the reason this is done at the sentence level rather than the document
	 * level, is that users will generally only edit one sentence at a time; so only
	 * that part of the document can change.
	 * 
	 * @param oldOne
	 * @return
	 */
	...
	/**
	 * Computes the AnonymityIndex of this sentence: SUM
	 * ((#appearancesOfFeature[i]/numFeaturesFoundInWord)*(infoGainOfFeature[i])*(%changeNeededOfFeature[i])).
	 * 
	 * This is only done directly for sentence level features (word bigrams,
	 * trigrams, punctuation, etc.)
	 * 
	 * The individual anonymity indices of all words within the sentence are also
	 * added to the sum computed above, which is what gets returned
	 * 
	...
	public double getSentenceAnonymityIndex() {
		...
		// totalFeatureOccurrences +=
		// sentenceLevelFeaturesFound.references.get(i).value;
	 ...
	 * if(wordListMap.containsKey(str)){ word.adjustVals(0,
	 * wordListMap.get(str).infoGainSum,wordListMap.get(str).percentChangeNeededSum)
	 * ;//check on this wordListMap.put(str,word); } else { wordListMap.put(str,
	 * word); } }
	 */
	 ...
	 * @param tagged
	 * 
	 * public void setGrammarStats(){ //setwordListMap(); FunctionWord fWord=new
	 * FunctionWord(); MisspelledWords mWord=new MisspelledWords(); for (int
	 * twCount=0;twCount<tagged.size();twCount++){ TaggedWord
	 * temp=tagged.get(twCount); //System.out.println(temp.tag());
	 * if(temp.word().matches("[\\w&&\\D]+")){//fixes the error with sentenceAppend
	 * button if(fWord.searchListFor(temp.word())){
	 * //functionWords.add(temp.word()); } else
	 * if(mWord.searchListFor(temp.word())){ misspelledWords.add(temp.word()); }
	 * java.util.regex.Matcher wordToSearch=punctuationRegex.matcher(temp.word());
	 * if(wordToSearch.find()){
	 * punctuation.add(temp.word().substring(wordToSearch.start(),
	 * wordToSearch.end())); } //adds digits
	 * wordToSearch=digit.matcher(temp.word()); if(wordToSearch.find()){ String
	 * digitSubStr=temp.word().substring(wordToSearch.start(), wordToSearch.end());
	 * for (int count=0;count<digitSubStr.length();count++){ if(count-2>=0){
	 * digits.add(digitSubStr.substring(count-2, count));
	 * digits.add(digitSubStr.substring(count-1, count)); } else if(count-1>=0){
	 * digits.add(digitSubStr.substring(count-1, count)); }//not sure if
	 * necessary...digits bi/trigrams digits.add(digitSubStr.substring(count,
	 * count)); } } wordToSearch=specialCharsRegex.matcher(temp.word());
	 * if(wordToSearch.find()){
	 * specialChars.add(temp.word().substring(wordToSearch.start(),
	 * wordToSearch.end())); }
	 * 
	 * wordLengths.add(temp.word().length()); //setHashMap(POS,temp.tag());
	 * //setHashMap(words,temp.word());
	 * 
	 * /*if(twCount-2>=0){//addsTrigrams&Bigrams
	 * setHashMap(POSTrigrams,tagged.get(twCount-2).tag()+tagged.get(twCount-1).tag(
	 * )+tagged.get(twCount).tag());
	 * setHashMap(wordTrigrams,tagged.get(twCount-2).word()+tagged.get(twCount-1).
	 * word()+tagged.get(twCount).word());
	 * setHashMap(POSBigrams,tagged.get(twCount-1).tag()+tagged.get(twCount).tag());
	 * //I feel that doing it this way with if/elif would speed up code
	 * setHashMap(wordBigrams,tagged.get(twCount-1).word()+tagged.get(twCount).word(
	 * )); } else if(twCount-1>=0){//addsBigrams
	 * setHashMap(POSBigrams,tagged.get(twCount-1).tag()+tagged.get(twCount).tag());
	 * setHashMap(wordBigrams,tagged.get(twCount-1).word()+tagged.get(twCount).word(
	 * )); } char[] untaggedWord=temp.word().toLowerCase().toCharArray(); for(int
	 * letterIndex=0;letterIndex<untaggedWord.length;letterIndex++){
	 * setHashMap(letters,untaggedWord[letterIndex]+""); if(letterIndex-2>=0){
	 * setHashMap(letterBigrams,untaggedWord[letterIndex-1]+untaggedWord[letterIndex
	 * ]+""); setHashMap(letterTrigrams,untaggedWord[letterIndex-2]+untaggedWord[
	 * letterIndex-1]+untaggedWord[letterIndex]+""); } else if(letterIndex-1>=0){
	 * setHashMap(letterBigrams,untaggedWord[letterIndex-1]+untaggedWord[letterIndex
	 * ]+""); } }
	 * 
	 * 
	 ...
	 * if(hashMap.containsKey(key)){ hashMap.put(key,
	 * (hashMap.get(key).intValue()+1)); } else { hashMap.put(key, 1); } }
	 */
	...
	public String toString() {
		return "[ untagged: <" + untagged + "> ||| tagged: " + wordsInSentence.toString()
				+ " ||| SparseReferences Object: " + getReferences().toString() + " ]";
		// ||| tense: "+tense.toString()+" ||| point of view: "+pointOfView.toString()+"
		// conjugation(s): "+conj.toString()+" ]";// ||| functionWords :
		// "+functionWords.toString()+" ]";
	}

	/*
	 * TODO: 'tagged' no longer holds tagged words. public ArrayList<String>
	 * getWordsWithTag(TheTags tag){ wordsToReturn = new
	 * ArrayList<String>(tagged.size());// Can't return more words than were tagged
	 * tagIter = tagged.iterator(); while (tagIter.hasNext()){ taggedWord =
	 * tagIter.next(); System.out.println(taggedWord.value());
	 * System.out.println(taggedWord.tag()); } return wordsToReturn; }
	 */

}

/*
 * Stuff for tenses if(temp.tag().startsWith("VB")){ //it is a verb
 * switch(TheTags.valueOf((temp.tag()))){ case VB:
 * conj.add(CONJ.SIMPLE);//"Verb, base form"; case VBD: tense.add(TENSE.PAST);
 * conj.add(CONJ.SIMPLE); // "Verb, past tense"; //case "VBG": //
 * "Verb, gerund or present participle"; //case "VBN": //
 * "Verb, past participle"; case VBP: tense.add(TENSE.PRESENT);//
 * "Verb, non-3rd person singular present"; case VBZ:
 * tense.add(TENSE.PRESENT);// "Verb, 3rd person singular present"; } } else if
 * (temp.tag().startsWith("PR")){//this is a pronoun String
 * tempWord=temp.word(); for(int j=0;j<firstPersonPronouns.length;j++){
 * if(firstPersonPronouns[j].equalsIgnoreCase(tempWord)){
 * if(!pointOfView.contains(POV.FIRST_PERSON))//will not add POVs twice
 * pointOfView.add(POV.FIRST_PERSON); } } for(int
 * j=0;j<secondPersonPronouns.length;j++){
 * if(secondPersonPronouns[j].equalsIgnoreCase(tempWord)){
 * if(!pointOfView.contains(POV.SECOND_PERSON))
 * pointOfView.add(POV.SECOND_PERSON); } } for(int
 * j=0;j<thirdPersonPronouns.length;j++){
 * if(thirdPersonPronouns[j].equalsIgnoreCase(tempWord)){
 * if(!pointOfView.contains(POV.THIRD_PERSON))
 * pointOfView.add(POV.THIRD_PERSON); } } } /*else
 * if(temp.word().equalsIgnoreCase("shall")||temp.word().equalsIgnoreCase("will"
 * )){ tense.add(TENSE.FUTURE); }actually, this is not necessarily true.
 */