...
public class DocumentTagger {
	...
	public ArrayList<TaggedDocument> tagDocs(List<Document> docs, boolean loadIfExists) throws Exception {
		...
		for (Document d : docs) {
			 ...
			 * == true && loadIfExists){ td =
			 * ObjectIO.readTaggedDocument(docTitle+"_"+currentAuthor,
			 * ThePresident.GRAMMAR_DIR, false); } else{
			 */
			d.load();
			fullDoc = d.stringify();// .replaceAll("\\p{C}"," ");// get rid of unicode control chars (causes parse
									// errors).
