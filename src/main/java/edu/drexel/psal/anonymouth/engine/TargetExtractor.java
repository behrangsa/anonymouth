...
public class TargetExtractor {
	 ...
	 *
	 * @param numAuthors
	 *            the number of authors (not including the user). This defines the
	 *            starting number of centroids
	 * @param attrib
	 *            the Attribute to extract a target for
	 */
	...
	public void kPlusPlusPrep() { // double[] thisFeature){ // parameter just for testing
		...
		for (i = 0; i < numMeans - 1; i++) {
			...
			if (dSquaredRaySum == 0) { // this will occur if we have multiple duplicate values. In that case, we can't
				// choose more centroids because the remaining data points are equal.
				maxCentroidsFound = true;
				numMeans = i + 1; // need to add one because it starts counting from '0', and even if every
				// document has the same value for this feature, there
				// will still be one centroid chosen because we randomly select the first one
				// before the
				// loop (if ALL values are the same, this will break out of the loop at i==0,
				// and we'll have
				// a single centroid, so, we need to account for it in numMeans)
				break;
			}
			for (k = 0; k < numFeatures; k++)
	...
	/**
	 * Initializes the clustering algorithm by evenly spacing 'numMeans' centroids
	 * between the features [min,max], and assigns features to partitions based upon
	 * Euclidean distance from centroids (single dimension)
	 */
	public boolean initialize() {
		...
		for (i = 0; i < differences[0].length; i++) { // differences array's columns (correspond to 'thisFeature'
														// indices (feature events
			// per document)
			j = 0;
			temp[0] = j;
			temp[1] = differences[j][i];
			for (j = 1; j < differences.length; j++) { // differences array's rows (correspond to 'thisFeaturesClusters'
														// cluster indices)
		...
		// Logger.logln(NAME+"Initial positions for elements found. Updating
		// Centroids.");
		return updateCentroids();
	}

	/**
	 * Updates the centroids to be the average of the values contained within their
	...
	public boolean reOrganize() {
		...
		// need to go through all elements, extract data, and check distance agains new
		// centroids
		// create list of all centroids
		int i;
		int j;
		int k;
		int m;
		double[] temp = new double[2]; // index '0' holds the centroid number that corresponds to the difference value
										// in
		// index '1'
		int bestCentroid;
		boolean movedElement = false;
		// System.out.println("all centroids: "+getAllCentroids().toString());
		// TODO: maybe there is a better way to go about this than casting from Object
		// to Double
		double[] allCentroids = getAllCentroids();
		double[] diffs = new double[allCentroids.length];
		Pair[] elementHolder;
		for (i = 0; i < numMeans; i++) { // for each cluster
			elementHolder = thisFeaturesClusters.get(i).getElements(); // get the element list, and change it to a
																		// Double[] (from
			// ArrayList<Double>)
	...
	/**
	 * returns the average absolute deviation of the elements in the target cluster
	 * from the centroid
	 *
	 * @return
	 */
	...
	/**
	 * Method that runs the modified k-means clustering algorithm, initialized via
	...
	public void aMeansCluster() { // a-means-cluster vs k-means-cluster
		...
		for (i = 0; i < numMeans; i++) {
			...
			if ((thisOne.getElements().length == 0) && (thisOne.getCentroid() == -1)) { // this may be redundant.. we
																						// can probably just pick one
																						// (I'd keep the first
				// one)
				thisFeaturesClusters.remove(thisOne); // no reason keeping empty clusters around
				numMeans--; // if we remove a cluster, we need to reduce the number of means, AND we need to
				// decrement 'i' so that we don't miss the cluster that moves forward into the
				// place of the cluster that was just deleted.
				i--;
			}
		}
		...
		while (clusterIter.hasNext()) {
			...
			Logger.logln(
					NAME + "Cluster " + clusterNumber + " has its centroid at " + thisOne.getCentroid() + " and has "
							+ thisOne.getElements().length + " elements. They are: " + holderForLogger.toString());
			clusterNumber += 1;
		}
	 ...
	 *
	 * <p>
	 * preference = (number of elements in cluster)*(positive distance between
	 * cluster centroid and user's average)
	 *
	 * @return
	 */
