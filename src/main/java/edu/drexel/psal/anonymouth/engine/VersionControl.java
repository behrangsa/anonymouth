...
public class VersionControl {
	...
	/**
	 * The absolute max allowed stack size for ether undo or redo.<br>
	 * <br>
	 * TODO: This could use some testing, we're not sure how large a given
	 * TaggedDocument is. If it turns out it doesn't use up that much space, then we
	 * can most certainly increase this cap.
	 */
	...
	/**
	 * Should be called after processing to set up the initial backups and prepare
	 * undo/redo. Can't be handled by the constructor since we don't yet have tagged
	 * documents ready when that happens, we have to wait.
	 */
	public void init() {
		/*
		 * undo.clear(); redo.clear(); indicesUndo.clear(); indicesRedo.clear();
		 * main.enableRedo(false); main.enableUndo(false);
		 */
	...
	/**
	 * Returns whether or not the undo or redo threads are done and ready for
	 * another action
	 *
	 * @return True or false, depending on whether or not it's ready
	 */
	...
	/**
	 * Automatically handles whether or not it's appropriate to add a copy of the
	 * current taggedDoc to the undo stack, simply call this every caretListener
	 * update from editorDriver
	 *
	 * @param taggedDoc
	 ...
	 * @param forceBackup
	 *            Whether or not to bypass and reset the curCharBackupBuffer and
	 *            immediately backup
	 */
	...
	/**
	 * Adds a version to the undo stack, should only be called by updateUndoRedo()
	...
	/**
	 * Should be called whenever you want a undo action to occur (so in Undo
	 * listeners)
	 *
	 * <p>
	 * Swaps the current taggedDoc in DriverEditor with the version on the top of
	 * the undo stack, updates the document text pane with the new taggedDoc, and
	 * pushed the taggedDoc that was just on the undo stack to the redo one.
	 */
	public void undo() {
	...
	/**
	 * Should be called whenever you want a redo action to occur (so in Redo
	 * listeners)
	 *
	 * <p>
	 * Swaps the current taggedDoc in DriverEditor with the version on the top of
	 * the redo stack, updates the document text pane with the new taggedDoc, and
	 * pushed the taggedDoc that was just on the redo stack to the undo one.
	 */
	public void redo() {
