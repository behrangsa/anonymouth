...
/**
 * Swaps original feature values out of the original feature vector of the
 * documen to anonymize, and replaces them with potential target values of the
 * feature in question. All of these swaps are tested against the trained
 * classifier, and the ClusterGroup with target values that confused the
 * classifier the most is returned.
 *
 * <p>
 * "Confused the classifier the most" means that the "other" authors (not the
 * user) had ownership estimates with the lowest variance with respect to a
 * single test instance, and that the user had a (low, less than random
 * chance..?)
 *
...
public class FeatureSwapper {
	...
	/**
	 * Will test the topN_ClusterGroupsToTest by swapping the centroid (target)
	 * values into the document to anonymize's Instance object, and testing the
	 * modified Instances against the original classifier.
	 *
	 * <p>
	 * It will return the best ClusterGroup (which is the one that would provide the
	 * most anonymity).
	 *
	 * <p>
	 * If "topN_ClusterGroupsToTest" is less than 1, or greater than the number of
	 * ClusterGroups available, all ClusterGroups will be tested.
	 *
	...
	public ClusterGroup getBestClusterGroup(int topN_ClusterGroupsToTest) {
		...
		for (i = 0; i < numClusterGroups; i++) { // for each cluster group
			...
			alteredInstance = hopefullyAnonymizedInstances.instance(0); // hardcoding in '0' because we only plan to
																		// have a single document (thus a single
			// instance)
			hopefullyAnonymizedInstances.delete(0); // deleting the original from the copy -- it will be replaced by an
													// Instance with
			// substituted values
			tempCG_centroids = clusterGroups[i].getCentroids(); // these are the "target values" that will be
																// substituted in
			for (j = 0; j < numTopFeatures; j++) {
				// the attributes (features) in topAttributes are in the same order as in each
				// ClusterGroup
				// (clusterGroups[i]),
				// AND in the same order as the centroids in the array returned by
				// "clusterGroups[i].getCentroids()" above.
		...
		/*
		 * Error check to make sure that Weka isn't being misused: if ALL of the
		 * representativeValue's are equal, then something is wrong.. because that means
		 * that all of the classifications were identical.
		 */
		...
		for (i = 1; i < numVals; i++) {
			...
			// "+wekaResultsArray[i].representativeValue+" ==>
			// "+clusterGroups[wekaResultsArray[i].respectiveIndexInClusterGroupArray].toString());
		}
		Logger.logln(
				NAME + "If all features are moved to their target values, the following classification will result: "
						+ clusterGroups[wekaResultsArray[0].respectiveIndexInClusterGroupArray].toString());
		if (allEqual && i != 1)
			Logger.logln(NAME
					+ "Oops! Weka must have been called incorrectly. All ClusterGroup representativeValue's are the same!\nThis is known to happen when the SMO is run without the '-M' flag.",
					LogOut.STDERR);
...
/**
 * Takes care of parsing the results from Weka, and calculates a representative
 * value that indicates how favorable a given result is, with respect to the
 * other results. Implements Comparable to allow for easy sorting via
 * Arrays.sort(array).
 *
 * @author Andrew W.E. McDonald
 */
class WekaResults implements Comparable<WekaResults> {
	...
	/**
	 * Calculates the variance among ONLY the "other" authors (not the actual
	 * author). (we want to pick the result with the lowest variance (meaning it was
	 * hard to tell who wrote the document)
	 */
	...
	/**
	 * This calculates the average between the variance of the other author's
	 * results (the degree of certainty that each other author wrote the document in
	 * question), and the degree of certainty that the user (who did write the
	 * document) wrote the document in question. This is done because it seemed to
	 * be the best way to factor in a low variance between other authors (meaning
	 * it's hard to tell if any of the other authors wrote the document), and the
	 * fact that we also want it to look as different from the author's writing as
	 * possible.
	 *
	 * <p>
	 * Taking the average of these two numbers means that the ClusterGroup that we
	 * select will have a value as close to zero as possible returned from this
	 * function.
	 *
	 * @return
	 */
...
/**
 * Small wrapper for the name and degree of certaintly returned by Weka, as a
