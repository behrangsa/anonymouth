...
public class InstanceConstructor {
	 ...
	 *
	 * @return String array containing attributes of training documents, each index
	 *         of array holds one attribute, with indices corresponding to indices
	 *         of String[] returned by @getTrainingInstances .
	 */
	 ...
	 *
	 * @return double array containing instances of training documents.
	 */
	...
	/**
	 * Constructor for InstanceConstructor, accepts boolean variable that tells
	 * WekaInstancesBuilder whether to expect sparse data or not. (if unsure, set
	 * false)
	 *
	 * @param isSparse
	 *            - boolean, true if expecting sparse data, false otherwise or if
	 *            unsure.
	 * @param cfd
	 *            - cumulative feature driver. Contains all features that will be
	...
	/**
	 * method runInstanceBuilder uses an instance of WekaInstancesBuilder to extract
	 * the features of both the input trainDocs and testDoc(s).
	 *
	 * @param trainDocs
	 *            list of Document objects to train the Weka classifier on
	 * @param testDocs
	 *            list (may be a single object list) of Document object(s) to
	 *            classify.
	...
	public boolean runInstanceBuilder(List<Document> trainDocs, List<Document> testDocs) {
		...
		// Initialize two new instances to hold training and testing instances
		// (attributes and data)
	...
	/**
	 * returns full set of training data in arff style formatting (contains list of
	 * attributes and data)
	 *
	 * @return Instances object containing training data
	 */
	...
	/**
	 * returns full set of test data in arff style formatting (contains list of
	 * attributes and data)
	 *
	 * @return Instances object containing testing data
	 */
	...
	/**
	 * Accepts Weka Instances object and returns the stripped attributes. Stripping
	 * performed by 'AttributeStripper'
	 *
	 * @param currentInstance
	 *            - Weka Instances object (arff format)
	 * @return
	 */
	...
	/**
	 * Accepts JSylo's Instances object and returns the instances (@data) portion of
	 * the ".arff file" (not really a file at this point though).
	 *
	 * @param currentInstance
	 *            - JStylo's Instances object (arff format)
	 * @return
	 */
	public Double[][] getInstances(Instances currentInstance) {
		...
		for (i = 0; i < numInstances; i++) {
			...
			while (!tempString.equals("")) {
				...
				if (j == skip) {
					tempInstance[i][j] = null; // set author name/ID to null rather than simply omit - allows attribute
												// lists
					// indices to stay synchronized
				} else {
