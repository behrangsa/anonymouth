...
public class ClusterAnalyzer {
	...
	public ClusterAnalyzer(ArrayList<String> featuresToUse, int maxClusters) {
		...
		// int num_docs_not_processed_by_jstylo = numDocs %
		// ThePresident.NUM_TAGGING_THREADS; // XXX
		// XXX XXX XXX XXX WE ONLY NEED THIS UNTIL JSTYLO FIXES ITS THREAD DIVISION
		// ISSUE
		// numDocs -= num_docs_not_processed_by_jstylo; // XXX XXX XXX XXX XXX XXX XXX
		// SEE COMMENT ABOVE
	...
	/**
	 * The clusters in the input Attribute are placed into their respective places
	 * in a 3d array based on feature, document, and Cluster number.
	 *
	 * @param attrib
	...
	public boolean addFeature(Attribute attrib) {
		...
		// attrib.getConcatGenNameAndStrInBraces());
		...
		for (i = 0; i < lenClusterRay; i++) {
			...
			for (j = 0; j < lenPairRay; j++) {
				...
				// the ordered clusters are ordered such that cluster '0' is at the beginning of
				// the array
				// of Clusters
				// we add one so that ...
			...
			// So, now clustersByDoc is an array with each row containing all cluster
			// numbers of all
			// features in a given document.
	...
	/**
	 * Runs analysis of the ClusterGroups, and orders them such that the
	 * ClusterGroup(s) most likely to be make the user anonymous, are at the top of
	...
	public void analyzeNow() {
		...
		int i, j, k;
		commonClusterSetMap = new HashMap<SmartIntegerArray, Integer>(theDocs.size()); // worst case, no two documents
																						// fall in same set of clusters
		i = 0;
		j = 0;
		k = 0;
		...
		for (i = 0; i < numDocs; i++) {
			...
			for (j = 0; j < numClusters; j++) {
				// System.out.println("document "+i+", cluster number "+j+" =>
				// "+clustersByDoc[i][j]);
		...
		while (csmkIter.hasNext()) {
			...
			for (i = 0; i < lenKey; i++) {
				// this allows feature importance (as determined by information gain), and
				// cluster
				// preference to influence the ordering of the cluster groups.
	...
	/**
	 * returns an array of ClusterGroups. Should only be called after
	 * {@link #analyzeNow()}
	 *
	 * @return
	 */
