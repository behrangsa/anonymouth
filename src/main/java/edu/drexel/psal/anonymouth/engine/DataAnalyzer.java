...
/**
 * Performs general calculations relating to analyzing the data, including (but
 * not limited to): averages, standard deviations, feature information gain, and
 * controlling the clustering and cluster analytics.
 *
...
public class DataAnalyzer {
	 ...
	 *            integer number corresponding to the top 'numFeaturesToReturn'
	 *            features
	 */
	...
	/**
	 * returns the array of top Attributes (features with the highest information
	 * gain)
	 *
	 * @return
	 */
	...
	/**
	 * takes average of each feature across all instances specified by
	 * "instancesArray"
	 */
	...
	/**
	 * finds the minimum and maximum values of each feature across all instances
	 * given
	 */
	...
	/**
	 * Finds the top 'numFeatures' after information gain has been calculated for
	 * them all
	 *
	 * @param theArffFile
	 *            the instances
	 * @param numFeatures
	 *            the number of features to return (starts from highest information
	 *            gain)
	 * @return Array of Attribute objects, one Attribute per feature
	 * @throws Exception
	 */
	public Attribute[] findMostInfluentialEvents(Instances theArffFile, int numFeatures) throws Exception {
		...
		if (numFeatures < numAttributes)
			...
			numAvailableFeatures = numAttributes;
		...
		for (i = 0; i < numAvailableFeatures; i++) {
			...
			Logger.logln(NAME + topAttribs[j].getFullName() + " info gain for this feature is: "
					+ topAttribs[j].getInfoGain() + ", calcHist is: " + topAttribs[j].getCalcHist()
					+ " string in the braces (if applicable): " + topAttribs[j].getStringInBraces()
					+ " toModify value is: " + topAttribs[j].getToModifyValue());
			j++;
		}
	...
	/**
	 * computes the information gain from the top 'numFeaturesToReturn' number of
	 * features specified by the constructor
	 *
	 * @param presentSet
	 *            - an unmodified Instances object to compute information gain from
	 * @return Array of Attribute objects, one Attribute for each feature
	 * @throws Exception
	 */
	public Attribute[] computeInfoGain(Instances presentSet) throws Exception {
		...
		// Extract the top "n" important features and place them into an array in order
		// of info gain
		// (greatest -> least). This happens per instance to allow for std. dev.
		// calculations.
		...
		for (i = 0; i < numAttribs; i++) {
			...
			if (trainAttribs.contains(tempAttrib) == true) {
				...
				trainOk = false;
				tempTrainIndex = 0; // basically re-setting... this wont be used in this case until the next
									// iteration.
			}
			...
			for (j = 0; j < numTrainInstances; j++) {
				if (trainOk == true)
					relevantTrainFeats[i][j] = trainingInstancesArray[j][tempTrainIndex]; // transpose and filter the
																							// instancesArray
				else
		...
		for (i = 0; i < topAttribs.length; i++) {
			if (topAttribs[i].getStringInBraces().equals(""))
				...
				theSpacer = " ";
			importantAttribs[i] = topAttribs[i].getGenericName().toString() + theSpacer
					+ topAttribs[i].getStringInBraces();
		...
		Logger.logln(NAME + "****** Current list of Present values for: " + ThePresident.sessionName
				+ " process request number: " + DocumentMagician.numProcessRequests + " ******");
		Logger.logln(NAME + holderForLogger.entrySet().toString());
		return topAttribs;
	}

	/**
	 * runs clustering algorithm on all features, and saves information in each
	 * feature's 'Attribute' object.
	 *
	 * @return number of maximum clusters found, to be used with @ClusterAnalyzer
	 */
	...
	/**
	 * runs the @ClusterAnalyzer . Note that this method alone does <i>not</i>
	 * modify the clusters or the preference ordering. it simply analyzes the
	 * existing ones.
	 *
	 * @param maxClusters
	 *            the maximum number of clusters out of all of the features
	 */
	...
	public void setSelectedTargets() {
		...
		for (i = 0; i < lengthTopAttributes; i++) {
			clusterNumber = clusterNumbers[i] - 1; // From ClusterAnalyzer, all cluster numbers are "1" greater than
													// their indices
			// (to ease computation)
			...
			topAttributes[i].setRangeForTarget(tempCluster.getMinValue(), tempCluster.getMaxValue()); // maybe this
																										// should be
																										// changed to
																										// avg. avs.
																										// dev.
	 ...
	 * @throws Exception
	 */
	public void runInitial(DocumentMagician magician, CumulativeFeatureDriver cfd, Classifier classifier)
			throws Exception {
