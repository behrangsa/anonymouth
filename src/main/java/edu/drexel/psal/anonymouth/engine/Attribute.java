...
/**
 * The Attribute class holds all information for each of the top 'n' features as
 * determined by the result of the information gain calculation.
 *
...
public class Attribute {
	...
	/**
	 * this is only set IF calcHist == true. In that case, the actual 'feature' is
	 * the string in the braces that replaces the dash: "{-}" => "{'string'}"
	 */
	 ...
	 *
	 * @param indexNumberInAttributeArray
	 *            the attributes index number (this may not be used anymore, though
	 *            it is still being set)
	 * @param fullName
	 *            the full attribute name taken directly from the Instances object
	 *            (@attribute...)
	 * @param calcHist
	 *            boolean, true if histogram was calculated (in full name, {-} will
	 *            be replaced by a string if this is true / should be set as true)
	 */
	...
	/**
	 * Strips the fullName to an enumerated generic name. This name is used to call
	 * suggestors and calculators by class name.
	 */
	public void setGenericName() {
		// manipulate fullName to match the feature's value in it's FeatureList
		// representation. calcHist
		// is a big factor in how this is done.
		trimmedAttrib = fullName.substring(fullName.indexOf("'") + 1, fullName.indexOf("{")); // saves ONLY the actual
																								// feature name
	...
	/**
	 * returns the index number of the feature represented by this attribute in the
	 * original Instance from the documentToAnonymize's features
	 *
	...
	/**
	 * returns the index number of this attribute in the Attribute array it is being
	 * held in
	 *
	 * @return the index number
	 */
	...
	/**
	 * Primarily used for @ClusterAnalyzer, this returns the concatenation of the
	 * generic name as a String with the string in braces. If there is no string in
	 * braces, the generic name is simply returned as a String.
	 *
	 * @return
	 */
	...
	/**
	 * sets the information gain for this attribute; relative to the rest of the
	 * attributes in the instances object.
	 *
	 * @param infoGain
	 */
	...
	/**
	 * Sets the clusters created with 'TargetExtractor' using all instances of this
	 * attribute
	 *
	 * @param clusters
	 */
	...
	/**
	 * sets the Cluster Array of ordered clusters for the feature defined by this
	 * Attribute
	 *
	...
	/**
	 * Sets this attribute's target centroid's elements' average absolute deviation
	 * from the targetCentroid
	 *
	...
	/**
	 * returns this attribute's target centroid's elements' average absolute
	 * deviation from the targetCentroid
	 *
	 * @return
	 */
	...
	/**
	 * Sets the values of all sample (not including user's) documents for this
	 * attribute
	 *
	 * @param trainVals
	 */
	...
	/**
	 * Sets the "present value", or the value of the document to modify for this
	 * attribute
	 *
	 * @param toModifyValue
	 */
	...
	/**
	 * returns the 'present value', or value of the document to modify for this
	 * attribute
	 *
	 * @return
	 */
	...
	/**
	 * returns true if the feature driver for this attribute returns true for
	 * 'isCalcHist"
	 *
	 * @return calcHist
	 */
	...
	/**
	 * If 'calcHist' is true, this will return the value in the braces of the
	 * 'attribute' (Weka). (e.g. a character bigram feature may contain 'ab' in the
	 * braces)
	 *
	 * @return
	 */
	...
	/**
	 * returns the full name of the attribute, as taken from Weka's Instances
	 * object.
	 *
	 * @return
	 */
	...
	/**
	 * returns something halfway between the full name and generic name of the
	 * attribute.
	 *
	 * @return
	 */
	...
	/**
	 * returns the percent change needed for the feature contained by this
	 * Attribute. Signed, so a negative number indicates the feature needs to be
	 * removed, and vice versa. Percent change needed is calculated at the time of
	 * function call, so the returned value will always be current.
	 *
	 * @param normalizeToBaseline
	 *            if true, method will return the percent change that is needed
	 *            normalized to the percent change that was needed initially (so,
	 *            100% is the greatest change you should see). if false, method will
	 *            return the absolute percent change needed, which does not
	 *            normalize to the baseline (meaning if you have 5 'a' characters,
	 *            and you need 15, then it would return 200%).
	...
	public double getPercentChangeNeeded(boolean normalizeToBaseline, boolean invertPercentage, boolean takeAbsVal) {
		...
		if (toModifyValue != 0) {
			...
			if ((temp * requiredDirectionOfChange < 0) && (Math.abs(temp) < minimumPercentChangeUnit)) // if the
																										// required
																										// direction of
																										// change is not
																										// the same
				// sign (or 0) as temp,
				// and the percent change needed is less than a minimumPercentChangeUnit, (i.e.,
				// if it
				// wants to move you backward past the target value after having already passed
				// it), temp =
				// 0
				temp = 0;
			else if ((theModulus / 2) < halfOfMin) // otherwise, if percent change needed is less than halfway between a
				// minimumPercentChangeUnit,
				temp = temp - theModulus; // round down to the closest minimumPercentChangeUnit
			else // otherwise, if percent change needed is greater than or exactly halfway
					// between a
					// minimumPercentChangeUnit,
				temp = temp + (minimumPercentChangeUnit - theModulus); // round up to the next highest
																		// minimumPercentChangeUnit

		} else {
			temp = Math.ceil(this.targetValue); // if value doesnt exist in document, set percent change needed to
			// the ceil value of the target value (e.g. add 5 occurrences of
			// 'if': 500%)
			// XXX NOTE: I am rounding this up because if the feature doesn't exist, and it
			// should be
			// present, it seems that it would be
			// fairly important to add. However, taking the actual percent change that it
			// would need is
	...
	/**
	 * checks the current value of this feature against the range of acceptable
	 * values as determined by the user's ClusterGroup selection
	 *
	 * @param currentValue
	 *            Attribute's current feature value
	 * @return a negative number if currentValue is too high (indicating how much it
	 *         needs to come down), positive number if currentValue is too low
	 *         (indicating how much it needs to go up), or '0' if the current value
