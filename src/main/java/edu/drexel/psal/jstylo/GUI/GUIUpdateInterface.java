...
public class GUIUpdateInterface {
	...
	protected static void showAbout(GUIMain main) {
		ImageIcon logo = new ImageIcon(
				Thread.currentThread().getClass().getResource(ANONConstants.GRAPHICS_PREFIX + "logo.png"),
				"JStylo Logo");
		String content = "<html><p>" + "<h3>JStylo</h3><br>" + "Version " + version + "<br>"
				+ "Authors: Ariel Stolerman and Travis Dutko<br>" + "Privacy, Security and Automation Lab (PSAL)<br>"
				+ "Drexel University<br>" + "<br>" + "Powered by: Weka, JGAAP" + "</p></html>";

		JOptionPane.showMessageDialog(main, content, "About JStylo", JOptionPane.INFORMATION_MESSAGE, logo);
	}
	...
	/**
	 * Creates a panel with parameters and their values for the given event driver /
	 * canonicizer / culler.
	 */
	protected static JPanel getParamPanel(Parameterizable p) {
		...
		for (Pair<String, ParamTag> param : params) {
			...
			if (temp.length() > 40) {
				String tmp = "<HTML>" + temp.substring(0, 24) + "<br>" + temp.substring(24, 48) + "<br>"
						+ temp.substring(48) + "</HTML>";
				temp = tmp;
	...
	/**
	 * Updates the list of selected classifiers with respect to the list of
	 * classifiers.
	 */
