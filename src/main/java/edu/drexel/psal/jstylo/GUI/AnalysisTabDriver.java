...
public class AnalysisTabDriver {
		...
		main.analysisRunJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (main.ps == null || main.ps.getAllTrainDocs().size() == 0) {
					...
					JOptionPane.showMessageDialog(main, "Feature set not set or has no features.", "Run Analysis Error",
							JOptionPane.ERROR_MESSAGE);
					return;
					...
					JOptionPane.showMessageDialog(main, "No classifiers added.", "Run Analysis Error",
							JOptionPane.ERROR_MESSAGE);
					return;
				} else if (main.analysisTrainCVJRadioButton.isSelected()) { // makes sure K and N are in the appropriate
																			// range
					int docCount = 0;
					try {
				...
				// if the number of calc threads entered is different then the current stored
				// one,
				// change it
	...
	/*
	 * ========================================== Main thread class for running the
	 * analysis ==========================================
	 */
	...
	public static class RunAnalysisThread implements Runnable {
		...
		public void run() {
			...
			Enumeration<DefaultMutableTreeNode> authors = ((DefaultMutableTreeNode) main.trainCorpusJTree.getModel()
					.getRoot()).children();
			DefaultMutableTreeNode author;
			while (authors.hasMoreElements()) {
				author = authors.nextElement();
			...
			InstancesBuilder tempBuilder = new InstancesBuilder(main.ps, main.cfd,
					main.analysisSparseInstancesJCheckBox.isSelected(), false, main.ib.getNumThreads());
			main.ib.reset();
			main.ib = tempBuilder;
			// training set
			...
			try {
				...
				JOptionPane.showMessageDialog(main, "Could not extract features from training corpus:\n"
			...
			try {
				...
				JOptionPane.showMessageDialog(main, "Could not create attributes from training corpus:\n"
						+ e1.getMessage() + "\n" + "Aborting analysis.", "Analysis Error", JOptionPane.ERROR_MESSAGE);
				updateBeforeStop();
				Thread.currentThread().stop();
			}
			...
			try {
				...
				JOptionPane.showMessageDialog(main, "Could not create instances from training corpus:\n"
						+ e.getMessage() + "\n" + "Aborting analysis.", "Analysis Error", JOptionPane.ERROR_MESSAGE);
				updateBeforeStop();
				Thread.currentThread().stop();
			}
			...
			if (main.analysisClassTestDocsJRadioButton.isSelected()) {
				...
				try {
					...
					JOptionPane.showMessageDialog(main, "Could not create instances from test documents:\n"
			...
			if (main.analysisCalcInfoGainJCheckBox.isSelected()) {
				...
				try {
					boolean apply = main.analysisCalcInfoGainJCheckBox.isSelected()
							&& main.analysisApplyInfoGainJCheckBox.isSelected();
					...
					for (int i = 0; i < infoGain.length; i++) {
						...
						content += String.format("> %-50s   %f\n",
								trainingInstances.attribute((int) infoGain[i][1]).name(), infoGain[i][0]);
					}
					updateResultsView();
					// content+=infoGain;
				} catch (Exception e) {
			...
			if (main.analysisClassTestDocsJRadioButton.isSelected()) {
				...
				for (int i = 0; i < numClass; i++) {
					...
					results = main.analysisDriver.classify(main.ib.getTrainingInstances(), main.ib.getTestInstances(),
					...
					// TODO have this be optional and toggle it via an option on the analysis tab.
					// Make sure
					// that there's some kind
					// of disclaimer since right now it only works if the author name is in the
					// document
					// title.
