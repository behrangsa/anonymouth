...
public class FeaturesTabDriver {
	...
	protected static void initListeners(final GUIMain main) {
		...
		main.featuresSetJComboBox.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				/*
				 * if (!isCFDEmpty(main.cfd)) { answer = JOptionPane.showConfirmDialog(main,
				 * "Are you sure you want to override current feature set?",
				 * "Load Preset Feature Set", JOptionPane.YES_NO_OPTION); }
				 */
		...
		main.featuresLoadSetFromFileJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				 ...
				 * if (!isCFDEmpty(main.cfd)) { answer = JOptionPane.showConfirmDialog(main,
				 * "Are you sure you want to override current feature set?",
				 * "Import Feature Set", JOptionPane.YES_NO_OPTION); }
				 */
		...
		main.featuresRemoveJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				int answer = JOptionPane.showConfirmDialog(main,
						...
						"Remove Feature", JOptionPane.YES_NO_OPTION);
				if (answer == JOptionPane.YES_OPTION) {
		...
		main.featuresCanonJList.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent e) {
				...
				// selected
					...
					Canonicizer c = main.cfd.featureDriverAt(main.featuresJList.getSelectedIndex())
							.canonicizerAt(selected);
	...
	protected static void initPresetCFDs(GUIMain main) {
		 ...
		 * Canonicizer stripEdgesPunct = new StripEdgesPunctuation(); Canonicizer
		 * unifyCase = new UnifyCase();
		 * 
		 * // unique words - number of unique words in the document (remove punctuation
		 * and unify case) FeatureDriver uniqueWords = new
		 * FeatureDriver("Unique Words Count", false, new
		 * UniqueWordsCounterEventDriver()); uniqueWords.
		 * setDescription("Number of unique words in the document, after removing punctuation and unifying case."
		 * ); uniqueWords.addCanonicizer(stripEdgesPunct);
		 * uniqueWords.addCanonicizer(unifyCase);
		 * nineFeatures.addFeatureDriver(uniqueWords);
		 * 
		 * // complexity - ratio of unique words to total number of words FeatureDriver
		 * complexity = new FeatureDriver("Complexity", false, new
		 * UniqueWordsCounterEventDriver()); complexity.
		 * setDescription("Ratio of unique words to total number of words in the document."
		 ...
		 * 
		 * // sentence count - number of sentences in the document FeatureDriver
		 * sentenceCount = new FeatureDriver("Sentence Count", false, new
		 * SentenceCounterEventDriver());
		 * sentenceCount.setDescription("Number of sentences in the document.");
		 * nineFeatures.addFeatureDriver(sentenceCount);
		 * 
		 * // average sentence length - in words (total number of words / total number
		 * of sentences) FeatureDriver avgSentenceLen = new
		 * FeatureDriver("Average Sentence Length", false, new
		 * WordCounterEventDriver()); avgSentenceLen.
		 * setDescription("Average sentence length in words (total number of words / total number of sentences)."
		 * ); avgSentenceLen.setNormBaseline(NormBaselineEnum.SENTENCES_IN_DOC);
		 * nineFeatures.addFeatureDriver(avgSentenceLen);
		 * 
		 * // average syllables - per word FeatureDriver avgSyllables = new
		 * FeatureDriver("Average Syllables in Word", false, new
		 * SyllableCounterEventDriver());
		 ...
		 * 
		 * // Gunning-Fog readability index FeatureDriver gfIndex = new
		 * FeatureDriver("Gunning-Fog Readability Index", false, new
		 * GunningFogIndexEventDriver()); gfIndex.
		 * setDescription("The Gunning-Fog readability index: 0.4*((total words / total sentences) + 100*(total complex words / total words)) [where complex words are words with 3 or more syllables]."
		 * ); nineFeatures.addFeatureDriver(gfIndex);
		 * 
		 * // character space - total number of characters, spaces included
		 * FeatureDriver charSpace = new FeatureDriver("Character Space", false, new
		 * CharCounterEventDriver()); charSpace.
		 * setDescription("The total number of characters in the document, spaces included."
		 * ); nineFeatures.addFeatureDriver(charSpace);
		 * 
		 * // letter space (no spaces / punctuation) FeatureDriver letterSpace = new
		 * FeatureDriver("Letter Space", false, new LetterCounterEventDriver());
		 * letterSpace.
		 * setDescription("The total number of letters (excluding spaces and punctuation)."
		 * ); nineFeatures.addFeatureDriver(letterSpace);
		 * 
		 * // Flesch reading ease score FeatureDriver fleschScore = new
		 * FeatureDriver("Flesch Reading Ease Score", false, new
		 * FleschReadingEaseScoreEventDriver()); fleschScore.
		 * setDescription("The Flesch reading ease score: 206.835 - 1.015*(total words / total sentences) -84.6*(total syllables / total words)"
		 * ); nineFeatures.addFeatureDriver(fleschScore);
		 * 
		 * main.presetCFDs.add(nineFeatures); /* /* ======================= WritePrints
		 * feature set =======================
		 */
	}
}
