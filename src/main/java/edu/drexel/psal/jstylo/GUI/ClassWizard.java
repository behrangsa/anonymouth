...
/**
 * Created in ClassTabDriver, this popup will allow for the editing of a
 * classifier's arguments, and will display basic information about that
...
public class ClassWizard extends javax.swing.JFrame {
	...
	/**
	 * ClassWizard does not have an empty/default constructor. It requires a parent
	 * and an Analyzer in order to function.
	 *
	 * @param parent
	 * @param tmpAnalyzer
	 *            the analyzer to be edited
	 */
	 ...
	 *
	 * @param currentOps
	 *            the current arg string
	 * @param optionsDesc
	 *            the descriptions and flags for all possible options the classifier
	 *            can take
	 */
	protected void initArgumentList(String[] currentOps, String[] optionsDesc) {
		ArrayList<Argument> argList = new ArrayList<Argument>();

		// Go over option flags, creating new Ags and adding them to the argList with
		// their descriptions
		// attached already
		...
		// iterate over the current options, adding the correct ones based on matching
		// the flags
		...
		// iterate over the argList. Any arg which does not yet have any value for the
		// option gets an
		// empty string
		for (Argument a : argList) {
		...
		try {
			setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
			{
...

				{
					...
					descriptionPanel.setBorder(BorderFactory.createCompoundBorder(defaultBorder,
							BorderFactory.createEmptyBorder(10, 10, 10, 10)));

					if (args == null || args.size() <= 5) { // if there are no options, make the description the main
															// focus of the
						// window
...

					{
						...
						descriptionJScrollPane = new JScrollPane(descriptionJTextArea,
					...
					}
					{
						...
						summaryJLabel.setText("<html><p>" + "<font size=12pt><b>Editing a Classifier</b></font><br>"
				...
				}
				{
					...
					optionsJScrollPane = new JScrollPane(optionsPanel, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
					...
					if (args != null && args.size() > 0) {
						...
						mainPanel.add(optionsPanel, BorderLayout.NORTH);
						{
							JLabel temp = new JLabel(
									"<html><font color=\"FF0000\">This analyzer has no options/arguments to edit.</color></html>");
	...
	/**
	 * A simple data concatenation class consisting of a flag, a description, and a
	 * value. <br>
