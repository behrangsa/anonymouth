	...
	protected static void initListeners(final GUIMain main) {
		...
		main.loadProblemSetJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// ask if current problem set is not empty
				if (main.ps != null && (main.ps.hasAuthors() || main.ps.hasTestDocs())) {
					answer = JOptionPane.showConfirmDialog(null, "Loading Problem Set will override current. Continue?",
							"Load Problem Set", JOptionPane.WARNING_MESSAGE, JOptionPane.YES_NO_CANCEL_OPTION);
				}
				if (answer == 0) {
		...
		main.testDocPreviewJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Logger.logln(
						"'Preview Document' button clicked under the 'Test Documents' section on the documents tab.");

				TreePath path = main.testDocsJTree.getSelectionPath();
				if (path == null || path.getPathCount() != 3) {
					JOptionPane.showMessageDialog(null, "You must select a test document in order to show its preview.",
					...
					if (answer == JFileChooser.APPROVE_OPTION) {
						...
						for (File dir : authors) { // for each directory selected
							for (File file : dir.listFiles()) { // for each file in that directory
								path = file.getPath().replaceAll("\\\\", "/");
								if (allTrainDocPaths.contains(path)) { // make sure that the file isn't already in the
																		// training set
									skipList += "\n" + path + " - already listed as a training document for author "
											+ dir.getName();
									continue;
						...
						if (!skipList.equals("")) { // if there was at least one file that was already present in one of
													// the
							// sets already, inform the user of which one it was
							if (skipList.length() < 3000) {
								JOptionPane.showMessageDialog(null, "Skipped the following documents:" + skipList,
										"Add Testing Documents", JOptionPane.WARNING_MESSAGE);
							} else {
		...
		main.addAuthorJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Logger.logln(
						"'Add Author...' button clicked under the 'Training Corpus' section on the documents tab.");
		...
		main.addTrainDocsJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Logger.logln(
						"'Add Document(s)...' button clicked under the 'Training Corpus' section on the documents tab.");
		...
		main.removeAuthorJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Logger.logln(
						"'Remove Author(s)' button clicked under the 'Training Corpus' section on the documents tab.");
		...
		main.removeTrainDocsJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Logger.logln(
						"'Remove Document(s)' button clicked under the 'Training Corpus' section on the documents tab.");
		...
		main.trainDocPreviewJButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				Logger.logln(
						"'Preview Document' button clicked under the 'Training Corpus' section on the documents tab.");
