...
public class ClassTabDriver {
	...
	protected static void initListeners(final GUIMain main) {
				...
				if (selectedNode.isLeaf()) {
					...
					try {
						...
						JOptionPane.showMessageDialog(main,
								...
								"Classifier Selection Error", JOptionPane.ERROR_MESSAGE);
						e.printStackTrace();
						return;
					}
				...
				if (tmpAnalyzer == null) {
					...
					try {
						...
						JOptionPane.showMessageDialog(main,
								...
								"Classifier Options Error", JOptionPane.ERROR_MESSAGE);
					...
					for (int i = 0; i < main.classJList.getModel().getSize(); i++) {
						...
						if (main.analyzers.get(i).getName().toString().equals((tmpAnalyzer.getName().toString()))) { // same
																														// classifier
							Logger.logln("already added: " + main.analyzers.get(i).getName().toString()
									+ " attempting to add: " + tmpAnalyzer.getName().toString());
							if (Arrays.equals(main.analyzers.get(i).getOptions(), (tmpAnalyzer.getOptions()))) { // same
																													// arguments
		...
		main.classJList.addListSelectionListener(new ListSelectionListener() {
			...
			public void valueChanged(ListSelectionEvent arg0) {
				...
				Logger.logln(
						"Classifier selected in the selected classifiers list in the classifiers tab: " + className);
	...
	protected static void initWekaClassifiersTree(GUIMain main) {
		...
		for (String className : loadedClasses) {
			...
			for (int i = 0; i < nameArr.length; i++) {
				...
				while (children.hasMoreElements()) {
					child = children.nextElement();
					if (child.getUserObject().toString().equals(nameArr[i])) { // this is the adding of a subsequent
																				// child to a node
						currNode = child;
						break;
					}
	...
	/**
	 * Strings which represent the "root" directories for a given set of
	 * classifiers<br>
	 ...
	 * "-F str" ignore file "str" File names must end in ".class" also, they're case
	 * sensitive<br>
	 * <br>
	 * UNUSABLE args, but nifty ideas, I think<br>
	 * <br>
	 * "-FC str" ignore a file containing a given string On its own, not very
	 * useful, but perhaps... <br>
	 * "-FPC str" ignore a file, in a package, containing the string. I could see
	 * this being used in, say jgaap where there's one package with <br>
	 * a huge number of classifiers that we may not want. (I don't know if jgaap's
	 * weka classifiers are basically the same as WEKA's or not <br>
	 * but if they were, they could be filtered out in this manner. <br>
	 */
	protected static String[] classifierGroups = new String[]{ // TODO move this into the properties file for ease of
																// editing
			"edu.drexel.psal.jstylo.analyzers -F AuthorWPdata.class -F WekaAnalyzer.class -F SynonymBasedClassifier.class",
			"weka.classifiers.bayes", "weka.classifiers.functions", "weka.classifiers.lazy", "weka.classifiers.meta",
			"weka.classifiers.misc", "weka.classifiers.rules", "weka.classifiers.trees"};
	...
	/**
	 * Rakes up the leaves of the loaded classifier tree and returns them. I don't
	 * really like how this is necessary. At some point, I might try to change
	 * something higher up to remove the need for this. However, right now it's
	 * needed because the algorithm to populate the tree using a string array and I
	 * can't just switch it thanks to the complications with weka
	 *
	 * @param loadedClassifiers
	 *            : the ArrayLists/trees which contain the loaded classifiers to be
	 *            converted
	...
	protected static ArrayList<Node> generateClassifiers() {
		...
		for (String ID : classifierGroups) {
			...
			if (components.length != 1) { // arg extraction if len==1 there are no args, skip and add the module
				...
				for (int i = 1; i < components.length; i++) { // skip the first component as that's the parent name
					...
					// if it is valid, increment i by another index, as each arg-flag is on even
					// indices
		...
		for (Node current : modules) {
			...
			// Logger.logln("\nClassifier Tree for "+current.toString()); // use this to see
			// each tree
	...
	/**
	 * Populates the tree with all of the .class files that are located in the
	 * directories supplied by the source string. Not quite dynamically importing
	 * them, but a big improvement on doing it manually.
	 *
	 * @param current
	 ...
	 * @return
	 */
	protected static Node populateNode(Node current, ArrayList<String> packagesToIgnore,
			ArrayList<String> filesToIgnore) {

		// Logger.logln("node to populate: "+current.getName());

		// non-leaf
		if (!current.getName().substring(current.getName().lastIndexOf(".")).equals(".class")) { // if it is not a
																									// .class, it is a
																									// directory

			URL resource = ClassLoader.getSystemClassLoader()
					.getResource(new String(current.getName().replace(".", "/")));
			File df = null;
			...
			// here is where the weka/jar related stuff will be handled
				...
				try {
					...
					JarFile source = new JarFile(
							resource.getFile().replaceFirst("[.]jar[!].*", ".jar").replaceFirst("file:", ""));
					...
					while (files.hasMoreElements()) {
						...
						// This hacky workaround is for running jstylo from a jar file and loading
						// WriteprintsAnalyzer because it has the wrong naming scheme.
						...
						if (fileName.contains(current.getName().replace(".", "/")) || bypass) {
							...
							if (bypass) {
								...
								String path = fileName.replaceAll("/", ".");
								path += "..."; // kinda hacky way to separate the .class portion of the filename without
								// removing ".class" from inside the string
