...
/**
 * Adds spaces before and after punctuation that is at an end or beginning of a
 * word.
 */
public class WordEndsPunctSeparator extends Canonicizer {
	...
	/**
	 * Separate punctuation by inserting whitespace into argument - only at
	 * beginning or end of word
	 *
	 * @param procText
	 *            Array of Characters to be processed
	 * @return Array of Characters after separating punctuation with single space.
	 */
	@Override
			...
			while (lineScan.hasNext()) {
				...
				if (token.matches("\\S+\\p{Punct}"))
					newToken = newToken.substring(0, newToken.length() - 1) + " "
							+ newToken.substring(newToken.length() - 1);
