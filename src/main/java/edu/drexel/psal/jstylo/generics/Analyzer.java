...
/**
 * Abstract class for analyzers - classification routines to be applied on test
 * sets, given a training set. The data representation is based on Weka's
 * Instances object.
 *
...
public abstract class Analyzer {
	...
	/**
	 * Mapping of test documents to distribution classification results for each
	 * unknown document.
	 */
	...
	/**
	 * Classifies the given test set based on the given training set. Should update
	 * the following fields along the classification: trainingSet, testSet, results
	 * and authors. Returns list of distributions of classification probabilities
	 * per instance.
	 *
	 * @param trainingSet
	 ...
	 * @return The mapping from test documents to distributions of classification
	 *         probabilities per instance, or null if prepare was not previously
	 *         called. Each result in the list is a mapping from the author to its
	 *         corresponding classification probability.
	 */
	public abstract Map<String, Map<String, Double>> classify(Instances trainingSet, Instances testSet,
			List<Document> unknownDocs);

	/**
	 * Runs cross validation with given number of folds on the given Instances
	 * object.
	 *
	 * @param data
	 ...
	 * @return Some object containing the cross validation results (e.g. Evaluation
	 *         for Weka classifiers CV results), or null if failed running.
	 */
	public abstract Evaluation runCrossValidation(Instances data, int folds, long randSeed);

	/**
	 * Runs a relaxed cross validation with given number of folds on the given
	 * Instances object. A classification result will be considered correct if the
	 * true class is one of the top <code>
	 * relaxFactor</code> results (where classes are ranked by the classifier
	 * probability they are the class).
	 *
	 * @param data
	 *            The data to run CV over.
	 * @param folds
	...
	/**
	 * A method intended for research purposes when testing and classifying large
	 * groups of documents. <br>
	 * Built with the intent on training on one set of documents (with given
	 * authors) and testing on another distinct set of documents (with the same
	 * author pool).<br>
	 * <br>
	 * At the moment it is limited in its functionality. It does need /some/ way to
	 * determine who the true author is. <br>
	 * For now, this is being checked via the document name, which must at least
	 * contain the author's name as it appears in the list of training document
	 * authors.<br>
	 * NOTE: this carries with it the problem of authors whose names are subsets of
	 * another author ie Robert and Rob--> Rob could mistakenly considered
	 * Robert.<br>
	 * &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp To avoid this, make sure each author name is
	 * unique and not a subset of another, adding symbols or numbers if
	 * necessary.<br>
	 *
	...
	public Evaluation getTrainTestEval(List<Document> testDocs) { // TODO overhaul this for the new author stuff
		...
		// use the results map to find all of the potential authors and add them to
		// extractedAuthors
		...
		// we need SOME way to tell who the real author is. Right now I'm just going to
		// arbitrarily
		// decide that this is via the document title, as I can't really think of an
		// easy way to do it otherwise; no matter what we decide to use, the test set
		// will need to be
		// prepped before hand regardless.
		for (String testDoc : results.keySet()) {
			...
			for (String potentialAuthor : results.get(currTestDoc.getTitle()).keySet()) {
				if (results.get(currTestDoc.getTitle()).get(potentialAuthor).doubleValue() > max) { // find which
																									// document has the
																									// highest
			...
			// check to see whether or not that author was correct, and evaluate the model
			// accordingly.
	...
	public String getTrainTestStatString(List<Document> testDocs) { // TODO implement ROC stuff
		...
		try {
			...
			for (Author a : stats) {
				...
				if (!(a.getName().equals("_dummy_"))) {
					...
					authorInfoString += String.format("\t\t%.3f    %.3f    %.3f      %.3f    %.3f      %.3f    %s\n",
							a.getTruePositiveRate(), a.getFalsePositiveRate(), a.getPrecision(), a.getRecall(),
							a.getF1Measure(), a.getMCC(), a.getName());
			...
			resultsString += String.format("\tCorrectly classified instances: %d\t%.3f %%\n", correctDocs,
					((double) correctDocs / totalDocs) * 100.0);
			resultsString += String.format("\tIncorrectly classified instances: %d\t%.3f %%\n\n",
					(totalDocs - correctDocs), ((double) (totalDocs - correctDocs) / totalDocs) * 100.0);
	...
	/**
	 * Returns the last training Weka Instances set that was used for
	 * classification.
	 *
	 * @return The last training Weka Instances set that was used for
	 *         classification.
	 */
	 ...
	 *
	 * @return The last test Weka Instances set that was used for classification.
	 */
	 ...
	 *
	 * @return The last list of author names.
	 */
	...
	/**
	 * Returns the last classification results or null if no classification was
	 * applied.
	 *
	 * @return The classification results or null if no classification was applied.
	 */
	 ...
	 *
	 * @param ops
	 *            array of strings (the arguments) for the analyzer/classifier
	 */
	 ...
	 * and the flag used to invoke each argument. It is essential that each the flag
	 * comes prior to the description and that in between them, an \<ARG\> tag
	 * appears. The arg parser expects the description in this format, and failing
	 * to comply to it will result in the inability to edit the args. Example:
	 * -C\<ARG\>Enables some function c such that...
	 *
	 * @return a description corresponding to each option the analyzer/classifier
	 *         has
	 */
	 ...
	 *
	 * @return the string describing how the analyzer/classifier functions and its
	 *         benefits/drawbacks
	 */
	 ...
	 *
	 * @return returns the analyzer's driving classifier if it has one, otherwise
	 *         returns null.
	 */
	...
	/**
	 * Returns the name of whatever is doing the "heavy lifting" in terms of
	 * classification
	 *
