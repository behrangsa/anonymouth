...
/**
 * The cumulative feature driver class is designed to create a concatenated
 * result of several feature drivers. For details about a feature driver, look
 * into the documentation of {@see FeatureDriver}.
 *
...
public class CumulativeFeatureDriver {
	...
	/**
	 * Creates a new cumulative feature driver, with the given list of feature
	 * drivers.
	 *
	 ...
	 * @param doc
	 *            Input document.
	 * @return List of all event sets extracted per each event driver.
	 * @throws Exception
	 */
	...
	/**
	 * Returns the feature driver at the given index from the list of feature
	 * drivers.
	 *
	 * @param i
	 *            The index of the desired feature driver in the list of feature
	 *            drivers.
	 * @return The feature driver at the given index in the list of feature drivers.
	 */
	 ...
	 *
	 * @return The display name of the cumulative feature driver.
	 */
	 ...
	 *
	 * @return The size of the feature drivers list.
	 */
	...
	/**
	 * Adds the given feature driver to the list of feature drivers and returns its
	 * position in the list. If the add operation failed, returns -1.
	 *
	 * @param ed
	 *            The feature driver to be added to the list of feature drivers.
	 * @return The position of the given feature driver in the list of feature
	 *         drivers. If the add operation failed, returns -1.
	 */
	...
	/**
	 * Replaces the feature driver at the given index with the given feature driver
	 ...
	 * @return The old feature driver at the given index.
	 */
	 ...
	 * @return The removed feature driver, or null if the index is out of bound.
	 */
	 ...
	 *
	 * @return The description of the feature set.
	 */
		...
		for (int i = 0; i < features.size(); i++) {
			...
			if (fd.getCanonicizers() != null) {
				for (Canonicizer c : fd.getCanonicizers()) {
					...
					for (Pair<String, FeatureDriver.ParamTag> param : FeatureDriver
							.getClassParams(c.getClass().getName())) {
						res += "\t\t\t\t<param name=\"" + param.getFirst() + "\" value=\""
								+ c.getParameter(param.getFirst()) + "\"/>\n";
					}
					res += "\t\t\t</canonicizer>\n";
				}
			}
			...
			if (fd.getCullers() != null) {
				for (EventCuller ec : fd.getCullers()) {
					...
					for (Pair<String, FeatureDriver.ParamTag> param : FeatureDriver
							.getClassParams(ec.getClass().getName())) {
						res += "\t\t\t\t<param name=\"" + param.getFirst() + "\" value=\""
								+ ec.getParameter(param.getFirst()) + "\"/>\n";
					}
					res += "\t\t\t</culler>\n";
				}
			}
	...
	private enum Tag {
		FEATURE_SET, FEATURE, EVENT_DRIVER, CANONICIZERS, CANONICIZER, CULLERS, CULLER, NORM, FACTOR,
	}
	...
	private class XMLParser extends DefaultHandler {
		...
		public void parse() throws Exception {
			...
			// load single value lists (ie lists where, any given feature set will have
			// exactly one value
			...
			for (int i = 0; i < items.getLength(); i++) {
				...
				try {
								...
								if (current.hasChildNodes()) {
									...
									for (int k = 0; k < canonicizers.getLength(); k++) {
										if (!canonicizers.item(k).getNodeName().equals("#text")) {
											Element currCanon = (Element) xmlDoc.importNode(canonicizers.item(k),
													false);
											if (currCanon.hasAttribute("class"))
												fd.addCanonicizer((Canonicizer) Class
														.forName(currCanon.getAttribute("class")).newInstance());
										}
									}
								}
								...
								for (int k = 0; k < evculls.getLength(); k++) {
									...
									if (!currEvCNode.getNodeName().equals("#text")) {
										...
										if (currEvCuller.hasAttribute("class")) {
											// create the class
											EventCuller culler = (EventCuller) Class
													.forName(currEvCuller.getAttribute("class")).newInstance();
											...
											if (currEvCNode.hasChildNodes()) {
												...
												for (int m = 0; m < params.getLength(); m++) {
													...
													if (!currPNode.getNodeName().equals("#text")) {
														...
														Element currParam = (Element) xmlDoc.importNode(params.item(m),
																true);
														if (currParam.hasAttribute("name")
																&& currParam.hasAttribute("value")) {
															culler.setParameter(currParam.getAttribute("name"),
																	currParam.getAttribute("value"));
														}
													}
												}
											}
					...
					if (print)
						Logger.logln(NAME + "Failed to load feature driver: " + currentElement.getAttribute("name"),
		 ...
		 *
		 * @return The generated cumulative feature driver.
		 */
	 ...
	 * FeatureWizardDriver.populateAll(); CumulativeFeatureDriver cfd = new
	 * CumulativeFeatureDriver(new
	 * File("feature_sets/example_feature_set.xml").getAbsolutePath()); cfd = new
	 * CumulativeFeatureDriver(); FeatureDriver fd = new
	 * FeatureDriver("word-lengths", true, new
	 * edu.drexel.psal.jstylo.eventDrivers.WordLengthEventDriver());
	 * cfd.addFeatureDriver(fd); ProblemSet ps = new
	 * ProblemSet("./problem_sets/sample_problem_set.xml"); WekaInstancesBuilder w =
	 * new WekaInstancesBuilder(false); w.prepareTrainingSet(ps.getTrainDocs("aa"),
	 * cfd); System.out.println(w.getTrainingSet()); }
	 */
}
