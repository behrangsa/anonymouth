...
/**
 * The WekaInstancesBuilder class is designed to parse a list of known event
 * sets representing the training corpus and a list of unknown event sets
 * representing a test corpus into a Weka Instances object. It further allows
 * training and classification based on the parsed data using an underlying Weka
 * classifier, and output the parsed Instances object into a standard ARFF file.
 *
...
public class WekaInstancesBuilder {
	...
	/**
	 * To hold last used list of sets of events, each set corresponds to a feature
	 * class (feature driver), i.e. each event corresponds to a feature in the Weka
	 * attribute list.
	 */
	...
	/**
	 * To hold last used map of instances to their list of total appearances per
	 * feature class (feature driver).
	 */
	private Map<Instance, int[]> featureClassPerInst;

	/**
	 * To hold last used list of total number of appearances of all features in a
	 * feature class across all TRAINNING documents.
	 */
	private int[] featureClassAllTrainDocs;

	/**
	 * To hold last used map of features to their total number of appearances across
	...
	/**
	 * To hold last used array of opening indices per feature class in the Weka
	 * attribute list. E.g. for feature classes 'digits' and 'letters', with
	 * features 0,1,...,9 and a,b,...,z it will hold [1, 11, 37] -- starts with 1
	 * since the at 0 resides the author name class.
	 */
	 ...
	 *
	 * @param isSparse
	 *            Set the representation of the data instances to be sparse (using
	 *            SparseInstance instead of Instance).
	 */
	...
	public static class CalcThread extends Thread {
		...
		public CalcThread(int div, int threadId, int knownDocsSize, List<Document> knownDocs,
				CumulativeFeatureDriver cfd) {
			this.div = div;
	...
	/**
	 * Prepares the training Weka Instances data: using the given cumulative feature
	 * driver, it extracts all features from the training documents, builds the
	 * corresponding attribute list and saves the data into trainingSet.
	 *
	 * @param knownDocs
	...
	public void prepareTrainingSet(List<Document> knownDocs, CumulativeFeatureDriver cfd) throws Exception {
		...
		for (int thread = 0; thread < numCalcThreadsToUse; thread++)
			calcThreads[thread] = new CalcThread(div, thread, knownDocsSize, knownDocs,
		...
		for (int j1 = 0; j1 < known.size(); j1++) {
			for (int iterator = 0; iterator < known.get(j1).size(); iterator++) {
				...
				// "+known.get(j1).get(iterator).getEventSetID()+" to
				// "+IDs.get(iterator));
		...
		for (int currEventSet = 0; currEventSet < numOfFeatureClasses; currEventSet++) {
			...
			if (cfd.featureDriverAt(currEventSet).isCalcHist()) { // calculate histogram
				...
				for (Event event : events)
					attributeList
							.addElement(new Attribute(event.getEvent().replaceFirst("\\{", "-" + (++count) + "{")));
		...
		// initialize vector size (including authorName and title if required) and first
		// indices of
		...
		for (i = 0; i < numOfVectors; i++) {
			...
			for (j = 0; j < numOfFeatureClasses; j++) {
				...
				if (cfd.featureDriverAt(j).isCalcHist()) {
					...
					for (Event e : events) {
						inst.setValue((Attribute) attributeList.elementAt(index++), currHist.getAbsoluteFrequency(e)); // use
																														// absolute
																														// values,
																														// normalize
																														// later
					...
					double value = Double.parseDouble(
							known.get(i).get(j).eventAt(0).toString().replaceAll(".*\\{", "").replaceAll("\\}", ""));
	...
	/**
	 * Applies InfoGain (Weka) on the training set and returns a description of the
	 * results: List of attributes sorted by weights and feature-type breakdown.
	 *
	 * @param changeAttributes
	 *            Whether to change the number of attributes in the training set
	 *            according to the results of the InfoGain.
	 * @param N
	 *            The number of most effecting attributes to reduce the list to.
	 * @throws Exception
	 */
	public String applyInfoGain(boolean changeAttributes, int N) throws Exception {
		...
		for (int i = 0; i < n - 1; i++) {
			if (infoArr[i][0] == 0)
				break;
			res += String.format("> %-" + len + "s   %f\n", trainingSet.attribute((int) infoArr[i][1]).name(),
					infoArr[i][0]);
		}
		res += "\n";
		...
		for (String attr : attrListBreakdown)
			res += String.format("> %-" + len + "s   %f (%.2f%%)\n", attr, featureTypeBreakdown.get(attr),
					featureTypeBreakdown.get(attr) * 100 / total);
		res += "\n";
		...
		if (changeAttributes) {
			if (N >= trainingSet.numAttributes() - 1) {
				...
				// the problem was twofold: 1) the Attributes were only being removed from the
				// trainingSet
				// this meant that testSet didn't line up properly, and caused errors down the
				// line
				// 2) the incorrect features were being cut out. I've inclluded a fix--basically
				// this entire
				// chunk was rewritten.
				// should work with any feature set and any N

				// create an array with the value of infoArr's [i][1] this array will be shrunk
				// and modified
				// as needed
	...
	/**
	 * Prepares the test Weka Instances data: it extracts all features from the test
	 * documents based on the list of attributes derived from the training
	 * preparation, and saves the data into testSet. Should be called only after the
	 * training Instances data is initialized.
	 *
	 * @param unknownDocs
	...
	public void prepareTestSet(List<Document> unknownDocs) throws Exception {
		...
		for (int currEventSet = 0; currEventSet < numOfFeatureClasses; currEventSet++) {
			...
			if (cfd.featureDriverAt(currEventSet).isCalcHist()) { // calculate histogram
				...
				// generate sole event (extract full event name and remove value)
				// Event event = new
				// Event(list.get(0).eventAt(0).getEvent().replaceAll("\\{.*\\}", "{-}"));
		...
		for (i = 0; i < numOfVectors; i++) {
			...
			for (j = 0; j < numOfFeatureClasses; j++) {
				...
				if (cfd.featureDriverAt(j).isCalcHist()) {
					...
					// extract numeric value from original sole event
					double value = Double.parseDouble(
							unknown.get(i).get(j).eventAt(0).toString().replaceAll(".*\\{", "").replaceAll("\\}", ""));
					inst.setValue((Attribute) attributeList.elementAt(index++), value);
				}
			}
	...
	/**
	 * Initializes the normalization baselines and values for the training
	 * documents.
	 *
	 * @throws Exception
	 */
	private void initNormTrainBaselines() throws Exception {
		...
		for (i = 0; i < numOfFeatureClasses; i++) {
			...
			if (norm == NormBaselineEnum.FEATURE_CLASS_IN_DOC || norm == NormBaselineEnum.FEATURE_CLASS_ALL_DOCS) {
				...
				// accumulate feature sum across all TRAINING documents for each feature in this
				// feature
				// class
				int sum;
				for (k = start; k < end; k++) {
			...
			if (norm == NormBaselineEnum.FEATURE_CLASS_ALL_DOCS) {
				...
				// accumulate appearances of all features in the feature class, across all
				// TRAINING
				// documents
				for (j = 0; j < numOfVectors; j++)
	...
	/**
	 * Applies normalization over the given Weka Instances data. Should be called
	 * after normalization initialization.
	 *
	 * @param insts
		...
		for (i = 0; i < numOfFeatureClasses; i++) {
			...
			if (norm == NormBaselineEnum.FEATURE_CLASS_IN_DOC) {
				...
				for (j = 0; j < numOfVectors; j++) {
					...
					for (k = start; k < end; k++)
						currInst.setValue(k,
								currInst.value(k) * factor / ((double) featureClassPerInst.get(currInst)[i]));
				}
				...
				for (j = 0; j < numOfVectors; j++) {
					currInst = insts.instance(j);
					for (k = start; k < end; k++)
						currInst.setValue(k, currInst.value(k) * factor
								/ ((double) featureAllTrainDocs.get(insts.attribute(k).name())));
				}
	...
	/**
	 * Given Weka Instances object, returns the same dataset represented as a Weka
	 * sparse dataset.
	 *
	 * @param data
	 *            The data to convert to sparse.
	 * @return The input data represented as a Weka sparse dataset.
	 */
	...
	public void setNumCalcThreads(int nct) {
		...
		if (jProps.exists()) { // write numCalcThreads to the file
			...
			try {
				...
				Logger.logln("Prop file empty! numCalcThreads defaulting to 1! Generating new prop file...",
						Logger.LogOut.STDERR);
				e.printStackTrace();
				numCalcThreads = 1;
			}
	...
	/**
	 * Sets the Instances representation to sparse if given true, and standard
	 * otherwise.
	 *
	 * @param isSparse
	 ...
	 *
	 * @return True if the Instances representation is sparse, and false otherwise.
	 */
	...
	/**
	 * Returns the list of lists of event sets extracted from the known documents in
	 * the last call to prepare().
	 *
	 * @return The list of lists of event sets extracted from the known documents in
	 *         the last call to prepare().
	 */
	...
	/**
	 * Returns the list of lists of event sets extracted from the unknown documents
	 * in the last call to prepare().
	 *
	 * @return The list of lists of event sets extracted from the unknown documents
	 *         in the last call to prepare().
	 */
	 ...
	 *
	 * @return The list of authors extracted in the last call to prepare().
	 */
	...
	/**
	 * Returns the Weka Instances object that represents the training set extracted
	 * in the last call to prepare().
	 *
	 * @return The Weka Instances object that represents the training set extracted
	 *         in the last call to prepare().
	 */
	...
	/**
	 * Returns the Weka Instances object that represents the test set extracted in
	 * the last call to prepare().
	 *
	 * @return The Weka Instances object that represents the test set extracted in
	 *         the last call to prepare().
	 */
	 ...
	 *
	 * @return The entire data in one Weka Instances set.
	 */
	...
	/**
	 * Returns true iff it is set to include document titles as an additional
	 * attribute.
	 *
	 * @return True iff it is set to include document titles as an additional
	 *         attribute.
	 */
	...
	/**
	 * Returns the dummy author name used to bypass Weka bug when using sparse
	 * representation.
	 *
	 * @return The dummy author name used to bypass Weka bug when using sparse
	 *         representation.
	 */
	/*
	 * public static String getDummy() { return dummy; }
	 */
