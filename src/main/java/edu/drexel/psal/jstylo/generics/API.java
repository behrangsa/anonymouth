...
public interface API {
	...
	/*
	 * 
	 * public void prepareTrainingSet(List<Document> knownDocs,
	 * CumulativeFeatureDriver cfd) public void prepareTestSet(List<Document>
	 * unknownDocs) public String applyInfoGain(boolean changeAttributes, int N)
	...
	// ex by creating a method (or adding the functionality to a current method) to
	// strip
	// normalization baselines from a document so that the doc doesn't need to be
	// carried

	// feature extraction - training set

	/**
	 * Extracts the List of EventSets from a document using the provided
	 * CumulativeFeatureDriver
	 *
	 * @param document
	 *            the document to have features extracted and made into event sets
	 * @param cumulativeFeatureDriver
	 *            the driver containing the features to be extracted and the
	 ...
	 *
	 * @param eventSets
	 *            A List which contains Lists of EventSets (represents a list of
	 *            documents' EventSets0
	 * @param cumulativeFeatureDriver
	...
	/**
	 * Goes over the culled List<List<EventSet>> and determines which events are
	 * histograms and which have a single<br>
	 * numerical value. Uses the information to prepare a List\<EventSet\> to
	 * extract from the test document(s)
	 *
	 * @param culledEventSets
	 *            The culled List<List<EventSet>>
	 * @param cumulativeFeatureDriver
	 *            The driver used to extract the EventSets
	...
	/**
	 * Generates the List\<Attributes\> from the List\<List\<EventSet\>\> that will
	 * be used to create the Instances object.
	 *
	 * @param culledEventSets
	 *            The culled list of EventSets that have been gathered from the
	 *            document set
	...
	public ArrayList<Attribute> getAttributeList(List<List<EventSet>> culledEventSets, List<EventSet> relevantEvents,
			CumulativeFeatureDriver cfd, boolean hasDocTitles) throws Exception;

	/**
	 * Takes various parameters related to a document and creates an Instance object
	 * from that data.
	 *
	 * @param attributes
	 *            the data used to construct the Instance object
	 * @param cumulativeFeatureDriver
	 *            driver used to determine the type of feature being added
	 * @param documentData
	...
	/**
	 * Normalizes all of the features of the specified instance. Does not support
	 * global normalization baselines!
	 *
	 * @param cumulativeFeatureDriver
	 *            the driver used to tell what to normalize and in what fashion.
	 * @param instance
	 *            the object to be normalized
	 * @throws Exception
	 */
	public void normInstance(CumulativeFeatureDriver cumulativeFeatureDriver, Instance instance, Document document,
	...
	// TODO fixme
	/**
	 * Calculates InfoGain on the instances to provide information on how useful
	 * each feature was to identifying the documents.
	 *
	 * @param insts
	 *            the instances to calculate over
	 * @return a two-dimensional sorted array with one element per feature. It is
	 *         sorted via how useful each feature was and stores the index.
	 * @throws Exception
	 */
	public double[][] calcInfoGain(Instances insts) throws Exception;

	/**
	 * Removes all but the top N features (as returned by calcInfoGain) from the
	 * instances
	 *
	 ...
	 *            the features from the EventSets which are going to be evaluated
