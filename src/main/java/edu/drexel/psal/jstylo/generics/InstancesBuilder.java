...
/**
 * An API for the feature extraction process. Designed for running on a single
 * machine
 *
 * @author Travis Dutko
 */
public class InstancesBuilder extends Engine {
	...
	// ThreadArrays so that we can stop them if the user cancels something mid
	// process
	...
	/**
	 * Empty constructor. For use if you want to build it as you go, rather then all
	 * at once<br>
	 * Ensure not to try to do anything fancy with it until all required information
	 * is set
	 */
	 ...
	 * @param cfdPath
	 *            the path to the XML file to read the cumulativeFeatureDriver from
	 * @param sparse
	 *            if sparse instances should be produced. If false, dense instances
	 *            will be produced
	 * @param udt
	 *            if document titles should be considered a part of classification
	 * @param nt
	 *            number of threads to use whenever multithreading is possible
	 */
	public InstancesBuilder(String probSetPath, String cfdPath, boolean sparse, boolean udt, int nt) {
		...
		try {
			...
			Logger.logln("Failure loading ProblemSet and CumulativeFeatureDriver!", LogOut.STDERR);
			e.printStackTrace();
		}
	 ...
	 *            number of threads to use whenever multithreading is possible
	 */
	public InstancesBuilder(ProblemSet probSet, CumulativeFeatureDriver cumulativeFeatureDriver, boolean sparse,
			boolean udt, int nt) {
	 ...
	 *            number of threads to use whenever multithreading is possible
	 */
	public InstancesBuilder(ProblemSet probSet, CumulativeFeatureDriver cumulativeFeatureDriver, int nt) {
	...
	/**
	 * Extracts the List\<EventSet\> from each document using a user-defined number
	 * of threads. Culls the eventSets as well.
	 *
	 * @throws Exception
	 */
	public void extractEventsThreaded() throws Exception {
		...
		for (int thread = 0; thread < threadsToUse; thread++) // create the threads
			featThreads[thread] = new FeatureExtractionThread(div, thread, knownDocsSize, knownDocs,
	...
	/**
	 * Initializes and stores the list of relevantEvents created from the culled
	 * List\<List\<EventSet\>\>
	 *
	 * @throws Exception
	 */
	...
	/**
	 * Initializes and stores the list of attributes created from the eventSets and
	 * relevantEvents
	 *
	 * @throws Exception
	 */
	...
	public void createTrainingInstancesThreaded() throws Exception {
		...
		trainingInstances = new Instances("Instances", attributes, eventList.size());
		...
		for (int thread = 0; thread < threadsToUse; thread++)
			trainThreads[thread] = new CreateTrainInstancesThread(trainingInstances, div, thread, numInstances,
					new CumulativeFeatureDriver(cfd));
		for (int thread = 0; thread < threadsToUse; thread++)
	...
	/**
	 * Creates Test instances from all of the information gathered (if there are
	 * any)
	 *
	 * @throws Exception
	 */
	public void createTestInstancesThreaded() throws Exception {
		...
		testInstances = new Instances("TestInstances", attributes, ps.getAllTestDocs().size());

		// if there are no test instances, set the instance object to null and move on
		// with our lives
	 ...
	 *
	 * @return a double[][] containing an entry for each feature denoting its index
	 *         and how useful it is. Sorted via usefulness.
	 * @throws Exception
	 */
	...
	/**
	 * @return Returns the infoGain value and stores it locally incase we decide to
	 *         apply it
	 */
	...
	/**
	 * A niche method for when you already have a training Instances object and only
	 * want to build test instances
	 *
	 * @param ti
	...
	/**
	 * For use when stopping analysis mid-way through it. Kills any processing
	 * threads
	 */
	...
	/**
	 * A thread used to parallelize the creation of Test instances from a set of
	 * documents
	 *
	 * @param data
	 *            the dataset (Instances object) that the test instance belongs to
	 * @param d
	 *            The "div" or divide--how many documents each thread processes at
	 *            most
	 * @param t
	 *            The threadId. Keeps track of which thread is doing which div of
	 *            documents
	 * @param n
	 *            the total number of instances (used to put a cap on the last div
	 *            so it doesn't try to process docs which don't exist
	 * @param cd
	 *            A copy of the cfd to assess features with
	 * @author Travis Dutko
	 */
		...
		public void run() {
			...
			for (int i = div * threadId; i < Math.min(numInstances, div * (threadId + 1)); i++)
				try {
					...
					// build the instance
					Instance instance = createInstance(attributes, relevantEvents, cfd, events, doc, isSparse,
							useDocTitles);
	...
	/**
	 * A thread used to parallelize the creation of Training instances from a set of
	 * documents
	 *
	 * @param data
	 *            the dataset (Instances object) that the training instance belongs
	 *            to
	 * @param d
	 *            The "div" or divide--how many documents each thread processes at
	 *            most
		...
		public void run() {
			...
			for (int i = div * threadId; i < Math.min(numInstances, div * (threadId + 1)); i++)
				try {
					...
					Instance instance = createInstance(attributes, relevantEvents, cfd, eventList.get(i), doc, isSparse,
							useDocTitles);
	 ...
	 *
	 * @param d
	 *            the divide--how many documents each thread processes at most
	 * @param threadId
	 *            keeps track of which thread is doing which div of documents
	 * @param knownDocs
	 *            a list of documents to be divvied up and have its features
	 *            extracted
	 * @param cd
	 *            A copy of the cfd to assess features with
	 * @author Travis Dutko
	...
	public class FeatureExtractionThread extends Thread {
		...
		ArrayList<List<EventSet>> list = new ArrayList<List<EventSet>>(); // The list of event sets for this division of
																			// docs
		...
		// Constructor
		public FeatureExtractionThread(int div, int threadId, int knownDocsSize, List<Document> knownDocs,
