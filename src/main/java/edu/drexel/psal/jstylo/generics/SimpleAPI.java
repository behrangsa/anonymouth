...
public class SimpleAPI {
	...
	public static enum analysisType {
		CROSS_VALIDATION, TRAIN_TEST, BOTH
	};
	 ...
	 *
	 * @param psXML
	 *            path to the XML containing the problem set
	 * @param cfdXML
	 *            path to the XML containing the cumulativeFeatureDriver/feature set
	 * @param numThreads
	 *            number of calculation threads to use for parallelization
	 * @param classPath
	 *            path to the classifier to use (of format
	...
	/**
	 * Constructor for use with a weka classifier. Do not call prepare Analyzer if
	 ...
	 *
	 * @param n
	 *            the number of features/attributes to keep
	 */
	...
	public void run() {
		...
		switch (selected) {
			...
			// do a train/test
			case TRAIN_TEST :
				trainTestResults = analysisDriver.classify(ib.getTrainingInstances(), ib.getTestInstances(),
						ib.getProblemSet().getAllTestDocs());
				break;
	 ...
	 *
	 * @param n
	 *            number of folds to use from now on
	 */
	...
	/**
	 * @return the infoGain data (not in human readable form lists indices and
	 *         usefulness)
	 */
	...
	/**
	 * Returns a string of features, in order of most to least useful, with their
	 * infogain values<br>
	 *
	 * @param showZeroes
	 *            whether or not to show features that have a 0 as their infoGain
	 *            value
	...
	/**
	 * @return String containing accuracy, metrics, and confusion matrix from cross
	 *         validation
	 */
	...
	/**
	 * @return the weka clsasifier being used by the analyzer. Will break something
	 *         if you try to call it on a non-weka analyzer
	 */
	 ...
	 * 
	 * // Check if any predictions have been collected if (m_Predictions == null) {
	 * return Utils.missingValue(); } else { ThresholdCurve tc = new
	 * ThresholdCurve(); Instances result = tc.getCurve(m_Predictions, classIndex);
	 ...
	 */
	/*
	 * public double weightedAreaUnderROC() { double[] classCounts = new
	 * double[m_NumClasses]; double classCountSum = 0; double[][] matrix =
	 * getTrainTestEval().confusionMatrix()
	 * 
	 * for (int i = 0; i < m_NumClasses; i++) { for (int j = 0; j < m_NumClasses;
	 * j++) { classCounts[i] += matrix[i][j]; } classCountSum += classCounts[i]; }
	 * 
	 * double aucTotal = 0; for (int i = 0; i < m_NumClasses; i++) { double temp =
	 * areaUnderROC(i); if (!Utils.isMissingValue(temp)) { aucTotal += (temp *
	 * classCounts[i]); } }
	 * 
	...
	public static void main(String[] args) {
		...
		SimpleAPI test = new SimpleAPI(
				...
				"./jsan_resources/feature_sets/writeprints_feature_set_limited.xml", 8,
				"weka.classifiers.functions.SMO", analysisType.CROSS_VALIDATION);
