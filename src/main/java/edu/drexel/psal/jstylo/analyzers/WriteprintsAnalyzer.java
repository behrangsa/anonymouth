...
/**
 * Implementation of the Writeprints method (supervised). For more details
 * see:<br>
...
public class WriteprintsAnalyzer extends Analyzer {
	...
	/**
	 * The list of training author data, including feature, basis and writeprint
	 * matrices.
	 */
	...
	/**
	 * Whether to average all feature vectors per author ending up with one feature
	 * vector or not. Increases performance but may reduce accuracy.
	 */
	...
	public Map<String, Map<String, Double>> classify(Instances trainingSet, Instances testSet,
		...
		// TODO if we get weird results somewhere the two lines below might be the
		// reason. Testing
		// didn't reveal anything though
		// this was added because authors weren't being cleared between one
		// classification and the next
		// ie different presses of the "run analysis" button.
		if (authors != null)
			authors.clear();
		...
		if (unknownDocs != null) {
			...
			for (int i = 0; i < numTestInstances; i++) {
				authorName = TEST_AUTHOR_NAME_PREFIX + String.format("%03d", i) + "_" + unknownDocs.get(i).getTitle();
				authorData = new AuthorWPData(authorName);
				Logger.logln("- " + authorName);
		...
		for (AuthorWPData testData : testAuthorData) {
			...
			for (AuthorWPData trainData : trainAuthorData) {
				...
				// Logger.logln("- " + trainData.authorName + ": " + totalDist+ " actual:
				// "+testData.authorName);
			}
	...
	public Map<String, Map<String, Double>> normalize(Map<String, Map<String, Double>> data) {
		...
		for (String testDoc : data.keySet()) { // iterates over the test docs
			Map<String, Double> dataSet = data.get(testDoc);
			for (String potentialAuthor : dataSet.keySet()) { // then over potential authors
				normalizedResults.get(testDoc).put(potentialAuthor,
						data.get(testDoc).get(potentialAuthor).doubleValue() / totalValue);
			}
		}
	...
	public Evaluation runCrossValidation(Instances data, int folds, long randSeed) {
		...
		// E.g. for 10 folds, use 1-5 for training, 6-10 for testing; 2-6 for training,
		// 1 + 7-10 for
		// testing, etc.
			...
			for (String testInstAuthor : results.keySet()) {
				...
				if (testInstAuthor.equals(selected)) { // train the eval with a food instance
					...
					try {
						...
						// Logger.logln("Attempting to add incorrect instance at:
						// "+(correctIndex*extractedAuthors.numValues()+incorrectIndex));
						// Logger.logln("\t "+"correctAuthor: "+testInstAuthor+" guess: "+selected);
						...
						index -= correctIndex; // adjusts for the fact that there are numAuthors-1 cells per row in
						// the bad instances part of the instance list
	...
	/**
	 * Calculates and returns a mapping of authors to the number of documents of the
	 * corresponding author in the first dataset, the second dataset, and the
	 * distances measured between them.<br>
	 * Both datasets must have the same features, including the same set of authors
	 ...
	 * @return
	 */
	public SortedMap<String, double[]> getCrossDatasetsDistances(Instances dataset1, Instances dataset2) {
		...
		// initialize synonym count mapping
		Logger.logln("Initializing word synonym count");
		Map<Integer, Integer> wordsSynCount = calcSynonymCount(dataset1, numFeatures);
		...
		for (AuthorWPData dataset1Data : dataset1AuthorData) {
			...
			for (AuthorWPData dataset2Data : dataset2AuthorData) {
				...
				results.put(dataset1Data.authorName + "," + dataset2Data.authorName,
						new double[]{dataset1Data.numAuthorInstances, dataset2Data.numAuthorInstances, avgDist});
				if (min > avgDist) {
					min = avgDist;
			...
			Logger.logln("minimum distance author: " + minAuthor + ", distance: " + min);
		}
	...
	private static String[] wordFeatures = {"Function-Words", "Words", "Word-Bigrams", "Word-Trigrams",
			"Misspelled-Words"};

	/**
	 * Constructs a mapping from all word-based feature indices to the number of
	 * their synonyms. The synonym counted are only those belonging to synsets of
	 * the most common part-of-speech synset-type. If the feature is an n-gram
	 * feature, the synonym count is the multiplication of synonym count values of
	 * each word in the n-gram.
	 *
	 * @param trainingSet
	 *            The training set from which to extract the features.
	 * @param numFeatures
	 *            The number of features.
	 * @return A mapping from the word feature indices of the given training set to
	 *         the synonym count.
	 */
	...
	/**
	 * Extracts the words from the given feature name and returns them in an array.
	 *
	 * @param featureName
	...
	/**
	 * Calculates and returns the information gain vector for all features based on
	 ...
	 * @param numFeatures
	 *            The number of features.
	 * @return The information gain vector for all features based on the given
	 *         training set.
	 * @throws Exception
	...
	/**
	 * Returns the average of the Euclidean distance between every pair of columns
	 * (corresponding to document feature values) of the given matrices.
	 *
	 * @param a
	 *            The first matrix.
	 * @param b
	...
	public String analyzerDescription() {
		String description = "Writeprints Analyzer\n"
	 ...
	 */
	/*
	 * public static void main(String[] args) throws Exception { // initialize log
	 * PrintStream logPS = new PrintStream(new File("./log/" +
	 * MultiplePrintStream.getLogFilename())); log = new
	 * MultiplePrintStream(System.out, logPS);
	 * 
	 * WriteprintsAnalyzer wa = new WriteprintsAnalyzer();
	 * 
	 * //ProblemSet ps = new ProblemSet(JSANConstants.JSAN_PROBLEMSETS_PREFIX +
	 * "drexel_1_train_test.xml"); ProblemSet ps = new
	 * ProblemSet(JSANConstants.JSAN_PROBLEMSETS_PREFIX + "drexel_1.xml");
	 * //ProblemSet ps = new ProblemSet(JSANConstants.JSAN_PROBLEMSETS_PREFIX +
	 * "amt.xml");
	 * 
	 * CumulativeFeatureDriver cfd = new
	 * CumulativeFeatureDriver(JSANConstants.JSAN_FEATURESETS_PREFIX +
	 * "writeprints_feature_set_limited.xml"); WekaInstancesBuilder wib = new
	 * WekaInstancesBuilder(false); List<Document> trainingDocs =
	 * ps.getAllTrainDocs(); List<Document> testDocs = ps.getTestDocs(); int
	 * numTrainDocs = trainingDocs.size(); int numTestDocs = testDocs.size();
	 * 
	 * // extract features System.out.println("feature pre extraction");
	 * trainingDocs.addAll(testDocs); System.out.println("feature extraction");
	 * wib.prepareTrainingSet(trainingDocs, cfd);
	 * System.out.println("feature post extraction"); Instances trainingSet =
	 * wib.getTrainingSet(); Instances testSet = new Instances( trainingSet,
	 * numTrainDocs, numTestDocs); wib.setTestSet(testSet); int total = numTrainDocs
	 * + numTestDocs; for (int i = total - 1; i >= numTrainDocs; i--)
	 * trainingSet.delete(i); System.out.println("done!");
	 * 
	 * Instances train = wib.getTrainingSet(); Instances test = wib.getTestSet(); //
	 ...
	 * //System.exit(0);
	 * 
	 * //Instances train = new Instances(new FileReader(new
	 * File("c:/tmp/drexel_1_train.arff")));
	 * train.setClassIndex(train.numAttributes() - 1); //Instances test = new
	 * Instances(new FileReader(new File("d:/tmp/drexel_1_tt_test.arff")));
	 * //test.setClassIndex(test.numAttributes() - 1);
	 * 
	 * // classify
	 * 
	 * System.out.println("classification"); Map<String,Map<String, Double>> res =
	 * wa.classify(train, test, ps.getTestDocs()); System.out.println("done!");
	 * Map<String,Double> docMap; String selectedAuthor = null; double maxValue;
	 * double success = 0; for (String doc: res.keySet()) { maxValue =
	 * Double.NEGATIVE_INFINITY; docMap = res.get(doc); System.out.println(doc+":");
	 * for (String key: docMap.keySet()) if (maxValue < docMap.get(key)) {
	 * selectedAuthor = key; maxValue = docMap.get(key); }
	 * System.out.println("- "+selectedAuthor+": "+maxValue); success +=
	 * doc.replaceFirst(TEST_AUTHOR_NAME_PREFIX + "\\d+_",
	 * "").startsWith(selectedAuthor) ? 1 : 0; } success = 100 * success /
	 * res.size(); System.out.printf("Total accuracy: %.2f\n",success);
	 * 
	 * 
	 * //cross-validation wa.runCrossValidation(train, 10, 0); }
	 */
}
...
class AuthorWPData {
	 ...
	 * If the <code>average</code> parameter is <code>true</code>, sets the matrix
	 * to be a single vector which is the average values across all the author's
	 * feature vectors.<br>
	...
	/**
	 * Extracts the feature matrix from the given training data (set of all
	 * extracted features), using only the given instance index. If the
	 * <code>average</code> parameter is <code>true</code>, sets the matrix to be a
	 * single vector which is the average values across all the author's feature
	 * vectors.<br>
	 * In addition records the list of features that have 0 frequency.
	 *
	 * @param trainingData
	 *            The ARFF training data representing various feature vectors to
	 *            extract the feature values from.
	 * @param instanceIndex
	 ...
	 *            of all extracted feature vectors.
	 */
	public void initFeatureMatrix(Instances trainingData, int instanceIndex, boolean average) {
	...
	/**
	 * Extracts the feature matrix from the given training data. If the
	 * <code>average</code> parameter is <code>true</code>, sets the matrix to be a
	 * single vector which is the average values across all the author's feature
	 * vectors.<br>
	 * In addition records the list of features that have 0 frequency.
	...
	private void initFeatureMatrixHelper(Instances data, boolean average) {
		...
		/*
		 * initialize a matrix of features (each row represents an instance, each column
	...
	/**
	 * Derives the basis matrix (set of eigenvectors) from feature usage covariance
	 * matrix using Karhunen-Loeve transform (PCA) as described in
	...
	public void initBasisAndWriteprintMatrices() {
		...
		/*
		 * (2) calculate eigenvalues followed by eigenvectors - the basis matrix, and
		 * calculate the principal component matrix - the author's writeprint
		 */
	...
	/**
	 * Generates a pattern for the target author using the target author's feature
	 * values and the basis author's basis matrix (in contrast with the paper,
	 * there's no particular use for the basis author's feature set, as it is the
	 * same for all authors in this implementation).
	 *
	 * @param basisAuthor
	 *            The basis author (the one supplying the basis matrix).
	 * @param targetAuthor
	 *            The target author (the one to generate the pattern for).
	 * @return The pattern matrix for the target author.
	 */
	...
	/**
	 * Adds pattern disruption values with respect to the given author data. That
	 * is, for any zero-frequency feature of this author, that is not a
	 * zero-frequency feature for the other author, adds pattern disruption values
	 * calculated as follows:<br>
	 * <code>d_p = IG(c,p) * K * (syn_total + 1) * (syn_used + 1)</code>
	 *
	 * @param other
	 *            The other author data to add the pattern disruption with respect
	 *            to.
	 * @param IG
	 *            Information gain for all features.
	 * @param wordsSynCount
	 *            Mapping from all word-based feature indices to their
	 *            synonym-count.
	 * @param otherPattern
	 *            The pattern of the other author generated with this author's basis
	 *            matrix.
	 */
	public void addPatternDisruption(AuthorWPData other, double[] IG, Map<Integer, Integer> wordsSynCount,
			Matrix otherPattern) {
	...
	/**
	 * Calculates and returns the average over all columns of the given matrix for
	 * the given row index.
	 *
	 * @param m
	 *            The matrix.
	 ...
	 * @return The average over all columns of the given matrix for the given column
	 *         index.
	 */
	...
	/**
	 * Clones only parts of this author data and shallow copies the rest. Only the
	 * basis and writeprint matrices are deep-copied.
	 *
	 * @return The half-cloned author data.
	 */
