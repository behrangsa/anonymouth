...
public class WekaAnalyzer extends Analyzer {
	...
	public WekaAnalyzer(String pathToClassifier) {
		try {
			...
			Logger.logln("ERROR! Failed loading trained classifier from " + pathToClassifier + "!",
	...
	/**
	 * Trains the Weka classifier using the given training set, and then classifies
	 * all instances in the given test set. Returns list of distributions of
	 * classification probabilities per instance.
	 *
	 * @param trainingSet
	 ...
	 * @return The mapping of test documents to distributions of classification
	...
	/**
	 * Trains and then saves the Weka classifier using the given training set, and
	 * then classifies all instances in the given test set. Returns list of
	 * distributions of classification probabilities per instance.
	 *
	 * @param trainingSet
	 *            The Weka Instances dataset of the training instances.
	 * @param testSet
	 ...
	 *         corresponding classification probability.
	 */
	public Map<String, Map<String, Double>> classifyAndSaveClassifier(Instances trainingSet, Instances testSet,
			List<Document> unknownDocs, String saveClassifierAs) {
		this.trainingSet = trainingSet;
		this.testSet = testSet;
		// initialize authors (extract from training set)
		List<String> authors = new ArrayList<String>();
	...
	/**
	 * Uses the pre-trained classifier to classify all instances in the given test
	 * set. Returns list of distributions of classification probabilities per
	 * instance.
	 *
	 * @param trainingSet
	 *            The Weka Instances dataset of the training instances.
	 ...
	 *         corresponding classification probability.
	 */
	public Map<String, Map<String, Double>> classifyWithPretrainedClassifier(Instances testSet,
			ArrayList<String> unknownDocTitles, Set<String> trainSetAuthors) {
		this.testSet = testSet;
	...
	/**
	 * Trains the Weka classifier using the given training set, and then classifies
	 * all instances in the given test set. Before classifying, it removes the
	 * second attribute in both sets, that is the document title. This should be
	 * used when the WekaInstancesBuilder object used to create the sets has
	 * hasDocNames set to true. Returns list of distributions of classification
	 ...
	 * @param unknownDocs
	 *            The test documents to be deanonymized.
	 * @return The list of distributions of classification probabilities per
	 *         instance, or null if prepare was not previously called. Each result
	 *         in the list is a mapping from the author to its corresponding
	 *         classification probability.
	 */
	public Map<String, Map<String, Double>> classifyRemoveTitle(Instances trainingSet, Instances testSet,
			List<Document> unknownDocs) {
		// remove titles
	 ...
	 * @return The evaluation object with cross-validation results, or null if
	 *         failed running.
	 */
	@Override
	...
	/**
	 * Runs cross validation with given number of folds on the given Instances
	 * object. Uses 0 as default random seed for fold generation.
	 *
	 * @param data
	 *            The data to run CV over.
	 * @param folds
	 *            The number of folds to use.
	 * @return The evaluation object with cross-validation results, or null if did
	 *         not succeed running.
	 */
	...
	/**
	 * Produces the string array of the flags and descriptions for all of the
	 * arguments the classifier can take.
	 */
	@Override
